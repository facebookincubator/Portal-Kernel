	...
	uint32_t arg_size)
{
	...
	for (i = 0, j = 0; i < vfe_out_data->num_wm; i++) {
		...
		/*
		 * Check if the sub sample period for the
		 * RDI ports is supported.
		 */
		if (wm_data->index <= 2 && io_cfg->subsample_period > 3) {
			CAM_ERR(CAM_ISP,
		...
		/* update framedrop_pattern register */
		...
		/* update framedrop_period register */
		...
		/* update subsample_period register */
		...
		/* update subsample_pattern register */
		...
		/* set initial hfr configuration done */
