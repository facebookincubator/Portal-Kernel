	...
	struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	/* parse the eeporm probe information */
	probe_info->eeprom_name[sizeof(probe_info->eeprom_name) - 1] = '\0';
	s_ctrl->sensordata->slave_info.eeprom_slave_addr =
		probe_info->eeprom_slave_addr;
	s_ctrl->sensordata->slave_info.eeprom_id_reg_addr =
		probe_info->eeprom_id_reg_addr;
	s_ctrl->sensordata->slave_info.eeprom_id =
		probe_info->eeprom_id;
	s_ctrl->sensordata->slave_info.eeprom_id_mask =
		probe_info->eeprom_id_mask;
	s_ctrl->sensordata->slave_info.camera_project_id_reg_addr =
			probe_info->camera_project_id_reg_addr;
	s_ctrl->sensordata->slave_info.camera_project_id =
			probe_info->camera_project_id;
	s_ctrl->sensordata->slave_info.eeprom_name =
		probe_info->eeprom_name;

	/* parse privacy probe */
	s_ctrl->sensordata->slave_info.privacy_probe =
			probe_info->privacy_probe;
...
static uint16_t cam_sensor_id_by_mask(struct cam_sensor_ctrl_t *s_ctrl,
	uint32_t chipid, uint8_t module_type)
{
	...
	if (module_type)
		...
		sensor_id_mask = s_ctrl->sensordata->slave_info.eeprom_id_mask;
...
int cam_sensor_match_id(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	/* Headless driver workaround. */
	/* TODO: Look for cleaner solution. */
#ifdef CONFIG_VIDEO_TC358840
	if (slave_info->sensor_slave_addr == 0xFF) {
		CAM_ERR(CAM_SENSOR, "Force probing camera!!!");
		return 0;
	}
#endif

	if (slave_info->privacy_probe == 1)
	...
	CAM_DBG(CAM_SENSOR, "read id: 0x%x expected id 0x%x:",
			 chipid, slave_info->sensor_id);
	if (cam_sensor_id_by_mask(s_ctrl, chipid, 1) != slave_info->sensor_id) {
		CAM_ERR(CAM_SENSOR, "chip id %x does not match %x",
				chipid, slave_info->sensor_id);
		return -ENODEV;
	} else if (slave_info->eeprom_slave_addr) {
		orig_sensor_sid = s_ctrl->io_master_info.cci_client->sid;

		s_ctrl->io_master_info.cci_client->sid =
				slave_info->eeprom_slave_addr >> 1;

		/* Attempt to probe the the eeprom_id */
		rc = camera_io_dev_read(
			&(s_ctrl->io_master_info),
			slave_info->eeprom_id_reg_addr, &chipid,
			CAMERA_SENSOR_I2C_TYPE_WORD,
			CAMERA_SENSOR_I2C_TYPE_BYTE);

		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "read eeprom failed %s slave add 0x%x",
				s_ctrl->sensordata->slave_info.eeprom_name,
				slave_info->eeprom_slave_addr);

			s_ctrl->io_master_info.cci_client->sid = orig_sensor_sid;
			return -ENODEV;
		}

		CAM_DBG(CAM_SENSOR, "read eeprom id: 0x%x expected eeprom id 0x%x:",
				chipid, slave_info->eeprom_id);
		if (cam_sensor_id_by_mask(s_ctrl, chipid, 0) !=
				slave_info->eeprom_id) {
			CAM_ERR(CAM_SENSOR, "eeprom chip id 0x%x does not match 0x%x",
				chipid, slave_info->eeprom_id);
			s_ctrl->io_master_info.cci_client->sid = orig_sensor_sid;
			return -ENODEV;
		}

		/* Attempt to probe the the camera_project_id */
		rc = camera_io_dev_read(
			&(s_ctrl->io_master_info),
			slave_info->camera_project_id_reg_addr, &chipid,
			CAMERA_SENSOR_I2C_TYPE_WORD,
			CAMERA_SENSOR_I2C_TYPE_BYTE);

		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "read project id failed %s slave add 0x%x",
					s_ctrl->sensordata->slave_info.eeprom_name,
					slave_info->eeprom_slave_addr);
		...
		CAM_DBG(CAM_SENSOR, "read project id: 0x%x expected project id 0x%x:",
				chipid, slave_info->camera_project_id);
		if (chipid != slave_info->camera_project_id) {
			CAM_ERR(CAM_SENSOR, "project id chip id 0x%x does not match 0x%x",
					chipid, slave_info->camera_project_id);
			s_ctrl->io_master_info.cci_client->sid = orig_sensor_sid;
			return -ENODEV;
		}
	...
	void *arg)
{
	...
	case CAM_SENSOR_PROBE_CMD: {
		...
		CAM_INFO(CAM_SENSOR,"Probe success,slot:%d,"
			"slave_addr:0x%x,"
			"sensor_id:0x%x,"
			"eeprom_slave_addr:0x%x,"
			"eeprom_name:%s",
			s_ctrl->soc_info.index,
			s_ctrl->sensordata->slave_info.sensor_slave_addr,
			s_ctrl->sensordata->slave_info.sensor_id,
	...
	case CAM_QUERY_CAP: {
		...
		CAM_INFO(CAM_SENSOR,
			"CAM_QUERY_CAP Success, sensor_id:0x%x,sensor_slave_addr:0x%x",
		    s_ctrl->sensordata->slave_info.sensor_id,
		    s_ctrl->sensordata->slave_info.sensor_slave_addr);
		break;
	}
	...
	case CAM_CONFIG_DEV: {
		...
		if (s_ctrl->i2c_data.init_settings.is_settings_valid &&
			...
			CAM_INFO(CAM_SENSOR,
				"CAM_CONFIG_DEV Success with initial config, sensor_id:0x%x,sensor_slave_addr:0x%x",
				s_ctrl->sensordata->slave_info.sensor_id,
				s_ctrl->sensordata->slave_info.sensor_slave_addr);
		}
		...
		if (s_ctrl->i2c_data.config_settings.is_settings_valid &&
			...
			CAM_INFO(CAM_SENSOR,
				"CAM_CONFIG_DEV Success with settings config, sensor_id:0x%x,sensor_slave_addr:0x%x",
				s_ctrl->sensordata->slave_info.sensor_id,
				s_ctrl->sensordata->slave_info.sensor_slave_addr);
...
#ifdef CONFIG_VIDEO_TC358840
	/* Headless usecase */
	if (s_ctrl->sensordata->slave_info.sensor_slave_addr == 0xFF) {
		if (req_id == 0) {
			switch (opcode) {
			case CAM_SENSOR_PACKET_OPCODE_SENSOR_STREAMON: {
