...
static int eeprom_config_write_data(struct msm_eeprom_ctrl_t *e_ctrl,
	...
	if (cdata->cfg.write_data.num_bytes < 1 ||
		cdata->cfg.write_data.num_bytes > SZ_16M) {
		CDBG("%s: Invalid size. exp %u, req %u\n", __func__,
			e_ctrl->cal_data.num_data,
	...
	ptr_dest = vzalloc(cdata->cfg.write_data.num_bytes);

	if (copy_from_user(ptr_dest,
			cdata->cfg.write_data.dbuffer,
			cdata->cfg.write_data.num_bytes)) {
	...
	if (!e_ctrl->mem_map_array.memory_map[0].
			mem_settings[0].delay) {
		e_ctrl->mem_map_array.memory_map[0].
			mem_settings[0].delay = 1;
	}
	...
	for (i = 0; i < cdata->cfg.write_data.num_bytes; i++) {
		rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_write(
			&(e_ctrl->i2c_client),
			e_ctrl->mem_map_array.memory_map[0].
				mem_settings[0].reg_addr + i,
			ptr_dest[i],
			e_ctrl->mem_map_array.memory_map[0].
				mem_settings[0].data_type);
		msleep(e_ctrl->mem_map_array.memory_map[0].
			mem_settings[0].delay);
	...
	rc = eeprom_parse_memory_map(e_ctrl, &(e_ctrl->mem_map_array));
	if (rc < 0) {
		pr_err("%s:%d memory map parse failed\n",
			__func__, __LINE__);
		goto power_down;
	...
	void __user *arg)
{
	...
	rc = copy_to_user(ptr_dest, e_ctrl->cal_data.mapdata,
		cdata.cfg.read_data.num_bytes);
	pr_err("%s:%d eeprom_config_read_cal_data32\n",
				__func__, __LINE__);
	return rc;
}

static int eeprom_config_write_data32(struct msm_eeprom_ctrl_t *e_ctrl,
		void __user *arg)
{
	int rc = 0;
	struct msm_eeprom_cfg_data32 *cdata32 =
	...
	cdata.cfg.write_data.dbuffer =
		(uint8_t *) compat_ptr(cdata32->cfg.write_data.dbuffer);
	...
	void __user *argp)
{
	...
	power_info = &(e_ctrl->eboard_info->power_info);
	mem_map_array = &(e_ctrl->mem_map_array);
	power_setting_array = &(e_ctrl->power_settings_array);
	...
	void __user *argp)
{
	...
	case CFG_EEPROM_WRITE_DATA:
		CDBG("%s E CFG_EEPROM_WRITE_DATA\n", __func__);
		rc = eeprom_config_write_data32(e_ctrl, cdata);
		break;
	case CFG_EEPROM_INIT:
		if (e_ctrl->userspace_probe == 0) {
			pr_err("%s:%d Eeprom already probed at kernel boot",
