...
#ifdef CONFIG_VIDEO_TC358840
#include <linux/tc358840.h>
#endif
#undef CDBG
...
int msm_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	for (retry = 0; retry < 3; retry++) {
		...
		if (s_ctrl->sensor_i2c_client->cci_client->sid == 0xFF >> 1 &&
		    s_ctrl->sensor_i2c_client->spi_client) {
			spi_client = s_ctrl->sensor_i2c_client->spi_client;
			init_completion(&spi_client->irq_done);
			spi_client->power_up_pending = 1;
		}
...
static uint16_t msm_sensor_id_by_mask(struct msm_sensor_ctrl_t *s_ctrl,
	uint16_t chipid, uint8_t module_type)
{
	uint16_t sensor_id = chipid;
	int16_t sensor_id_mask;

	if (module_type)
		sensor_id_mask = s_ctrl->sensordata->slave_info->sensor_id_mask;
	else
		sensor_id_mask = s_ctrl->sensordata->cam_slave_info->
			eeprom_id_info.eeprom_id_mask;
...
int msm_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	slave_info = s_ctrl->sensordata->slave_info;
	sensor_name = s_ctrl->sensordata->sensor_name;
	eeprom_name = s_ctrl->sensordata->eeprom_name;
	cam_slave_info = s_ctrl->sensordata->cam_slave_info;

	if (!sensor_i2c_client || !slave_info ||
		!sensor_name || !cam_slave_info) {
		pr_err("%s:%d failed: %pK %pK %pK %pK\n",
	...
	if (sensor_i2c_client->cci_client->sid == 0xFF >> 1) {
#ifdef CONFIG_VIDEO_TC358840
		/* When headless playback camera driver is used,
		*  timeout time have to be decreased.
		*/
		if (!strcmp(sensor_name, "tc358840xbg"))
			timeout_ms = 1000;
#endif
		rc = 0;
		if (s_ctrl->sensor_i2c_client->spi_client) {
			spi_client = s_ctrl->sensor_i2c_client->spi_client;
			if (!wait_for_completion_timeout(
				&spi_client->irq_done,
				msecs_to_jiffies(timeout_ms))) {
				pr_err("NOT booted within %dms\n",
						timeout_ms);
			}
			spi_client->power_up_pending = 0;
		} else
			pr_debug("%s: SPI client not attached!", __func__);

		pr_err("Probe successful -- forced ...\n");

		return rc;
	}

	pr_err("%s: probe sensor_id:%d privacy_mode:%d\n", __func__,
		slave_info->sensor_id, s_ctrl->privacy_probe);
	if (s_ctrl->privacy_probe == 1)
	...
	if (msm_sensor_id_by_mask(s_ctrl, chipid, 1) != slave_info->sensor_id) {
		pr_err("%s chip id %x does not match %x\n",
				__func__, chipid, slave_info->sensor_id);
		return -ENODEV;
	} else if (cam_slave_info->eeprom_slave_addr) {

		uint16_t orig_sensor_sid =
			sensor_i2c_client->cci_client->sid;

		sensor_i2c_client->cci_client->sid =
			cam_slave_info->eeprom_slave_addr >> 1;

		rc = sensor_i2c_client->i2c_func_tbl->i2c_read(
			sensor_i2c_client,
			cam_slave_info->eeprom_id_info.eeprom_id_reg_addr,
			&chipid, MSM_CAMERA_I2C_BYTE_DATA);
		if (rc < 0) {
			pr_err("%s: %s: read eeprom failed\n",
		...
		if (msm_sensor_id_by_mask(s_ctrl, chipid, 0) !=
			cam_slave_info->eeprom_id_info.eeprom_id) {
			pr_err("%s eeprom vendor id %x does not match %x\n",
				__func__, chipid,
				cam_slave_info->eeprom_id_info.eeprom_id);
			sensor_i2c_client->cci_client->sid =
				orig_sensor_sid;
	...
	void __user *argp)
{
	...
	case CFG_SPI_WRITE: {
		...
		uint8_t *data = NULL;
		uint8_t i = 0;
		struct sensor_spi_write_cfg_32 spi_write_cfg32;

		if (copy_from_user(&spi_write_cfg32,
			(void *)compat_ptr(cdata->cfg.setting),
			sizeof(struct sensor_spi_write_cfg_32))) {
			pr_err("%s:%d failed\n", __func__, __LINE__);
			rc = -EFAULT;
			break;
		}

		CDBG("%s: spi_write_cfg length = %d\n", __func__,
			spi_write_cfg32.length);

		data = kzalloc(spi_write_cfg32.length * sizeof(uint8_t), GFP_KERNEL);
		if (!data) {
			pr_err("%s:%d failed\n", __func__, __LINE__);
			rc = -ENOMEM;
			break;
		...
		if (copy_from_user(data,
			(void *)(compat_ptr(spi_write_cfg32.data)),
			spi_write_cfg32.length * sizeof(uint8_t))) {
			pr_err("%s:%d failed\n", __func__, __LINE__);
			kfree(data);
			rc = -EFAULT;
			break;
		...
		for(i = 0; i < spi_write_cfg32.length; i++) {
			CDBG("%s: data[%d] = 0x%x\n", __func__, i, data[i]);
		}

		if(s_ctrl->sensor_i2c_client->spi_client) {
			spi_write(s_ctrl->sensor_i2c_client->spi_client->spi_master,
				data, spi_write_cfg32.length);
		} else {
			pr_debug("%s: SPI client not attached!", __func__);
		}
		...
		if (s_ctrl->cci_debug) {
			/* CCI DEBUG CODE TOP */
			reg_setting = kzalloc(4 * (sizeof(struct msm_camera_i2c_reg_array)), GFP_KERNEL);

			reg_setting[0].reg_addr = 0x1020;
			reg_setting[0].reg_data = 0x3040;

			reg_setting[1].reg_addr = 0x1234;
			reg_setting[1].reg_data = 0x5678;

			reg_setting[2].reg_addr = 0x5555;
			reg_setting[2].reg_data = 0xAAAA;

			reg_setting[3].reg_addr = 0xFFFF;
			reg_setting[3].reg_data = 0x0000;
			...
			pr_err("CCI_DEBUG: writing test array to CCI!\n");
			...
			rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->
				...
				0xAA,
				&test_data, MSM_CAMERA_I2C_WORD_DATA);
	...
	case CFG_WRITE_I2C_ARRAY_ASYNC: {
		...
		if (s_ctrl->sensor_i2c_client->cci_client->sid == 0xFF >> 1) {
			if (conf_array.size != 0 &&
				conf_array.reg_setting[0].reg_addr == 0xFFFF) {
				pr_err("%s: first address is 0xFFFF skip this array ...\n"
						, __func__);
				goto DONE;
#ifdef CONFIG_VIDEO_TC358840
			} else if (conf_array.size != 0 &&
				!strcmp(sensor_name, "tc358840xbg") &&
				conf_array.reg_setting[0].reg_addr == 0xFF01) {
				tc358840_start_stream();
				goto DONE;
			} else if (conf_array.size != 0 &&
				!strcmp(sensor_name, "tc358840xbg") &&
				conf_array.reg_setting[0].reg_addr == 0xFF00) {
				tc358840_stop_stream();
				goto DONE;
#endif
			}

			data = kzalloc(conf_array.size * sizeof(uint8_t), GFP_KERNEL);
			if (!data) {
				pr_err("%s:%d failed\n", __func__, __LINE__);
				rc = -ENOMEM;
				break;
			...
			for(i = 0; i < conf_array.size; i++) {
				data[i] = conf_array.reg_setting[i].reg_data;
			}

			if (s_ctrl->sensor_i2c_client->spi_client) {
				spi_client =
					s_ctrl->sensor_i2c_client->spi_client;
				spi_write(spi_client->spi_master,
					data, conf_array.size);
			} else
				pr_debug("%s: SPI client not attached!",
						__func__);
		...
		if (s_ctrl->is_csid_tg_mode ||
			s_ctrl->sensor_i2c_client->cci_client->sid == 0xFF >> 1)
		goto DONE;
