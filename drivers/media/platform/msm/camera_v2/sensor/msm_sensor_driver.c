...
static ssize_t cci_debug_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct msm_sensor_ctrl_t *s_ctrl = dev_get_drvdata(dev);

	return sprintf(buf, "%s\n", s_ctrl->cci_debug ? "enable" : "disable");
}

static ssize_t cci_debug_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct msm_sensor_ctrl_t *s_ctrl = dev_get_drvdata(dev);

	s_ctrl->cci_debug = strstr(buf, "enable") ? true : false;

	return count;
}
static const DEVICE_ATTR_RW(cci_debug);
...
static int msm_cam_i2c_read_temp(struct thermal_zone_device *thermal,
				int *temp)
{
	struct msm_sensor_ctrl_t *s_ctrl = thermal->devdata;
	int ret = 0;

	if (!s_ctrl || !s_ctrl->sensordata->thermal
		|| s_ctrl->sensordata->thermal->temp_register == UINT_MAX
	...
	*temp = 0;
	ret = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(
		s_ctrl->sensor_i2c_client,
		s_ctrl->sensordata->thermal->temp_register,
		(uint16_t *)temp, MSM_CAMERA_I2C_BYTE_DATA);
	mutex_unlock(s_ctrl->msm_sensor_mutex);

	if (*temp < s_ctrl->sensordata->thermal->temp_range[CAM_TEMP_MIN])
		*temp = s_ctrl->sensordata->thermal->temp_range[CAM_TEMP_MIN];
	else if (*temp > s_ctrl->sensordata->thermal->temp_range[CAM_TEMP_MAX])
		*temp = s_ctrl->sensordata->thermal->temp_range[CAM_TEMP_MAX];

	return ret;
}

static int msm_cam_isp_read_temp(struct thermal_zone_device *thermal,
				int *temp)
{
	*temp = 0;
	return 0;
...
static struct thermal_zone_device_ops msm_camera_thermal_ops = {
	.get_temp = msm_cam_i2c_read_temp,
};

static int32_t msm_sensor_thermal_setup(struct msm_sensor_ctrl_t *s_ctrl)
{
	struct thermal_zone_device *tz;
	struct device_node *of = s_ctrl->of_node;
	char thermal_type[THERMAL_NAME_LENGTH];

	if (!of_find_property(of, "qcom,cam-thermal", NULL))
		return -ENODEV;

	s_ctrl->sensordata->thermal = devm_kzalloc(&s_ctrl->pdev->dev,
			sizeof(struct msm_camera_thermal_info), GFP_KERNEL);
	if (!s_ctrl->sensordata->thermal)
		return -ENOMEM;

	s_ctrl->sensordata->thermal->temp_register = UINT_MAX;
	of_property_read_u32(of, "qcom,cam-temp-reg",
			&s_ctrl->sensordata->thermal->temp_register);
	if (of_property_read_u32_array(of, "qcom,cam-temp-range",
		s_ctrl->sensordata->thermal->temp_range, CAM_TEMP_RANGE)) {
		s_ctrl->sensordata->thermal->temp_range[CAM_TEMP_MIN] =
								INT_MIN;
		s_ctrl->sensordata->thermal->temp_range[CAM_TEMP_MAX] =
								INT_MAX;
	}

	snprintf(thermal_type, THERMAL_NAME_LENGTH, "msm-cam-sensor.%d",
								s_ctrl->id);
	tz = thermal_zone_device_register(thermal_type, 0, 0,
				s_ctrl, &msm_camera_thermal_ops, NULL, 0, 0);
	if (IS_ERR(tz)) {
		dev_warn(&s_ctrl->pdev->dev, "failed to register thermal zone, err %lx\n",
								-PTR_ERR(tz));
		devm_kfree(&s_ctrl->pdev->dev, s_ctrl->sensordata->thermal);
		s_ctrl->sensordata->thermal = NULL;
	}
	...
	struct msm_sensor_info_t *probed_info, char *entity_name)
{
	...
	if (is_compat_task()) {
		...
		slave_info->privacy_probe = slave_info32->privacy_probe;

		slave_info->i2c_freq_mode = slave_info32->i2c_freq_mode;
		slave_info->sensor_id_info = slave_info32->sensor_id_info;
		slave_info->eeprom_id_info = slave_info32->eeprom_id_info;

		slave_info->slave_addr = slave_info32->slave_addr;
		slave_info->eeprom_slave_addr = slave_info32->eeprom_slave_addr;
	...
	if (s_ctrl->is_probe_succeed == 1) {
		...
		if (slave_info->sensor_id_info.sensor_id ==
			...
			!strcmp(slave_info->eeprom_name, s_ctrl->sensordata->
				cam_slave_info->eeprom_name)) {
...
CSID_TG:
	...
	*  sensor control structure */
	...
	/* Pass eeprom slave info */
	s_ctrl->sensordata->cam_slave_info = slave_info;

	/* Power up and probe sensor */
	s_ctrl->privacy_probe = slave_info->privacy_probe;
	...
	pr_err("%s probe succeeded (eeprom = %s)",
		slave_info->sensor_name, slave_info->eeprom_name);
	...
	/* Clear the privacy probe flag */
	s_ctrl->privacy_probe = 0;
	...
	/* A very ugly way to check that sensor is behind an ISP... */
	if (slave_info->slave_addr == 0xFF && s_ctrl->sensordata->thermal)
		msm_camera_thermal_ops.get_temp = msm_cam_isp_read_temp;
...
static int32_t msm_sensor_driver_platform_probe(struct platform_device *pdev)
{
	...
	rc = sysfs_create_file(&pdev->dev.kobj, &dev_attr_cci_debug.attr);

	msm_sensor_thermal_setup(s_ctrl);
...
static irqreturn_t device_powerup_isp(int irq, void *data)
{
	struct msm_camera_spi_client *spi_client = data;

	if (spi_client) {
		if (spi_client->power_up_pending)
			complete(&spi_client->irq_done);
		else
			CDBG("spi_client->power_up_pending is 0\n");
	} else {
		pr_err("spi_client is null...\n");
	}

	return IRQ_HANDLED;
}

static int msm_sensor_spi_setup(
	struct spi_device *spi, int gpio, enum of_gpio_flags gpio_flags)
{
	struct msm_sensor_ctrl_t *s_ctrl = NULL;
	...
	pr_err("%s: sctrl %p client %p\n",
		__func__, s_ctrl, s_ctrl->sensor_i2c_client);
	...
	client = s_ctrl->sensor_i2c_client;

	spi_client = (struct msm_camera_spi_client *)kzalloc(sizeof(*spi_client), GFP_KERNEL);
	...
	spi_client->device_gpio_irq = gpio;
	spi_client->device_gpio_irq_flags = gpio_flags;

	if (gpio_is_valid(spi_client->device_gpio_irq))
		spi_client->device_interrupt =
			gpio_to_irq(spi_client->device_gpio_irq);

	spi_client->power_up_pending = 0;
	if (spi_client->device_interrupt > 0) {
		uint32_t flags = IRQF_SHARED;
		uint32_t ret = 0;

		flags |= gpio_flags ?
			IRQF_TRIGGER_RISING : IRQF_TRIGGER_FALLING;
		ret = request_irq(spi_client->device_interrupt,
			device_powerup_isp,
			flags, "spi_irq", spi_client);
		if (ret) {
			pr_err("Unable to claim irq %d, err = %d\n",
				spi_client->device_interrupt, ret);
		}
	}
...
static int msm_sensor_spi_probe(struct spi_device *spi)
{
	int irq, cs, cpha, cpol, cs_high, lsb;
	int gpio_irq;
	enum of_gpio_flags irq_active_flag;
	struct device_node *of_node = NULL;

	dev_err(&spi->dev, "%s: Enter\n", __func__);

	of_node = spi->dev.of_node;

	spi->bits_per_word = 8;
	spi->mode = SPI_MODE_3;
	spi_setup(spi);

	gpio_irq = of_get_named_gpio_flags(of_node, "gpio-irq", 0,
			&irq_active_flag);
	...
	lsb = (spi->mode & SPI_LSB_FIRST) ? 1 : 0;
	pr_err("%s: irq[%d] cs[%x] CPHA[%x] CPOL[%x] CS_HIGH[%x] LSB[%x]\n",
		__func__, irq, cs, cpha, cpol, cs_high, lsb);
	pr_err("%s: max_speed[%u]\n", __func__, spi->max_speed_hz);

	return msm_sensor_spi_setup(spi, gpio_irq, irq_active_flag);
}

static int msm_sensor_spi_remove(struct spi_device *spi)
{
	struct msm_sensor_ctrl_t *s_ctrl = NULL;
	struct msm_camera_spi_client *spi_client =
		s_ctrl->sensor_i2c_client->spi_client;

	s_ctrl = g_sctrl[0];

	if (spi_client && spi_client->device_interrupt > 0)
		free_irq(spi_client->device_interrupt, spi_client);

	kfree(s_ctrl->sensor_i2c_client->spi_client);

	return 0;
}

static const struct of_device_id msm_sensor_spi_dt_match[] = {
	{.compatible = "qcom,sensor_spi"},
	{}
};

static struct spi_driver msm_sensor_spi_driver = {
	.driver = {
		.name = "sensor_spi",
		.owner = THIS_MODULE,
		.of_match_table = msm_sensor_spi_dt_match,
	},
	.probe = msm_sensor_spi_probe,
	.remove = msm_sensor_spi_remove,
};
...
static int __init msm_sensor_driver_init(void)
{
	...
	rc = spi_register_driver(&msm_sensor_spi_driver);
