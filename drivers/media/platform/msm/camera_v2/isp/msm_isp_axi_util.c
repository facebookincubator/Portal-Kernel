	...
	struct msm_vfe_axi_stream *stream_info)
{
	...
	if (stream_info->num_isp == 0) {
		...
		stream_info->controllable_stream_skip = 0;
		stream_info->activated_framedrop_period =
					MSM_VFE_STREAM_STOP_PERIOD;
		if (stream_cfg_cmd->controllable_output) {
			/* Honor the user space request for skip all pattern */
			if (stream_cfg_cmd->frame_skip_pattern == SKIP_ALL) {
				stream_info->controllable_stream_skip = 1;
				stream_info->frame_skip_pattern = SKIP_ALL;
			} else {
				/*Initial skip state */
	...
	struct msm_vfe_axi_stream_request_cmd *stream_cfg_cmd)
{
	...
	case V4L2_PIX_FMT_YVU420:
		/* Set num_planes = 1 so that ISP can write whole frame with RDI */
		stream_info->num_planes = 1;
		stream_info->format_factor = ISP_Q2;
		break;
	...
	struct msm_isp_timestamp *ts, struct msm_isp_sof_info *sof_info)
{
	...
	for (i = 0; i < RDI_INTF_0; i++) {
		...
		if (stream_info->controllable_output && (stream_info->
			controllable_stream_skip || !vfe_dev->reg_updated)) {
	...
	if (done_buf &&
		...
		if (stream_info->sw_ping_pong_bit != -1 &&
			...
			rc = msm_isp_cfg_ping_pong_address(
				...
				done_buf);
			if (rc < 0) {
				ISP_DBG("%s:config error\n", __func__);
				drop_buffer = 1;
			}
		} else if (stream_info->controllable_stream_skip) {
			drop_buffer = 1;
		}
		if (drop_buffer) {
		...
		/* Do not reset framedrop when skip all desired for */
		/* controllable streams */
		if (stream_info->controllable_stream_skip == 0) {
			...
			vfe_dev->axi_data.src_info[VFE_PIX_0].accept_frame =
				false;
		}
	...
	uint32_t frame_id, uint32_t buf_index)
{
	...
	*/
	...
	stream_cfg_cmd.frame_skip_pattern = stream_info->frame_skip_pattern;
