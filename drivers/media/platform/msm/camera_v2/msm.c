...
static atomic_t qosusers = ATOMIC_INIT(0);
...
void msm_pm_qos_add_request(void)
{
	/* first user */
	if (atomic_inc_return(&qosusers) == 1) {
...
void msm_pm_qos_remove_request(void)
{
	/* last user */
	if (atomic_dec_and_test(&qosusers)) {
