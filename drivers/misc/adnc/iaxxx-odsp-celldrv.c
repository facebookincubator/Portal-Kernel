...
static inline bool iaxxx_should_suspend(struct fbp_notifier_info *info)
{
	return fbp_enter_privacy(info);
}

static inline bool iaxxx_should_resume(struct fbp_notifier_info *info)
{
	return fbp_exit_privacy(info);
}
							...
							unsigned long arg)
{
	...
	case ODSP_PLG_SET_PARAM_BLK:
		...
		blk_buff = vmemdup_user(blk_buff, param_blk_info.param_size);
...
static int odsp_dev_open(struct inode *inode, struct file *file)
{
	...
	if (!test_bit(ODSP_FLG_FW_RADY, &odsp_dev_priv->flags))
		return -EPERM;

	pm_runtime_get_sync(odsp_dev_priv->dev);
...
static int odsp_dev_release(struct inode *inode, struct file *file)
{
	...
	pm_runtime_put(odsp_dev_priv->dev);
...
static int iaxxx_odsp_notifier_cb(struct notifier_block *nb,
		unsigned long val, void *data)
{
	struct odsp_device_priv *dsp_dev_priv =
			container_of(nb, struct odsp_device_priv, nb_core);

	switch (val) {
	case IAXXX_EV_STARTUP:
	case IAXXX_EV_RECOVERY:
		/* Allow open odsp device */
		set_bit(ODSP_FLG_FW_RADY, &dsp_dev_priv->flags);
		break;

	case IAXXX_EV_CRASH:
		/* Block open odsp device and ioctl's to it */
		clear_bit(ODSP_FLG_FW_RADY, &dsp_dev_priv->flags);
		break;
	}
...
static int iaxxx_odsp_dev_probe(struct platform_device *pdev)
{
	...
	dev_set_drvdata(&pdev->dev, odsp_dev_priv);
	pr_info("ODSP device cdev initialized.\n");

	odsp_dev_priv->nb_core.notifier_call = iaxxx_odsp_notifier_cb;
	ret = iaxxx_fw_notifier_register(priv->dev, &odsp_dev_priv->nb_core);
	if (ret) {
...
static const struct dev_pm_ops iaxxx_odsp_pm_ops = {
	SET_RUNTIME_PM_OPS(iaxxx_odsp_rt_suspend, iaxxx_odsp_rt_resume, NULL)
};
