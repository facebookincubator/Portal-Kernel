...
int mha_fw_populate(struct mha_ctx *mha)
{
	...
	rval = adap->pm_get ? adap->pm_get(adap) : 0;
	if (rval)
		return rval;
...
static int mha_open(struct inode *inode, struct file *file)
{
	...
	struct mha_ctx *mha = dev_get_drvdata(mdev->parent);
...
static int __init mha_core_init(void)
{
	...
	mha_class = class_create(THIS_MODULE, "mha");
	if (IS_ERR_OR_NULL(mha_class)) {
		pr_err("mha: Can't create MH class ; ret %ld\n",
			PTR_ERR(mha_class));
		return PTR_ERR(mha_class);
	}

	dev = device_create(mha_class, NULL, 0, mha, "mha-core");
	if (IS_ERR_OR_NULL(dev)) {
		pr_err("mha: Can't create MH Core device ; ret %ld\n",
			PTR_ERR(dev));
		return PTR_ERR(dev);
	}
	...
	if (rval) {
		pr_err("mha: Can't create MH Acousting misc device!\n");
		return rval;
	}
