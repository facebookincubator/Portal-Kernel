...
tlc591xx_set_mode(struct regmap *regmap, u8 mode)
{
	...
	val = MODE2_OCH_STOP | MODE2_BLINK | mode;
...
static int
tlc591xx_blink_delay(struct tlc591xx_priv *priv,
		     unsigned long delay_on,
		     unsigned long delay_off)
{
	int err;
	int val;
	unsigned long blink_period = delay_on + delay_off;

	val = (blink_period * 24) / 1000 - 1;
	if (val < 0)
		val = 0;

	val = (val < 255) ? val : 255;

	err = regmap_write(priv->regmap, TLC591XX_REG_GRPPWM,
			   (delay_on * 256) / blink_period);
	if (!err)
		err = regmap_write(priv->regmap, TLC591XX_REG_GRPFREQ, val);

	return err;
}

static int
tlc591xx_blink_set(struct led_classdev *led_cdev,
		   unsigned long *delay_on,
		   unsigned long *delay_off)
{
	struct tlc591xx_led *led = ldev_to_led(led_cdev);
	...
	/* Do nothing if delay_on or delay_off are zero */
	if (!*delay_on || !*delay_off || !brightness)
		return 0;

	mutex_lock(&led->mutex);
	err = tlc591xx_blink_delay(priv, *delay_on, *delay_off);
	if (err)
		goto err;
	err = tlc591xx_set_ledout(priv, led, LEDOUT_BLINK);
	if (err)
		goto err;
	err = tlc591xx_set_pwm(priv, led, brightness);
err:
	if (err)
		dev_err(led->ldev.dev, "Failed configure blinking\n");
		   ...
		   const struct tlc591xx *tlc591xx)
{
	...
	for (i = 0; i < TLC591XX_MAX_LEDS; i++) {
		...
		led->ldev.blink_set = tlc591xx_blink_set;
	       ...
	       const struct i2c_device_id *id)
{
	...
	for_each_child_of_node(np, child) {
		...
		mutex_init(&priv->leds[reg].mutex);
	}
...
static void
tlc591xx_shutdown(struct i2c_client *client)
{
	struct tlc591xx_priv *priv = i2c_get_clientdata(client);
	unsigned int i;
	unsigned int offset = priv->reg_ledout_offset;

	/* Set all LEDs to off */
	for (i = 0; i < priv->max_leds >> 2; i++)
		regmap_write(priv->regmap, offset + i, LEDOUT_OFF);
}
