...
static struct list_head panel_ops_list = LIST_HEAD_INIT(panel_ops_list);
static DEFINE_MUTEX(panel_ops_mutex);

int mdss_panel_ops_add(struct mdss_panel_ops *ops)
{
	mutex_lock(&panel_ops_mutex);

	INIT_LIST_HEAD(&ops->item);
	list_add(&ops->item, &panel_ops_list);

	mutex_unlock(&panel_ops_mutex);

	return 0;
}


int mdss_panel_ops_del(struct mdss_panel_ops *ops)
{
	mutex_lock(&panel_ops_mutex);

	list_del_init(&ops->item);
...
struct mdss_panel_ops *mdss_panel_ops_find(const char *name)
{
	struct mdss_panel_ops *ops, *find = NULL;

	mutex_lock(&panel_ops_mutex);

	list_for_each_entry(ops, &panel_ops_list, item)
...
static void mdss_dsi_panel_bklt_pwm(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	...
	if (level == 0) {
		if (ctrl->pwm_enabled) {
			...
			if (pinfo->backlight_post_off_sleep > 0)
				msleep(pinfo->backlight_post_off_sleep);
		}
		ctrl->pwm_enabled = 0;
		return;
	}
	...
	if (!ctrl->pwm_enabled) {
		...
		mdelay(1);
		if (gpio_is_valid(ctrl->bklt_en_gpio)) {
			ret = gpio_direction_output(ctrl->bklt_en_gpio, 1);
			if (ret)
...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		if (!ctrl_pdata->rst_gpio_always_on)
...
exit:
	if (ctrl_pdata->panel_ops && ctrl_pdata->panel_ops->power_on)
		ctrl_pdata->panel_ops->power_on(ctrl_pdata->panel_ops, enable);
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	if (ctrl->panel_ops && ctrl->panel_ops->video_on) {
		struct mdss_panel_info *pinfo = &pdata->panel_info;
		ctrl->panel_ops->video_on(ctrl->panel_ops, true, pinfo);
	}
...
static int mdss_dsi_panel_off(struct mdss_panel_data *pdata)
{
	...
	if (ctrl->panel_ops && ctrl->panel_ops->video_on)
		ctrl->panel_ops->video_on(ctrl->panel_ops, false, NULL);
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	rc = of_property_read_u32(np,
		"qcom,mdss-dsi-off-delay-send-rst-ms", &tmp);
	pinfo->delay_send_rst_ms = (!rc ? tmp : 0);

	rc = of_property_read_u32(np,
		"qcom,mdss-backlight-post-off-sleep", &tmp);
	pinfo->backlight_post_off_sleep = (!rc ? tmp : 0);
	...
	pinfo->is_ops_panel = of_property_read_bool(np, "qcom,ops-panel");
