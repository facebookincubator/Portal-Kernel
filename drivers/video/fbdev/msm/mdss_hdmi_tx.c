...
#include <trace/events/fb_hdmi.h>
...
static void hdmi_panel_update_colorimetry(struct hdmi_tx_ctrl *ctrl,
		bool use_bt2020);
static int hdmi_tx_get_vend_prod_edid_blk(struct platform_device *pdev,
	struct msm_ext_disp_vend_prod_edid_blk *blk);
...
static int hdmi_tx_init_ext_disp(struct hdmi_tx_ctrl *hdmi_ctrl)
{
	...
	hdmi_ctrl->ext_audio_data.codec_ops.get_vend_prod_edid_blk =
		hdmi_tx_get_vend_prod_edid_blk;
...
static void hdmi_tx_update_deep_color(struct hdmi_tx_ctrl *hdmi_ctrl)
{
	...
	if (deep_color & BIT(4)) {
		pinfo->deep_color |= MDP_DEEP_COLOR_YUV420_30B;
		hdmi_ctrl->dc_support = true;
		pinfo->bpp = 30;
	}
}
...
static int hdmi_tx_notify_events(struct hdmi_tx_ctrl *hdmi_ctrl, int val)
{
	...
	if (!rc) {
		pr_info("%s: Successfully sent %s notification\n", __func__,
			val ? "CONNECT" : "DISCONNECT");
		trace_fb_hdmi_connect(val);
	} else {
	...
	struct msm_ext_disp_vend_prod_edid_blk *blk)
{
	...
	return hdmi_edid_get_vendor_product_blk(
		hdmi_tx_get_fd(HDMI_TX_FEAT_EDID), blk);
}
	...
	bool polarity)
{
	...
	trace_fb_hdmi_polarity(polarity);
...
static int hdmi_tx_power_on(struct hdmi_tx_ctrl *hdmi_ctrl)
{
	...
	pr_info("%s: setting pixel clk %d\n", __func__, pixel_clk);

	info = &hdmi_ctrl->timing;
	trace_fb_hdmi_tx_on(info->active_h, info->active_v,
		info->interlaced, info->refresh_rate / 1000,
		pixel_clk/1000,
		hdmi_ctrl->panel.pinfo->out_format == MDP_Y_CBCR_H2V2,
		get_bitdepth(hdmi_ctrl->panel.bitdepth),
		hdmi_ctrl->panel.dc_enable,
		hdmi_ctrl->panel.pinfo->hdr_properties.hdr_enabled,
		info->video_format);
...
static int hdmi_tx_probe(struct platform_device *pdev)
{
	...
	if (mdss_debug_register_io("hdmi",
		&hdmi_ctrl->pdata.io[HDMI_TX_CORE_IO], &dbg_blk))
		DEV_WARN("%s: hdmi_tx debugfs register failed\n", __func__);

	mdss_debug_register_dump_range(pdev, dbg_blk, "qcom,regs-dump-hdmi",
		"qcom,regs-dump-names-hdmi", "qcom,regs-dump-xin-id-hdmi");
