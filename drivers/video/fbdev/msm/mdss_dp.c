...
#define DP_RESUME_CHECK_COUNT	3
#define DP_RESUME_WAIT_MS	100
...
static int dp_get_vend_prod_edid_blk(struct platform_device *pdev,
	struct msm_ext_disp_vend_prod_edid_blk *blk)
{
	...
	rc = hdmi_edid_get_vendor_product_blk
			(dp->panel_data.panel_info.edid_data, blk);
	if (rc)
		DEV_ERR("%s:edid_get_audio_blk failed\n", __func__);
...
static int mdss_dp_init_ext_disp(struct mdss_dp_drv_pdata *dp)
{
	...
	dp->ext_audio_data.codec_ops.teardown_done =
		dp_audio_teardown_done;
	dp->ext_audio_data.codec_ops.get_vend_prod_edid_blk =
		dp_get_vend_prod_edid_blk;
...
static int mdss_dp_enable_mainlink_clocks(struct mdss_dp_drv_pdata *dp)
{
	...
	if (dp->pixel_parent && dp->pixel_clk_two_div &&
		...
		/* WORKAROUND: Using pixel_clk_four_div when link
		* rate is 540Mbps causes a number of black screen,
		* pop noise on the DELL 4K monitors, beside causing
		* UI to slow down noticebly.
		*/
		...
		*		dp->pixel_clk_four_div);
		*	dp->use_pixel_clk_four_div = 1;
		* } else
		*/
		{
			clk_set_parent(dp->pixel_parent,
				dp->pixel_clk_two_div);
			dp->use_pixel_clk_four_div = 0;
		}
	}
...
int mdss_dp_on(struct mdss_panel_data *pdata)
{
	...
	bool hpd;
	int rc = 0;
	int wait_hpd_cnt = DP_RESUME_CHECK_COUNT;
	...
	if (!dp_drv->alt_mode.dp_status.hpd_high) {
		/* If we are here, TV could possibly be coming out of suspend.
		 * Check to see if there is a high, before giving up. Giving up
		 * here could possibly lead to a screen that is not turned on.
		 */
		pr_warn("DP sink probably in suspend/resume, retrying...\n");
		while (wait_hpd_cnt) {
			if (dp_drv->alt_mode.dp_status.hpd_high)
				break;

			if (wait_hpd_cnt-- == 0) {
				pr_err("DP status is low\n");
				return -EINVAL;
			}

			msleep(DP_RESUME_WAIT_MS);
		}
	}
...
static int mdss_dp_host_init(struct mdss_panel_data *pdata)
{
	...
	ptn36502_setmode(DP4_LANE, dp_drv->orientation);
