...
static int mdss_dsi_panel_power_off(struct mdss_panel_data *pdata)
{
	...
	if (pinfo->delay_send_rst_ms > 0)
		msleep(pinfo->delay_send_rst_ms);
...
static void mdss_dsi_panel_ops_work(struct work_struct *work)
{
	struct delayed_work *dw = to_delayed_work(work);
	struct mdss_dsi_ctrl_pdata *ctrl_pdata;

	ctrl_pdata = container_of(dw, struct mdss_dsi_ctrl_pdata, ops_work);
	if (!ctrl_pdata) {
		pr_err("%s: invalid ctrl data\n", __func__);
		return;
	}

	ctrl_pdata->panel_ops = mdss_panel_ops_find("sn65dsi86");
	if (IS_ERR_OR_NULL(ctrl_pdata->panel_ops))
		queue_delayed_work(ctrl_pdata->workq, &ctrl_pdata->ops_work, HZ);
}
				  ...
				  int event, void *arg)
{
	...
	case MDSS_EVENT_FB_REGISTERED:
		...
		if (pdata->panel_info.is_ops_panel) {
			queue_delayed_work(ctrl_pdata->workq,
				&ctrl_pdata->ops_work, HZ);
		}
		break;
...
static int mdss_dsi_ctrl_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&ctrl_pdata->ops_work, mdss_dsi_panel_ops_work);
