...
#define EDID_VEND_ID_OFFSET     0x08
#define EDID_VEND_PROD_SIZE     10
...
struct hdmi_edid_ctrl {
	...
	u8 vendor_product_block[EDID_VEND_PROD_SIZE];
...
int hdmi_edid_reset_parser(void *input)
{
	...
	memset(edid_ctrl->vendor_product_block, 0,
		sizeof(edid_ctrl->vendor_product_block));
...
static void hdmi_edid_extract_vendor_product_block(
				struct hdmi_edid_ctrl *edid_ctrl)
{
	...
	memcpy(edid_ctrl->vendor_product_block,
			&edid_ctrl->edid_buf[EDID_VEND_ID_OFFSET],
			EDID_VEND_PROD_SIZE);
}
	...
	const u8 *in_buf)
{
	...
	pr_info("deep color: Y444|RGB30|RGB36|RGB48|Y420_30|Y420_36|Y420_48: (%d|%d|%d|%d|%d|%d|%d)\n",
	...
	u32 video_format, u32 video_3d_format)
{
	...
	pr_info("%s: EDID[3D]: format: %d [%s], %s %s\n", __func__,
		video_format, msm_hdmi_mode_2string(video_format),
	...
	u32 video_format)
{
	...
	if (video_format >= HDMI_VFRMT_MAX) {
		pr_info("%s: video format: %s is not supported\n", __func__,
			msm_hdmi_mode_2string(video_format));
		return;
	}
...
static void hdmi_edid_get_display_mode(struct hdmi_edid_ctrl *edid_ctrl)
{
	...
	edid_blk1 = &data_buf[EDID_BLOCK_SIZE];

	/*
	 * edid_blk1 is really the block with video data block,
	 * it can be any block not just block1.
	 */
	for (i = 1; i <= num_of_cea_blocks; i++) {

		ptr = &data_buf[i*EDID_BLOCK_SIZE];

		/* check for valid CEA block */
		if (ptr[0] != 2)
			continue;

		if (hdmi_edid_extract_ieee_reg_id(edid_ctrl, ptr)
				== EDID_IEEE_REG_ID) {
			edid_blk1 = ptr;
			break;
		}
	...
	svd = num_of_cea_blocks ?
		hdmi_edid_find_block(edid_blk1, DBC_START_OFFSET,
	...
	hdmi_edid_get_extended_video_formats(edid_ctrl, edid_blk1);
...
int hdmi_edid_parser(void *input)
{
	...
	hdmi_edid_extract_vendor_product_block(edid_ctrl);
...
int hdmi_edid_get_vendor_product_blk(
			void *input, struct msm_ext_disp_vend_prod_edid_blk* blk)
{
	struct hdmi_edid_ctrl *edid_ctrl = (struct hdmi_edid_ctrl *)input;
	...
	blk->vend_prod_blk = edid_ctrl->vendor_product_block;
	blk->blk_size = EDID_VEND_PROD_SIZE;
