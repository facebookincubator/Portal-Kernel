...
static int hdmi_cec_msg_read(struct hdmi_cec_ctrl *cec_ctrl)
{
	...
	if (msg->frame_size < 1) {
		...
		return -EINVAL;
	} else if (msg->frame_size > MAX_CEC_FRAME_SIZE) {
		u32 buf[msg->frame_size-1];

		for (i = 0; i < msg->frame_size - 1; i++)
			buf[i] = DSS_REG_R_ND(io, HDMI_CEC_RD_DATA);

		DEV_ERR("%s: invalid message (frame length = %d) data = %x\n",
			__func__, msg->frame_size, data);
		print_hex_dump(KERN_ERR, "CEC LENGTH ERROR:", DUMP_PREFIX_NONE,
			32, 4, buf, (msg->frame_size-1)*4, false);
		return -EINVAL;
	}
...
static void hdmi_cec_msg_recv(struct work_struct *work)
{
	...
	while (CIRC_CNT(head, tail, CEC_RECV_Q_SIZE) >= 1) {
		...
		/*
		 * T48746421: removed injection of KEY_POWER when HDMI CEC
		 * standby message is received. Going to standby is already
		 * handled in HdmiControlService.java
		 */
