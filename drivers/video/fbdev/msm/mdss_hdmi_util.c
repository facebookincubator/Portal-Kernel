...
int msm_hdmi_get_timing_info(
	struct msm_hdmi_mode_timing_info *out_mode, int id)
{
	int ret = 0;
	struct msm_hdmi_mode_timing_info local_mode = {0};
	struct msm_hdmi_mode_timing_info *mode = &local_mode;
	int max_width = CONFIG_FB_EXT_DISPLAY_MAX_RESOLUTION_WIDTH;
	int max_height = CONFIG_FB_EXT_DISPLAY_MAX_RESOLUTION_HEIGHT;
	...
	if (!ret && max_width > 0 && max_height > 0 &&
		(mode->active_v > max_height || mode->active_h > max_width)) {
		ret = -EINVAL;
		pr_debug("(%d, %d) not supported due to limitation max (%d, %d)\n",
			mode->active_h, mode->active_v,
