...
static int fts_power_source_init(struct fts_ts_data *data)
{
	...
	if (regulator_count_voltages(data->vcc_i2c) > 0) {
		ret = regulator_set_voltage(data->vcc_i2c,
					...
					FTS_I2C_VTG_MAX_UV);
		if (ret) {
			FTS_ERROR("vcc_i2c regulator set_vtg failed ret=%d",
				ret);
...
static void fts_show_touch_buffer(u8 *buf, int point_num)
{
	...
	if (len > (fts_data->pnt_buf_size - 3))
		...
		if (len == 0)
			len += FTS_ONE_TCH_LEN;
	count += snprintf(g_sz_debug, PAGE_SIZE, "%02X,%02X,%02X",
				buf[0], buf[1], buf[2]);
	for (i = 0; i < len; i++)
		count += snprintf(g_sz_debug + count,
				PAGE_SIZE, ",%02X", buf[i + 3]);
...
static int fts_irq_registration(struct fts_ts_data *ts_data)
{
	...
	ret = request_threaded_irq(ts_data->irq, NULL,
			fts_ts_interrupt, pdata->irq_gpio_flags | IRQF_ONESHOT,
...
static int fts_input_init(struct fts_ts_data *ts_data)
{
	...
	if (pdata->have_key) {
		...
		for (key_num = 0; key_num < pdata->key_number; key_num++)
			...
			EV_KEY, pdata->keys[key_num]);
	}

#if FTS_MT_PROTOCOL_B_EN
	input_mt_init_slots(input_dev,
	pdata->max_touch_number, INPUT_MT_DIRECT);
#else
	...
	ts_data->events = kzalloc(point_num * sizeof(struct ts_event),
													GFP_KERNEL);
	if (!ts_data->events) {
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	if (pdata->have_key) {
		...
		pdata->keys[2], pdata->key_x_coords[0], pdata->key_x_coords[1],
		pdata->key_x_coords[2], pdata->key_y_coord);
	}
	...
	ret = of_property_read_u32(np, "focaltech,max-touch-number",
				   &temp_val);
	if (ret == 0) {
		if (temp_val < 2)
				 ...
				 unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == MSM_DRM_EVENT_BLANK &&
		...
		blank = evdata->data;
		FTS_DEBUG("calling resume/suspend!");
...
{
	struct fts_ts_data *data = container_of(handler, struct fts_ts_data,
						early_suspend);
			...
			const struct i2c_device_id *id)
{
	...
	if (ret)
		FTS_ERROR("[DRM]Unable to register DRM notifier: %d", ret);
#elif defined(CONFIG_HAS_EARLYSUSPEND)
...
static int fts_ts_suspend(struct device *dev)
{
	...
	if (ts_data->pdata->touch_wakeup_on) {
		...
		fts_i2c_write_reg(ts_data->client, FTS_REG_POWER_MODE,
