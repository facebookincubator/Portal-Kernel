...
static int fts_pram_ecc_cal_algo(struct i2c_client *client,
				...
				int retries, int retries_delay)
{
	...
	for (i = 0; i < retries; i++) {
		...
		ret = fts_i2c_read(client, &cmd, 1, val,
...
int fts_fwupg_erase(struct i2c_client *client, u32 delay)
{
	...
	flag = fts_fwupg_check_flash_status(
			client, FTS_CMD_FLASH_STATUS_ERASE_OK,
			FTS_RETRIES_REASE,
...
int fts_fwupg_ecc_cal(struct i2c_client *client, u32 saddr, u32 len)
{
	...
	for (i = 0; i < packet_num; i++) {
		...
		FTS_DEBUG("ecc calc startaddr:0x%04x, len:%d",
				addr, packet_len);
		...
		ret = fts_fwupg_check_flash_status(
				client, FTS_CMD_FLASH_STATUS_ECC_OK,
				FTS_RETRIES_ECC_CAL,
...
int fts_flash_write_buf(struct i2c_client *client,
		u32 saddr, u8 *buf, u32 len, u32 delay)
{
	int ret = 0;
	u32 i = 0;
	u32 j = 0;
...
int fts_flash_read_buf(struct i2c_client *client, u32 saddr, u8 *buf, u32 len)
{
	...
	if (remainder > 0)
		packet_number++;
	packet_len = FTS_FLASH_PACKET_LENGTH;
	FTS_INFO("read packet_number:%d, remainder:%d",
...
static int fts__get_fw_file_via_request_fw(struct fts_upgrade *upg,
		struct i2c_client *client, const char *fw_name, u8 **fwbuf)
{
	int ret = 0;
	...
	if (ret == 0) {
		FTS_INFO("firmware(%s) request successfully", fw_name);
		*fwbuf = vmalloc(fw->size);
		if (*fwbuf == NULL) {
			...
			upg->lic = *fwbuf;
			upg->lic_length = fw->size;
		}
	} else {
		FTS_INFO("firmware(%s) request fail,ret=%d", fw_name, ret);
	}
...
int fts_upgrade_bin(struct i2c_client *client, char *fw_name, bool force)
{
	...
	ret = fts__get_fw_file_via_request_fw(upg, client, fw_name, &fw_file_buf);
	if (ret < 0) {
		return ret;
	}

	fw_file_len = upg->fw_length;
	FTS_INFO("fw bin file len:%d", fw_file_len);
	if (force) {
		if (upg->func->force_upgrade) {
			ret = upg->func->force_upgrade(client,
					fw_file_buf, fw_file_len);
		} else {
		...
		if (upg->func->lic_upgrade)
			ret = upg->func->lic_upgrade(client,
					fw_file_buf, fw_file_len);
		else
			FTS_INFO("lic_upgrade function is null, no upgrade");
#endif
		if (upg->func->upgrade)
			ret = upg->func->upgrade(client, fw_file_buf,
						fw_file_len);
		else
...
int fts_lic_upgrade(struct i2c_client *client, struct fts_upgrade *upg)
{
	...
	if (hlic_upgrade) {
		...
		do {
			...
			ret = upg->func->lic_upgrade
				(client, upg->lic, upg->lic_length);
			if (ret < 0) {
				fts_fwupg_reset_in_boot(client);
			} else {
...
static bool fts_fwupg_need_upgrade(struct i2c_client *client)
{
	...
	if (fwvalid) {
		...
		FTS_INFO("fw version in tp:%x, host:%x",
		...
		if (upgrade_flag) {
			...
			if (upg->func->param_upgrade) {
				if (fts_param_need_upgrade(client)) {
					...
					ret = upg->func->param_upgrade(client,
						upg->fw, upg->fw_length);
					if (ret < 0) {
						fts_fwupg_reset_in_boot(
								client);
					} else {
						fts_param_get_ver_in_tp
							(client, &ver);
...
int fts_fwupg_get_vendorid(struct fts_ts_data *ts_data, u16 *vid)
{
	...
	if (fwvalid) {
		...
		if (ts_data->ic_info.is_incell)
			ret = fts_i2c_read_reg(client,
					FTS_REG_MODULE_ID, &module_id);
	} else {
		fwcfg_addr =  upg->func->fwcfgoff;
		ret = fts_flash_read(client, fwcfg_addr,
...
int fts_fwupg_init(struct fts_ts_data *ts_data)
{
	...
	int func_count = sizeof(upgrade_func_list)
		       / sizeof(upgrade_func_list[0]);
