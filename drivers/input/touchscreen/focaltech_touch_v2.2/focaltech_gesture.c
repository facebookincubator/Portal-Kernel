...
static DEVICE_ATTR(fts_gesture_mode, 0640,
			fts_gesture_show, fts_gesture_store);
static DEVICE_ATTR(fts_gesture_buf, 0440,
				   ...
				   u8 *buf, int read_bytes)
{
	...
	if (read_bytes <= I2C_BUFFER_LENGTH_MAXINUM) {
		...
		ret = fts_i2c_read(client, buf, 1, buf,
