...
{
	int i, j, ret;
	u8 *data = kmalloc_array(size, sizeof(u8), GFP_KERNEL);
...
/**
  ...
  */
int getMSFrame3(enum MSFrameType type, struct MutualSenseFrame *frame)
{
	u16 offset;
...
LOAD_FRC:
	...
	frame->node_data = kmalloc(frame->node_data_size *
...
/**
  ...
  */
int getSSFrame3(enum SSFrameType type, struct SelfSenseFrame *frame)
{
	...
	frame->force_data = kmalloc(frame->header.force_node *
					     sizeof(short), GFP_KERNEL);
	...
	frame->sense_data = kmalloc(frame->header.sense_node *
					     sizeof(short), GFP_KERNEL);
...
/**
  ...
  * @return OK if success or an error code which specify the type of error
  */
int readSyncDataHeader(u8 type, struct DataHeader *msHeader,
		       struct DataHeader *ssHeader, u64 *address)
{
...
/**
  ...
  */
int getSyncFrame(u8 type, struct MutualSenseFrame *msFrame,
		 struct SelfSenseFrame *ssFrame)
{
	int res;
	u64 address;
	...
	msFrame->node_data = kmalloc(msFrame->node_data_size *
					      sizeof(short), GFP_KERNEL);
	...
	ssFrame->force_data = kmalloc(ssFrame->header.force_node *
					       sizeof(short), GFP_KERNEL);
	...
	ssFrame->sense_data = kmalloc(ssFrame->header.sense_node *
					       sizeof(short), GFP_KERNEL);
