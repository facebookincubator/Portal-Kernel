...
int fts_read(u8 *outBuf, int byteToRead)
{
	...
	while (retry < I2C_RETRY && ret < OK) {
		...
		if (ret < OK) {
			logError(1,
				"%s %s failed: attempt %d! Retry after: %d ms.\n",
				TAG, __func__, retry, I2C_WAIT_BEFORE_RETRY);
...
int fts_write(u8 *cmd, int cmdLength)
{
	...
	while (retry < I2C_RETRY && ret < OK) {
		...
		retry++;
		if (ret < OK) {
			logError(1, "%s %s failed: attempt %d!\n",
				TAG, __func__, retry);
			logError(1, "%s %s Retry after: %d ms.\n",
				TAG, __func__, I2C_WAIT_BEFORE_RETRY);
			msleep(I2C_WAIT_BEFORE_RETRY);
		}
	}
	if (ret < 0) {
...
int fts_writeU8UX(u8 cmd, enum AddrSize addrSize, u64 address, u8 *data, int
		  dataSize)
{
	u32 ret;
	u8 *finalCmd = NULL;
	...
	ret = OK;
	finalCmd = kzalloc(1 + addrSize + WRITE_CHUNK, GFP_KERNEL);
	if (!finalCmd) {
		logError(1,
			 "%s %s: Unable to allocate finalCmd! ERROR %08X\n",
	...
	if (addrSize <= sizeof(u64)) {
		while (remaining > 0) {
			...
			ret = fts_write(finalCmd, 1 + addrSize + toWrite);
			if (ret < OK) {
				logError(1, "%s %s: ERROR %08X\n", TAG,
...
/**
  ...
  * @return OK if success or an error code which specify the type of error
  */
int fts_writeReadU8UX(u8 cmd, enum AddrSize addrSize, u64 address, u8 *outBuf,
		      int byteToRead, int hasDummyByte)
{
	...
	ret = OK;
	finalCmd = kzalloc(1 + addrSize, GFP_KERNEL);
	if (!finalCmd) {
		logError(1,
			 "%s %s: Unable to allocate finalCmd! ERROR %08X\n",
			 __func__, TAG, ERROR_ALLOC);
	...
	buff = kzalloc(READ_CHUNK + 1, GFP_KERNEL);
	if (!buff) {
		logError(1,
			 "%s %s: Unable to allocate buff! ERROR %08X\n",
			 __func__, TAG, ERROR_ALLOC);
		ret = ERROR_ALLOC;
		goto exit;
	}
	...
	while (remaining > 0) {
		...
		if (hasDummyByte == 1) {
			ret = fts_writeRead(finalCmd,
				1 + addrSize, buff, toRead + 1);
			if (ret < OK) {
				logError(1,
 ...
 * max 8 bytes
 */
int fts_writeU8UXthenWriteU8UX(u8 cmd1, enum AddrSize addrSize1, u8 cmd2,
		enum AddrSize addrSize2, u64 address, u8 *data, int dataSize)
{
	u32 ret;
	u8 *finalCmd1 = NULL;
	u8 *finalCmd2 = NULL;
	...
	ret = OK;
	finalCmd1 = kzalloc(1 + addrSize1, GFP_KERNEL);
	if (!finalCmd1) {
		logError(1,
			 "%s %s: Unable to allocate finalCmd1! ERROR %08X\n",
			 __func__, TAG, ERROR_ALLOC);
		ret = ERROR_ALLOC;
		goto exit;
	}

	finalCmd2 = kzalloc(1 + addrSize2 + WRITE_CHUNK, GFP_KERNEL);
	if (!finalCmd2) {
		logError(1,
			 "%s %s: Unable to allocate finalCmd2! ERROR %08X\n",
			 __func__, TAG, ERROR_ALLOC);
		ret = ERROR_ALLOC;
		goto exit;
	}
	...
	while (remaining > 0) {
		...
		ret = fts_write(finalCmd1, 1 + addrSize1);
		if (ret < OK) {
		...
		ret = fts_write(finalCmd2, 1 + addrSize2 + toWrite);
		if (ret < OK) {
			logError(1,
 ...
 * max 8 bytes
 */
int fts_writeU8UXthenWriteReadU8UX(u8 cmd1, enum AddrSize addrSize1, u8 cmd2,
				   enum AddrSize addrSize2, u64 address,
				   u8 *outBuf, int byteToRead, int hasDummyByte)
{
	u32 ret;
	u8 *finalCmd1 = NULL;
	u8 *finalCmd2 = NULL;
	...
	finalCmd2 = kzalloc(1 + addrSize2, GFP_KERNEL);
	if (!finalCmd2) {
		logError(1,
			 "%s %s: Unable to allocate finalCmd2! ERROR %08X\n",
			 __func__, TAG, ERROR_ALLOC);
	...
	while (remaining > 0) {
		...
		if (hasDummyByte == 1) {
			ret = fts_writeRead(finalCmd2, 1 +
				addrSize2, buff, toRead + 1);
			if (ret < OK) {
				logError(1,
					 "%s %s: read error... ERROR %08X\n",
					 TAG,
			...
			ret = fts_writeRead(finalCmd2,
				1 + addrSize2, buff, toRead);
			if (ret < OK) {
				logError(1,
					 "%s %s: read error... ERROR %08X\n",
					 TAG,
