...
/**
  ...
  */
unsigned int elapsedMillisecond(struct StopWatch *w)
{
	unsigned int result;
	const struct timespec result_timespec = timespec_sub(w->end, w->start);

	result = timespec_to_ns(&result_timespec) / 1000000;
...
/**
  ...
  * if the startStopWatch and stopStopWatch were not called before)
  */
unsigned long elapsedNanosecond(struct StopWatch *w)
{
	unsigned long result;
	const struct timespec result_timespec = timespec_sub(w->end, w->start);

	result = timespec_to_ns(&result_timespec);
