...
int u8ToU16n(u8 *src, int src_length, u16 *dst)
{
	...
	j = 0;
	dst = kmalloc((src_length / 2) * sizeof(u16),
			     GFP_KERNEL);
...
{
	int i;
	short **matrix = kmalloc(((int)(size / columns)) *
...
{
	int i;
	u16 **matrix = kmalloc(((int)(size / columns)) * sizeof(u16 *),
				       GFP_KERNEL);

	if (matrix != NULL) {
		for (i = 0; i < (int)(size / columns); i++)
			matrix[i] = kmalloc_array(columns, sizeof(u16),
						   GFP_KERNEL);
...
{
	int i;
	u8 **matrix = kmalloc(((int)(size / columns)) * sizeof(u8 *),
				     GFP_KERNEL);

	if (matrix != NULL) {
		for (i = 0; i < (int)(size / columns); i++)
			matrix[i] = kmalloc_array(columns, sizeof(u8),
						  GFP_KERNEL);
...
{
	int i;
	i8 **matrix = kmalloc(((int)(size / columns)) * sizeof(i8 *),
				     GFP_KERNEL);

	if (matrix != NULL) {
		for (i = 0; i < (int)(size / columns); i++)
			matrix[i] = kmalloc_array(columns, sizeof(i8),
						  GFP_KERNEL);
