...
struct Message {
	...
	enum Actions action;/* /< type of operation requested by the host @see
			 * Actions
			 */
...
static int limit;       /* /< store the amount of data to
			 * print into the shell
			 */
...
char buf_chunk[CHUNK_PROC] = { 0 };/* /< buffer used to store the message
				    * info received
				    */
static struct Message mess = { 0 };/* /< store the information of the
				    *  Scriptless
				    * message received
				    */
...
static void *fts_seq_next(struct seq_file *s, void *v, loff_t *pos)
{
	...
	/* logError(0, "%s %s:
	 * In next(), updated pos = %ld limit = %d printed = %d\n",
...
static void fts_seq_stop(struct seq_file *s, void *v)
{
	...
	if (v) {
		...
		if (driver_test_buff != NULL) {
			/* logError(0, "%s %s:
			 *  Freeing and clearing driver_test_buff.\n",
			 *	TAG, __func__);
			 */
			...
			/* logError(0, "%s %s:
			 *  driver_test_buff is already null.\n",
			 *	TAG, __func__);
			 */
		}
	}
				     ...
				     size_t count, loff_t *pos)
{
	...
	struct TotSelfSenseData totComData;

	u64 address;

	struct Firmware fw;
	struct LimitFile lim;
	...
	if (numberParam >= 1) {
		...
		case CMD_READ_BYTE:
			if (numberParam >= 3) {	/* need to pass: byteToRead1
						 ...
						 */
				if (numberParam == 3 ||
				    (numberParam == 4 && cmd[numberParam - 1] ==
				     0))
					mess.dummy = 0;
				else
					mess.dummy = 1;
		...
		case CMD_WRITEU8UX_BYTE:
			/* need to pass: cmd addrSize addr[0] …
			 * addr[addrSize-1]
			 * data[0] data[1] …
			 */
			if (numberParam >= 4) {
		...
		case CMD_WRITEREADU8UX_BYTE:
			/* need to pass: cmd addrSize
			 * addr[0] … addr[addrSize-1]
		...
		case CMD_WRITEU8UXTHENWRITEREADU8UX_BYTE:
			/* need to pass: cmd1 addrSize1 cmd2 addrSize2
			 * addr[0]…addr[addrSize1+addrSize2-1]
			 * byteToRead1 byteToRead0 hasDummybyte
			 */
			if (numberParam >= 8) {
				if ((cmd[2] + cmd[4]) <= sizeof(u64)) {
		...
		case CMD_GETSSFRAME:
			if (numberParam == 2) {
				 ...
				 * msleep(WAIT_AFTER_SENSEOFF);
				 */
				res = getSSFrame3((enum SSFrameType)cmd[1],
						  &frameSS);
		...
		case CMD_DIAGNOSTIC:
			...
			readData = kmalloc((ERROR_DUMP_ROW_SIZE *
						  ERROR_DUMP_COL_SIZE) *
