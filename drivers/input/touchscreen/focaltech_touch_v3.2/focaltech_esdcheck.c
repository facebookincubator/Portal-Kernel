...
static int esdcheck_algorithm(struct fts_ts_data *ts_data)
{
	...
	if (fts_esdcheck_data.suspend == 1) {
		 ...
		 * will active ESD check(active = 1); But in suspend, then
		 * will don't queue_delayed_work, when resume, don't check
		 * ESD again
		 */
		return 0;
	}
 ...
 *
 */
static DEVICE_ATTR(fts_esd_mode, 0640, fts_esdcheck_show, fts_esdcheck_store);
