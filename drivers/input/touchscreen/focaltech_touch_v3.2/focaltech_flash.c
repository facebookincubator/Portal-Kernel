				    ...
				    enum FW_STATUS *fw_sts)
{
	...
	usleep_range(FTS_CMD_START_DELAY*1000, FTS_CMD_START_DELAY*1000);
...
static int fts_fwupg_reset_to_boot(struct fts_upgrade *upg)
{
	...
	usleep_range(FTS_DELAY_UPGRADE_AA * 1000, FTS_DELAY_UPGRADE_AA * 1000);
	...
	for (i = 0; i < packet_number; i++) {
		...
		for (j = 0; j < FTS_RETRIES_WRITE; j++) {
			...
			if (wr_ok == read_status)
				break;
			usleep_range(FTS_RETRIES_DELAY_WRITE * 1000,
					FTS_RETRIES_DELAY_WRITE * 1000);
		}
	}
...
static int fts_flash_read_buf(u32 saddr, u8 *buf, u32 len)
{
	...
	for (i = 0; i < packet_number; i++) {
		...
		if (upg->ts_data->bus_type == BUS_TYPE_I2C) {
			...
			msleep(FTS_CMD_READ_DELAY);
			usleep_range(FTS_CMD_READ_DELAY * 1000,
					FTS_CMD_READ_DELAY * 1000);
			wbuf[0] = FTS_CMD_READ;
...
int fts_upgrade_bin(char *fw_name, bool force)
{
	...
	if ((ret < 0) || (upg->fw_length < FTS_MIN_LEN)) {
		FTS_ERROR("read fw bin file(%s) fail, ret:%d len=%d",
				fw_name, ret, upg->fw_length);
		goto err_bin;
	}

	FTS_INFO("fw bin file len:%d", upg->fw_length);
	if (force) {
		if (upg->func->force_upgrade) {
			ret = upg->func->force_upgrade(upg->fw,
						       upg->fw_length);
		} else {
		...
		if (upg->func->lic_upgrade)
			ret = upg->func->lic_upgrade(upg->fw, upg->fw_length);
		else
		...
		if (upgrade_flag) {
			...
			if (upg->func->upgrade) {
				...
				if (ret < 0) {
					...
					FTS_INFO(
					  "upgraded to fw version %02x", ver);
					break;
				}
			} else {
				FTS_ERROR(
				  "upgrade func/upgrade is null, returning");
				ret = -ENODATA;
				break;
			}
		}
...
static int fts_fwupg_get_module_info(struct fts_upgrade *upg)
{
	...
	if (FTS_GET_MODULE_NUM > 1) {
		/* support multi modules, must read correct module
		 * id(vendor id)
		 */
						...
						char *name)
{
	...
	if (name)
		snprintf(fwname, FILE_NAME_LENGTH, "%s", name);
	else
...
static int fts_fwupg_get_fw_file(struct fts_upgrade *upg)
{
	...
	if (FTS_FW_REQUEST_SUPPORT) {
		ret = fts_get_fw_file_via_request_firmware(upg, NULL);
		if (ret != 0)
			get_fw_i_flag = true;
	} else
		get_fw_i_flag = true;
