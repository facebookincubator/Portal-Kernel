				   ...
				   const char *buf, size_t count)
{
	...
	FTS_DEBUG("log level:%d->%d ret=%d", fts_data->log_level, value, ret);
...
static DEVICE_ATTR(fts_fw_version, 0440, fts_tpfwver_show, NULL);
 ...
 *       read:  echo 10005           >
 *			rw_reg ---read reg 0x00-0x05
 *       write: echo 000050102030405 >
 *			rw_reg ---write reg 0x00-0x05 as 01,02,03,04,05
 *  Get result:
 *       cat rw_reg
 */
static DEVICE_ATTR(fts_rw_reg, 0640, fts_tprwreg_show, fts_tprwreg_store);
...
static DEVICE_ATTR(fts_driver_info, 0440, fts_driverinfo_show,
		   NULL);
static DEVICE_ATTR(fts_dump_reg, 0440, fts_dumpreg_show, NULL);
static DEVICE_ATTR(fts_hw_reset, 0440, fts_hw_reset_show, NULL);
static DEVICE_ATTR(fts_irq, 0640, fts_irq_show, fts_irq_store);
static DEVICE_ATTR(fts_boot_mode, 0640, fts_bootmode_show, fts_bootmode_store);
static DEVICE_ATTR(fts_touch_point, 0440, fts_tpbuf_show, NULL);
static DEVICE_ATTR(fts_log_level, 0640, fts_log_level_show,
		   fts_log_level_store);
