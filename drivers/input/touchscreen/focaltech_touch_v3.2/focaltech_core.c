...
#endif
#define FTS_POINT_ADJUST_BY_HOST 0
/*****************************************************************************
...
static int fts_input_report_b(struct fts_ts_data *data)
{
	...
	for (i = 0; i < data->touch_point; i++) {
		...
		if (EVENT_DOWN(events[i].flag)) {
			if (data->suspended)
				FTS_INFO("In suspend. Do not report touch.\n");
			else {
				...
				input_report_abs(data->input_dev,
					ABS_MT_POSITION_X,
					pdata->x_max - events[i].x);
				input_report_abs(data->input_dev,
					ABS_MT_POSITION_Y,
					pdata->y_max - events[i].y);
#else
				input_report_abs(data->input_dev,
					ABS_MT_POSITION_X,
					events[i].x);
			...
			if (data->suspended && !data->gesture_wakeup_mode)
...
static int fts_input_init(struct fts_ts_data *ts_data)
{
	...
	/* Enable KEY_WAKEUP event if wakeup support is required */
...
static int fts_power_source_ctrl(struct fts_ts_data *ts_data, int enable)
{
	...
	if (enable) {
		if (ts_data->power_disabled) {
			...
			usleep_range(1000, 1100);
			ret = regulator_set_load(ts_data->vdd, 10000000);
			if (ret < 0) {
				FTS_ERROR("regulator_set_load vdd fail\n");
				return -EINVAL;
			}
