...
static int ft5822_rawdata_test(struct fts_test *tdata, bool *test_result)
{
	...
	if (tdata->normalize != normalize) {
		ret = fts_test_write_reg(FACTORY_REG_NORMALIZE,
					 tdata->normalize);
		if (ret < 0) {
	...
	result = true;
	result2 = true;
	if (tdata->normalize == NORMALIZE_AUTO) {
		...
		/* compare */
		result = compare_array(rawdata_tmp, thr->rawdata_h_min,
				       thr->rawdata_h_max, false);

		rawdata_cnt += tdata->node.node_num;
	} else if (tdata->normalize == NORMALIZE_OVERALL) {
		if (thr->basic.rawdata_set_lfreq) {
			FTS_TEST_SAVE_INFO(
				"NORMALIZE_OVERALL + SET_LOW_FREQ:\n");
			rawdata_tmp = rawdata + rawdata_cnt;
			...
			/* compare */
			result = compare_array(rawdata_tmp, thr->rawdata_l_min,
					       thr->rawdata_l_max, false);
		...
		if (thr->basic.rawdata_set_hfreq) {
			FTS_TEST_SAVE_INFO(
				"NORMALIZE_OVERALL + SET_HIGH_FREQ:\n");
			rawdata_tmp = rawdata + rawdata_cnt;
			...
			/* compare */
			result2 = compare_array(rawdata_tmp, thr->rawdata_h_min,
						thr->rawdata_h_max, false);
...
test_err:
	...
	/* save data */
	fts_test_save_data("Rawdata Test", CODE_M_RAWDATA_TEST, rawdata,
			   rawdata_cnt, false, false, *test_result);
...
static int ft5822_scap_cb_test(struct fts_test *tdata, bool *test_result)
{
	...
	if (thr->basic.scap_cb_wp_on_check && fw_wp_check) {
		...
		tmp_result =
			compare_mc_sc(tx_check, rx_check, scb_tmp,
				      thr->scap_cb_on_min, thr->scap_cb_on_max);
...
test_err:
	...
	/* save data */
	fts_test_save_data("SCAP CB Test", CODE_M_SCAP_CB_TEST, scap_cb,
			   scb_cnt, true, false, *test_result);
...
test_err:
	...
	fts_test_save_data("SCAP Rawdata Test", CODE_M_SCAP_RAWDATA_TEST,
			   scap_rawdata, srawdata_cnt, true, false,
			   *test_result);
