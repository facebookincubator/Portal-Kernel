...
struct fts_test_results *fts_results;
...
int fts_test_read(u8 addr, u8 *readbuf, int readlen)
{
	...
	/* FTS_TEST_DBG("packet num:%d, remainder:%d",
	 *		packet_num, packet_remainder);
	 */
...
int get_cb_sc(int byte_num, int *cb_buf, enum byte_mode mode)
{
	...
	for (i = 0; i < packet_num; i++) {
		...
		if (tdata->func->hwtype == IC_HW_MC_SC) {
			if (offset >= 256) {
				ret = fts_test_write_reg(off_h_addr,
							 offset >> 8);
				if (ret < 0) {
					FTS_TEST_SAVE_ERR(
...
#if TXT_SUPPORT
	if (true == allocate) {
		if (fts_results && fts_results->testresult) {
			vfree(fts_results->testresult);
			fts_results->testresult = NULL;
			fts_results->testresult_len = 0;
		}
...
static void fts_test_save_data_csv(struct fts_test *tdata)
{
	...
	if (!fts_results->csv_buffer) {
		fts_results->csv_length = 0;
		vfree(fts_results->csv_buffer);
		fts_results->csv_buffer = NULL;
	}
		...
		for (j = 1; j <= data_count; j++) {
			...
			if (tdata->func->hwtype == IC_HW_MC_SC) {
				...
				if ((info->code == CODE_M_SCAP_CB_TEST) ||
					...
					index =
					  fts_test_get_item_count_scap_csv(j);
				}
			}
	...
	fts_results->csv_buffer = csv_buffer;
	fts_results->csv_length = csv_length;
...
static void fts_test_save_result_txt(struct fts_test *tdata)
{
	...
	fts_results->testresult = tdata->testresult;
	fts_results->testresult_len = tdata->testresult_len;
...
static int fts_test_malloc_free_mc_sc(struct fts_test *tdata, bool allocate)
{
	...
	if (true == allocate) {
		...
		if (tdata->func->rawdata2_support) {
			...
			if (!thr->rawdata_l_min)
				return -ENOMEM;
			fts_malloc_r(thr->rawdata_l_max, buflen);
			if (!thr->rawdata_l_max)
				return -ENOMEM;
		}
...
static void *selftest_csv_seq_start(struct seq_file *s, loff_t *pos)
{
	if (!fts_results->csv_buffer)
		return NULL;

	if (*pos >= fts_results->csv_length)
		return NULL;

	return (void *)((unsigned long) *pos + 1);
}

static void *selftest_csv_seq_next(struct seq_file *s, void *v, loff_t *pos)
{
	++*pos;
	return selftest_csv_seq_start(s, pos);
}

static void selftest_seq_stop(struct seq_file *s, void *v)
{
}

static int selftest_csv_seq_show(struct seq_file *s, void *v)
{
	unsigned long n = (unsigned long)v - 1;

	if (fts_results->csv_buffer)
		seq_write(s, fts_results->csv_buffer + n, 1);

	return 0;
}

static const struct seq_operations csv_seq_ops = {
	.start = selftest_csv_seq_start,
	.next = selftest_csv_seq_next,
	.stop = selftest_seq_stop,
	.show = selftest_csv_seq_show
};

static int selftest_csv_open(struct inode *inode, struct file *file)
{
	return seq_open(file, &csv_seq_ops);
}

static const struct file_operations selftest_csv_fops = {
	.open = selftest_csv_open,
	.release = seq_release,
	.read = seq_read,
	.llseek = seq_lseek,
};

static void *selftest_txt_seq_start(struct seq_file *s, loff_t *pos)
{
	if (!fts_results->testresult)
		return NULL;

	if (*pos >= fts_results->testresult_len)
		return NULL;

	return (void *)((unsigned long) *pos + 1);
}

static void *selftest_txt_seq_next(struct seq_file *s, void *v, loff_t *pos)
{
	++*pos;
	return selftest_txt_seq_start(s, pos);
}

static int selftest_txt_seq_show(struct seq_file *s, void *v)
{
	unsigned long n = (unsigned long)v - 1;

	if (fts_results->csv_buffer)
		seq_write(s, fts_results->testresult + n, 1);

	return 0;
}

static const struct seq_operations txt_seq_ops = {
	.start = selftest_txt_seq_start,
	.next = selftest_txt_seq_next,
	.stop = selftest_seq_stop,
	.show = selftest_txt_seq_show
};

static int selftest_txt_open(struct inode *inode, struct file *file)
{
	return seq_open(file, &txt_seq_ops);
}

static const struct file_operations selftest_txt_fops = {
	.open = selftest_txt_open,
	.release = seq_release,
	.read = seq_read,
	.llseek = seq_lseek,
};

static int fts_test_results_init(struct fts_ts_data *ts_data)
{

	fts_results = kzalloc(sizeof(*fts_results), GFP_KERNEL);
	if (!fts_results) {
		FTS_TEST_ERROR("malloc memory for test fail");
		return -ENOMEM;
	}

	fts_results->debugfs = debugfs_create_dir(dev_name(ts_data->dev), NULL);
	debugfs_create_file("selftest_csv", 0600, fts_results->debugfs, ts_data,
				&selftest_csv_fops);
	debugfs_create_file("selftest_txt", 0600, fts_results->debugfs, ts_data,
				&selftest_txt_fops);
	return 0;
}

static void fts_test_results_deinit(struct fts_ts_data *ts_data)
{
	debugfs_remove(fts_results->debugfs);
...
int fts_test_exit(struct fts_ts_data *ts_data)
{
	...
	fts_test_results_deinit(ts_data);
