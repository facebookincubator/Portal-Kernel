...
#define FTS_FW_NAME_PREX_WITH_REQUEST		"LQ_T800_FT8201_BOE_app"
...
struct upgrade_module module_list[] = {
	{FTS_MODULE_ID, FTS_MODULE_NAME, fts8201_fw_file, sizeof(fts8201_fw_file)},
	{FTS_MODULE2_ID, FTS_MODULE2_NAME, fts8201_fw_file2, sizeof(fts8201_fw_file2)},
	{FTS_MODULE3_ID, FTS_MODULE3_NAME, fts8201_fw_file3, sizeof(fts8201_fw_file3)},
};

struct upgrade_func *fts8201_upgrade_func_list[] = {
	&upgrade_func_ft8006m,
};

struct fts_upgrade *fts8201_fwupgrade;
	...
	enum FW_STATUS *fw_sts)
{
	...
	if (fts8201_data->ic_info.is_incell)
...
/************************************************************************
 * Name: fts8201_fwupg_reset_in_boot
 * Brief: RST CMD(07), reset to romboot(bootloader) in boot environment
 * Input:
 * Output:
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int fts8201_fwupg_reset_in_boot(void)
{
	int ret = 0;
	u8 cmd = FTS_CMD_RESET;
...
/************************************************************************
 * Name: fts8201_fwupg_enter_into_boot
 * Brief: enter into boot environment, ready for upgrade
 * Input:
 * Output:
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int fts8201_fwupg_enter_into_boot(void)
{
	int ret = 0;
	bool fwvalid = false;
	bool state = false;
	struct fts_upgrade *upg = fts8201_fwupgrade;
	...
	if (fwvalid) {
		...
		ret = fts8201_fwupg_reset_in_boot();
		if (ret < 0) {
...
/************************************************************************
 * Name: fts8201_fwupg_erase
 * Brief: erase flash area
 * Input: delay - delay after erase
 * Output:
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int fts8201_fwupg_erase(u32 delay)
{
	int ret = 0;
	u8 cmd = 0;
	bool flag = false;
 ...
 ***********************************************************************/
int fts8201_fwupg_ecc_cal(u32 saddr, u32 len)
{
	int ret = 0;
	u32 i = 0;
 ...
 ***********************************************************************/
int fts8201_flash_write_buf(u32 saddr,
			    u8 *buf,
			    u32 len,
			    u32 delay)
{
 ...
 ***********************************************************************/
int fts8201_flash_read_buf(u32 saddr, u8 *buf, u32 len)
{
	int ret = 0;
	u32 i = 0;
	u32 packet_number = 0;
 ...
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int fts_fl8201ash_read(u32 addr, u8 *buf, u32 len)
{
	int ret = 0;
	...
	ret = fts8201_fwupg_enter_into_boot();
	if (ret < 0) {
		FTS_ERROR("enter into pramboot/bootloader fail");
		goto read_flash_err;
	}

	ret = fts8201_flash_read_buf(addr, buf, len);
	if (ret < 0) {
...
int fts8201_read_file(char *file_name, u8 **file_buf)
{
	int ret = 0;
...
int fts8201_upgrade_bin(char *fw_name, bool force)
{
	int ret = 0;
	u32 fw_file_len = 0;
	u8 *fw_file_buf = NULL;
	...
	ret = fts8201_read_file(fw_name, &fw_file_buf);
...
static int fts_param_get_ver_in_host(struct fts_upgrade *upg, u8 *ver)
{
	if ((!upg) || (!upg->func) || (!upg->fw) || (!ver)) {
		FTS_ERROR("fts8201_data/upgrade/func/fw/ver is NULL");
		return -EINVAL;
	}
...
static int fts_param_ide_in_host(struct fts_upgrade *upg)
{
	...
	if ((!upg) || (!upg->func) || (!upg->fw)) {
		FTS_ERROR("fts8201_data/upgrade/func/fw is NULL");
		return -EINVAL;
	}
 ...
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int fts8201_fwupg_upgrade(struct fts_upgrade *upg)
{
	int ret = 0;
		...
		if (upgrade_flag) {
			...
			if (upg->func->param_upgrade) {
				...
				if (ret <= 0) {
					...
					FTS_INFO("force upgrade fw app(times:%d)",
							upgrade_count);
					if (upg->func->upgrade) {
					...
					FTS_INFO("upgrade param area(times:%d)",
							upgrade_count);
					ret = upg->func->param_upgrade(upg->fw,
								       upg->fw_length);
					if (ret < 0) {
						fts8201_fwupg_reset_in_boot();
					} else {
...
static int fts_fwupg_get_vendorid(struct fts_upgrade *upg, int *vid)
{
	...
	if (fwvalid) {
		...
		ret = fts_fl8201ash_read(fwcfg_addr, cfgbuf, FTS_HEADER_LEN);
 ...
 *		If support muitl modules, please set FTS_GET_MODULE_NUM and
 *		FTS_- MODULE_ID/FTS_MODULE_NAME;
 *
 *		If get fw via .i file, please set FTS_FW_REQUEST_SUPPORT=0 and
 *		FTS_MODULE_ID; will use module id to distingwish different modules;
 *
 *		If get fw via reques_firmware(), please set FTS_FW_REQUEST_SUPPORT =1 and
 *		FTS_MODULE_NAME;
 *		fw file name will be composed of "focaltech_ts_fw_" & FTS_VENDOR_NAME;
 *
...
int fts8201_fwupg_init(struct fts_ts_data *ts_data)
{
	int i = 0;
	int j = 0;
	int ic_stype = 0;
	struct upgrade_func *func = fts8201_upgrade_func_list[0];
	int func_count = sizeof(fts8201_upgrade_func_list) /
			 sizeof(fts8201_upgrade_func_list[0]);
	...
	fts8201_fwupgrade = kzalloc(sizeof(*fts8201_fwupgrade), GFP_KERNEL);
	if (fts8201_fwupgrade == NULL) {
	...
	if (func_count == 1) {
		fts8201_fwupgrade->func = func;
	} else {
		for (i = 0; i < func_count; i++) {
			func = fts8201_upgrade_func_list[i];
			for (j = 0; j < FTX_MAX_COMPATIBLE_TYPE; j++) {
				if (func->ctype[j] == 0)
					break;
				else if (func->ctype[j] == ic_stype) {
					FTS_INFO("match upgrade function,type:%x",
							(int)func->ctype[j]);
					fts8201_fwupgrade->func = func;
				}
			}
	...
	if (fts8201_fwupgrade->func == NULL) {
		FTS_ERROR("no upgrade function match, can't upgrade");
		return -ENODATA;
	}

	fts8201_fwupgrade->ts_data = ts_data;
...
int fts8201_fwupg_exit(struct fts_ts_data *ts_data)
{
	FTS_FUNC_ENTER();
	if (fts8201_fwupgrade) {
		if (fts8201_fwupgrade->fw_from_request) {
			vfree(fts8201_fwupgrade->fw);
			fts8201_fwupgrade->fw = NULL;
		}

		kfree(fts8201_fwupgrade);
		fts8201_fwupgrade = NULL;
	}
	FTS_FUNC_EXIT();
	return 0;
}
