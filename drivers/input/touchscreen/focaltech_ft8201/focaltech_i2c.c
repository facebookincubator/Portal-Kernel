...
int fts_read(u8 *cmd, u32 cmdlen, u8 *data, u32 datalen)
{
	...
	if (!fts8201_data || !fts8201_data->client || !data || !datalen) {
		FTS_ERROR("fts8201_data/client/data/datalen(%d) is invalid",
				datalen);
		return -EINVAL;
	}
	client = fts8201_data->client;

	mutex_lock(&fts8201_data->bus_lock);
	...
	mutex_unlock(&fts8201_data->bus_lock);
	return ret;
}
...
int fts_write(u8 *writebuf, u32 writelen)
{
	...
	if (!fts8201_data || !fts8201_data->client || !writebuf || !writelen) {
		FTS_ERROR("fts8201_data/client/data/datalen(%d) is invalid",
				writelen);
		return -EINVAL;
	}
