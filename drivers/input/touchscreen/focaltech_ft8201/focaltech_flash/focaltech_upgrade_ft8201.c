...
static int cal_lcdinitcode_ecc(u8 *buf, u16 *ecc_val)
{
	...
	u8 bank_mapping[] = { 0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9,
			      ...
			      0x13, 0x14, 0x18, 0x19, 0x1A, 0x1C, 0x1D, 0x1E,
			      0x1F, 0x20, 0x22, 0x23, 0x24 };
	u8 banknum = 0;
	...
	while (pos < file_len) {
		...
		if (i >= sizeof(bank_mapping) / sizeof(u8)) {
			FTS_INFO("actual mipi bank(%d) not find in bank mapping, need jump",
					bank_num);
		} else {
...
static int replace_3gamma(u8 *initcode, u8 *gamma, u16 gamma_len)
{
	...
	if ((initcode[gamma_analog[0]] == gamma[gamma_pos])
		...
		memcpy(initcode + gamma_analog[0] + 4,
		       gamma + gamma_pos + 4, gamma_analog[5]);
	...
	if ((initcode[gamma_digital1[0]] == gamma[gamma_pos])
		...
		memcpy(initcode + gamma_digital1[0] + 4,
		       gamma + gamma_pos + 4, gamma_digital1[5]);
	...
	if ((initcode[gamma_digital2[0]] == gamma[gamma_pos])
		...
		memcpy(initcode + gamma_digital2[0] + 4,
		       gamma + gamma_pos + 4, gamma_digital2[5]);
...
static int fts_ft8006m_write_ecc(u32 saddr, u8 *buf, u32 len)
{
	...
	ecc_in_host = fts8201_flash_write_buf(saddr, buf, len, 1);
	if (ecc_in_host < 0) {
	...
	/* ecc */
	ecc_in_tp = fts8201_fwupg_ecc_cal(saddr, len);
	if (ecc_in_tp < 0) {
