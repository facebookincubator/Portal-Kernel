...
struct fts_esdcheck_st {
	...
	/*
	 * Flow Work Cnt(reg0x91) keep a same value for x times.
...
static bool get_flow_cnt(struct fts_ts_data *ts_data)
{
	...
	if (fts_esdcheck_data.flow_work_hold_cnt >= 5) {
		FTS_DEBUG("[ESD]: Flow Work Cnt(reg0x91) keep a value for 5 times");
		FTS_DEBUG("Need execute TP reset!!");
		return true;
	}
...
static int esdcheck_algorithm(struct fts_ts_data *ts_data)
{
	...
	if (fts_esdcheck_data.suspend == 1) {
		 ...
		 * But in suspend, then will don't queue_delayed_work, when resume,
		 * don't check ESD again
		 */
		return 0;
	}
