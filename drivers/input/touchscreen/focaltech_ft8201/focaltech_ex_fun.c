...
int fts8201_create_apk_debug_channel(struct fts_ts_data *ts_data)
{
	struct ftxxxx_proc *proc = &ts_data->proc;

	proc->proc_entry = proc_create(PROC_NAME, 0660, NULL, &fts_proc_fops);
...
void fts8201_release_apk_debug_channel(struct fts_ts_data *ts_data)
{
	struct ftxxxx_proc *proc = &ts_data->proc;
...
{
	ssize_t count = 0;
	struct irq_desc *desc = irq_to_desc(fts8201_data->irq);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (FTS_SYSFS_ECHO_ON(buf)) {
		...
		fts8201_data->fw_is_running = false;
	} else if (FTS_SYSFS_ECHO_OFF(buf)) {
		FTS_INFO("[EX-FUN]set to fw mode");
		fts8201_data->fw_is_running = true;
	}
	mutex_unlock(&input_dev->mutex);
	FTS_FUNC_EXIT();
	...
	struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	FTS_FUNC_ENTER();
	mutex_lock(&input_dev->mutex);
	if (true == fts8201_data->fw_is_running)
	...
	struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < fts8201_data->pnt_buf_size; i++) {
		count += snprintf(buf + count, PAGE_SIZE, "%02x ",
				  fts8201_data->point_buf[i]);
	}
	count += snprintf(buf + count, PAGE_SIZE, "\n");
	mutex_unlock(&input_dev->mutex);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mutex_lock(&input_dev->mutex);
	sscanf(buf, "%d", &value);
	FTS_DEBUG("log level:%d->%d", fts8201_data->log_level, value);
	fts8201_data->log_level = value;
	mutex_unlock(&input_dev->mutex);
	FTS_FUNC_EXIT();
...
/*
 * upgrade from fw bin file
 * example:
 *	echo "*.bin" > fts8201_upgrade_bin
 */
...
static DEVICE_ATTR(fts_log_level, 0644, fts_log_level_show, fts_log_level_store);
...
int fts8201_create_sysfs(struct fts_ts_data *ts_data)
{
	int ret = 0;
	...
	ts_data->fw_ver_kobj = kobject_create_and_add("touchscreen", NULL);
	if (ts_data->fw_ver_kobj == NULL) {
		FTS_ERROR("[EX]: kobject_create_and_add() failed!!");
		return -ENOMEM;
	}

	ret = sysfs_create_group(ts_data->fw_ver_kobj, &fts_version_attribute_group);
	if (ret) {
		FTS_ERROR("[EX]: fts_version_attribute_group: sysfs_create_group() failed");
		kobject_put(ts_data->fw_ver_kobj);
	}
	return ret;
}

int fts8201_remove_sysfs(struct fts_ts_data *ts_data)
{
	sysfs_remove_group(&ts_data->dev->kobj, &fts_attribute_group);
	if (ts_data->fw_ver_kobj != NULL) {
		sysfs_remove_group(ts_data->fw_ver_kobj, &fts_version_attribute_group);
		kobject_del(ts_data->fw_ver_kobj);
		kobject_put(ts_data->fw_ver_kobj);
	}
	return 0;
}
