...
static int fts_ft5822_upgrade_in_kernel_get_app_bin_file_ver(struct i2c_client
							     *client);
static int fts_ft5822_upgrade_in_kernel_with_app_bin(struct i2c_client *client);
...
struct fts_upgrade_fun fts_updatefun = {
	...
	.get_app_i_file_ver = fts_ft5822_upgrade_in_kernel_get_app_bin_file_ver,
	.upgrade_with_app_i_file = fts_ft5822_upgrade_in_kernel_with_app_bin,
					  ...
					  u8 *vendor_id)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if ((reg_val[0] == chip_types.bootloader_idh)
			...
			FTS_DEBUG("[UPGRADE]: read bootloader id ok!! ID1 = 0x%x,ID2 = 0x%x!!",
			     reg_val[0], reg_val[1]);
			break;
		}
		FTS_ERROR("[UPGRADE]: read bootloader id fail!! ID1 = 0x%x,ID2 = 0x%x!!",
			reg_val[0], reg_val[1]);
		continue;
	}
...
/************************************************************************
* Name: fts_ft5822_upgrade_in_kernel_get_app_bin_file_ver
* Brief:  get .bin file version
* Input: no
* Output: no
* Return: fw version
***********************************************************************/
							     ...
							     *client)
{
	...
	if (!fw) {
		dev_err(&client->dev, "%s [ERROR] request_firmware\n",
			__func__);
		return -EIO;
	}
	...
	if (fw->size <= APP_FILE_VER_MAPPING) {
		release_firmware(fw);
		return -ENOMEM;
	}

	fw_buf = (char *)fw->data;
	fwver = fw_buf[APP_FILE_VER_MAPPING];
	release_firmware(fw);
...
static bool fts_check_app_bin_valid(u8 *pbt_buf)
{
	...
	if (((ecc1 + ecc2) != 0xFF) || ((ecc3 + ecc4) != 0xFF)) {
		FTS_DEBUG("[UPGRADE]APP.BIN Verify- ECC(%x %x) XOR error", ecc1,
			  ecc2);
		return false;
	}
...
* Return: fail <0
***********************************************************************/
static int fts_ft5822_upgrade_use_buf(struct i2c_client *client, u8 *pbt_buf,
				      u32 dw_lenth)
{
	u8 reg_val[4] = { 0 };
	u32 i = 0;
	...
	/*********Step 3:Check bootloader ID *****/
		...
		if ((reg_val[0] == chip_types.bootloader_idh)
		    && (reg_val[1] == chip_types.bootloader_idl)) {
			FTS_DEBUG("[UPGRADE]: read bootload id ok!! ID1 = 0x%x,ID2 = 0x%x!!",
			     reg_val[0], reg_val[1]);
			break;
		}
		FTS_ERROR("[UPGRADE]: read bootload id fail!! ID1 = 0x%x,ID2 = 0x%x!!",
			reg_val[0], reg_val[1]);
		continue;
	}

	if (i >= FTS_UPGRADE_LOOP) {
		FTS_ERROR("[UPGRADE]: failed writing  0x55 and 0xaa : i = %d!!",
			  i);
		return -EIO;
	}
	...
	for (j = 0; j < packet_number; j++) {
		...
		for (i = 0; i < 30; i++) {
			...
			FTS_DEBUG
			    ("[UPGRADE]: reg_val[0] = %x reg_val[1] = %x!!",
			     reg_val[0], reg_val[1]);
			/* msleep(1); */
	...
	if ((dw_lenth) % FTS_PACKET_LENGTH > 0) {
		...
		for (i = 0; i < 30; i++) {
			...
			FTS_DEBUG("[UPGRADE]: reg_val[0] = %x reg_val[1] = %x  reg_val[2] = 0x%x!!",
			     reg_val[0], reg_val[1],
			     (((packet_number * FTS_PACKET_LENGTH) /
...
/************************************************************************
* Name: fts_ft5822_upgrade_in_kernel_with_app_bin
...
static int fts_ft5822_upgrade_in_kernel_with_app_bin(struct i2c_client *client)
{
	...
	fw_buf = (char *)fw->data;
	i_ret = fts_ft5822_upgrade_use_buf(client, fw_buf, fw->size);
						...
						char *firmware_name)
{
	...
	FTS_ERROR("[UPGRADE]**********start upgrade with app.bin**********");
	...
	if (pbt_buf[APP_FILE_CHIPID_MAPPING] != chip_types.chip_idh) {
		FTS_ERROR
		    ("[UPGRADE]: chip id is error, app.bin upgrade failed!!");
		goto ERROR_BIN;
	}
	...
	if (ecc_ok) {
		...
		if (i_ret != 0) {
			...
			FTS_ERROR("[UPGRADE]: upgrade app.bin succeed");
		}
	} else {
