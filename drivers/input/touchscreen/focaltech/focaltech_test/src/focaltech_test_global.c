...
int g_TestItemNum;
char g_strIcName[strIcNameSIZE] = { 0 };
			    ...
			    char *IniFile)
{
	...
	if (ini_get_key(IniFile, section, ItemName, value) < 0) {
		...
		snprintf(returnValue, returnValueSize, "%s", value);
		return 0;
	}
	len = snprintf(returnValue, returnValueSize, "%s", value);
...
void fts_SetTestItemCodeName(unsigned char ucitemcode)
{
	 ...
	 * g_strEnumTestItem_FT8607[ucitemcode];
	 */
...
void InitStoreParamOfTestData(void)
{
	...
	/* Msg Area, Add Line1 */
	g_lenStoreMsgArea +=
	    snprintf(g_pStoreMsgArea, BUFF_LEN_STORE_MSG_AREA,
...
void MergeAllTestData(void)
{
	...
	iLen = snprintf(g_pTmpBuff, BUFF_LEN_TMP_BUFFER, "TestItem, %d, ",
			m_iTestDataCount);
	...
	FTS_TEST_DBG("lenStoreMsgArea=%d,  lenStoreDataArea = %d",
...
void FreeMemory(void)
{
	...
	/*if(NULL == g_pStoreAllData) fts_free(g_pStoreAllData); */
	if (g_pTmpBuff != NULL)
		fts_free(g_pTmpBuff);
}
