...
#define FTS_INI_TEST_RESULT_FILE_PATH "/data/"
			     ...
			     unsigned char *readbuf, int readlen)
{
	...
	iret =
	    fts_i2c_read(fts_i2c_client, writebuf, writelen, readbuf, readlen);
...
/*
 * Gets the configuration file size for
 * allocating memory to read configuration
 */
static int fts_test_get_ini_size(char *config_name)
{
...
static int fts_test_save_test_data(char *file_name, char *data_buf, int iLen)
{
	...
	memset(filepath, 0, sizeof(filepath));
	snprintf(filepath, sizeof(filepath), "%s%s",
			FTS_INI_TEST_RESULT_FILE_PATH, file_name);
	if (pfile == NULL)
...
static int fts_test_entry(char *ini_file_name)
{
	...
	char *printdata = NULL;
	/*
	 * The actual length of the test data in the library is
	...
	/*
	 * Used to obtain the test data stored in the library,
	 * pay attention to the size of the distribution space.
	 */
	...
	for (i = 0; i < iTestDataLen; i++) {
		if (('\0' == testdata[i])	/* Meet the end */
			/* Meet the print string length requirements */
			...
			if (icycle == 0) {
				...
				memcpy(printdata, testdata + print_index,
				       icycle);
...
int fts_test_init(struct i2c_client *client)
{
	...
	if (err != 0) {
		FTS_TEST_ERROR
		    ("[focal] %s() - ERROR: sysfs_create_group() failed.",
		     __func__);
