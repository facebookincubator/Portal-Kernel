...
#include "focaltech_test_ft5826s.h"
...
boolean FT5822_StartTest(void)
{
	...
	for (iItemCount = 0; iItemCount < g_TestItemNum; iItemCount++) {
		...
		if (Code_FT5822_ENTER_FACTORY_MODE ==
			...
			if (ReCode != ERROR_CODE_OK || (!bTempResult)) {
				bTestResult = false;
...
unsigned char FT5822_TestItem_EnterFactoryMode(void)
{
	...
	if (ReCode != ERROR_CODE_OK)
		return ReCode;
	/* After the success of the factory model,
...
unsigned char FT5822_TestItem_RawDataTest(bool *bTestResult)
{
	...
	/* Determine whether for v3 TP first, and then read the value of
	 * the 0x54 and check the mapping type is right or not,if not,
	 * write the register rawdata test mapping
	...
	if (ReCode != ERROR_CODE_OK)
		goto TEST_ERR;
	/* if original value is not the value needed,
	 * write the register to change
	 */
	if (g_ScreenSetParam.isNormalize == Auto_Normalize) {
		...
		if (g_stCfg_FT5822_BasicThreshold.RawDataTest_SetLowFreq) {
			...
			if (ReCode != ERROR_CODE_OK)
				goto TEST_ERR;
			SysDelay(100);
			/* change register value before,
			 * need to lose 3 frame data
			 */
			for (index = 0; index < 3; ++index)
				ReCode = GetRawData();
	...
	TestResultLen +=
	    snprintf(TestResult + TestResultLen,
		     BUFF_LEN_TMP_BUFFER - TestResultLen,
		     "RawData Test is %s.\n\n",
		     (btmpresult ? "OK" : "NG"));
	/* -------------------------Result */
	if (btmpresult) {
...
/* int ibiggerValue = 0; */
	...
	if (g_stCfg_FT5822_BasicThreshold.SCapRawDataTest_SetWaterproof_ON
		...
		for (i = 0; bFlag && i < g_ScreenSetParam.iRxNum; i++) {
			...
			if (Value > RawDataMax || Value < RawDataMin) {
				btmpresult = false;
				FTS_TEST_ERROR("Failed. Num = %d, Value = %d, range = (%d, %d):",
	...
	if (g_stCfg_FT5822_BasicThreshold.SCapRawDataTest_SetWaterproof_OFF
		...
		for (i = 0; bFlag && i < g_ScreenSetParam.iRxNum; i++) {
			...
			/* FTS_TEST_DBG("zaxzax3 Value %d RawDataMin %d
			 * RawDataMax %d ", Value, RawDataMin, RawDataMax);
			 */
			/* strTemp += str; */
			if (iMax < Value)
				iMax = Value;
		...
		for (i = 0; bFlag && i < g_ScreenSetParam.iTxNum; i++) {
			...
			/* FTS_TEST_DBG("zaxzax4 Value %d RawDataMin %d
			 * RawDataMax %d ",Value, RawDataMin,
			 * RawDataMax);
			 */
			iAvg += Value;
			if (iMax < Value)
	...
	if (m_bV3TP) {
		...
		/* Only self content will be used before the Mapping,
		 * so the end of the test items, need to go after Mapping
		 */
		GetChannelNum();
	}
...
/* int ibiggerValue = 0; */
	...
	if (g_stCfg_FT5822_BasicThreshold.SCapCbTest_SetWaterproof_ON
		...
		FTS_TEST_DBG("SCap CB in Waterproof-ON, Max : %d, Min: %d, Deviation: %d, Average: %d",
		     iMax, iMin, iMax - iMin, iAvg);
		/* ////////////////////////////Save Test Data */
		/* ibiggerValue = g_ScreenSetParam.iTxNum >
	...
	bFlag = GetTestCondition(WT_NeedProofOffTest, wc_value);
	if (g_stCfg_FT5822_BasicThreshold.SCapCbTest_SetWaterproof_OFF
	    && bFlag) {
...
unsigned char FT5822_TestItem_PanelDifferTest(bool *bTestResult)
{
	...
	/* Previously changed the register required to lose three frame data,
	 * the fourth frame is the use of the data
	 */
			  ...
			  int ByteNum, int *pRevBuffer)
{
	...
	for (i = 1; i < iReadNum; i++) {
		...
		if (i == iReadNum - 1) {	/* last packet */
			focal_msleep(10);
			ReCode =
			    Comm_Base_IIC_IO(NULL, 0,
					     m_ucTempData + BYTES_PER_TIME * i,
...
*        int iArrayIndex, unsigned char Row, unsigned char Col,
*	 unsigned char ItemCount
* Output: none
* Return: none
***********************************************************************/
			   ...
			   unsigned char ItemCount)
{
	...
	/* Save  Msg (ItemCode is enough, ItemName is not necessary,
	 * so set it to "NA".)
	 */
	iLen = snprintf(g_pTmpBuff, BUFF_LEN_TMP_BUFFER,
			"NA, %d, %d, %d, %d, %d, ", m_ucTestItemCode, Row, Col,
			m_iStartLine, ItemCount);
	...
	for (i = 0 + iArrayIndex; i < Row + iArrayIndex; i++) {
		for (j = 0; j < Col; j++) {
			...
			if (j == (Col - 1))
				iLen =
				    snprintf(g_pTmpBuff, BUFF_LEN_TMP_BUFFER,
					     "%d,\n ", iData[i][j]);
			else
				iLen = snprintf(g_pTmpBuff, BUFF_LEN_TMP_BUFFER,
						"%d, ", iData[i][j]);
...
static boolean GetTestCondition(int iTestType, unsigned char ucChannelValue)
{
	...
	switch (iTestType) {
	/* Bit5:  0:test waterProof mode ;  1 not test waterProof mode */
	case WT_NeedProofOnTest:
		bIsNeeded = !(ucChannelValue & 0x20);
		break;
	/* Bit7: 0: test normal mode  1:not test normal mode */
	...
	case WT_NeedRxOnVal:
		...
		break;
	/* Bit1,Bit0:  00:test normal Tx; 10: test normal Rx+Tx */
	case WT_NeedTxOffVal:
		bIsNeeded = (0x00 == (ucChannelValue & 0x03))
		    || (0x02 == (ucChannelValue & 0x03));
		break;
	/* Bit1,Bit0:  01: test normal Rx;    10: test normal Rx+Tx */
