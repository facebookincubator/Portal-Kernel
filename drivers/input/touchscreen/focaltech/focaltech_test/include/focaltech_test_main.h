...
int get_test_data(char *pTestData);	/* test result data. (pTestData,
					 *  External application for memory,
					 *  buff size >= 1024*80)
					 */
...
struct stTestItem {
	/* Classification of test items CfgItem, DataTestItem, GraphTestItem, */
...
(FTS_CHIP_TEST_TYPE == FT5822_TEST) ||			\
(FTS_CHIP_TEST_TYPE == FT5X46_TEST) ||			\
(FTS_CHIP_TEST_TYPE == FT3D47_TEST))
#define IC_Capacitance_Type     1
#elif ((FTS_CHIP_TEST_TYPE == FT8606_TEST) ||		\
(FTS_CHIP_TEST_TYPE == FT8607_TEST) ||			\
(FTS_CHIP_TEST_TYPE == FT8716_TEST) ||			\
(FTS_CHIP_TEST_TYPE == FT8736_TEST) ||			\
(FTS_CHIP_TEST_TYPE == FTE716_TEST) ||			\
(FTS_CHIP_TEST_TYPE == FTE736_TEST) ||			\
(FTS_CHIP_TEST_TYPE == FT8006_TEST))
...
#define FTS_TEST_DBG(fmt, args...)					\
		pr_err("[FTS] [TEST]%s. line: %d.  "fmt"\n",		\
		       __func__, __LINE__, ##args)
#define FTS_TEST_FUNC_ENTER()						\
	pr_err("[FTS][TEST]%s: Enter(%d)\n", __func__, __LINE__)
#define FTS_TEST_FUNC_EXIT()						\
	pr_err("[FTS][TEST]%s: Exit(%d)\n", __func__, __LINE__)
#else
...
#define FTS_TEST_INFO(fmt, args...)					\
		pr_err("[FTS][TEST][Info]%s. line: %d.  "fmt"\n",	\
		       __func__, __LINE__, ##args)
#define FTS_TEST_ERROR(fmt, args...)					\
		pr_err("[FTS][TEST][Error]%s. line: %d.  "fmt"\n",	\
		       __func__, __LINE__, ##args)
