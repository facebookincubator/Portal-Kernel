...
int idc_esdcheck_lcderror(void)
{
	...
	if (val == 0xAA) {
		 ...
		 * 2. LCD driver need reset(recovery) LCD and set
		 *    lcd_need_reset to 0
		 * 3. recover TP state
		 */
...
static int esdcheck_algorithm(void)
{
	...
	/* 3. check fts_esdcheck_data.proc_debug state, if 1-proc busy,
	 * no need check esd
	 */
	...
	if (err < 0) {
		...
		FTS_INFO(
		  "[ESD]: In factory mode, not check esd, return immediately!");
		return 0;
	}
...
*  Brief: When FTS TP driver is removed, then call this function to destroy work
*queue
*  Input:
*  Output:
*  Return:
...
#endif /* CONFIG_TOUCHSCREEN_FTS_ESDCHECK_EN */
