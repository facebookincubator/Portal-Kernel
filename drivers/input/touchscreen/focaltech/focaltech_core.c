...
#ifdef CONFIG_TOUCHSCREEN_FTS_POWER_SOURCE_CUST_EN
...
#define FTS_NORMAL_OPERATION_CURRENT 15600
...
#ifdef CONFIG_TOUCHSCREEN_FTS_DEBUG_EN
int g_show_log = 1;
#else
int g_show_log;
#endif
...
*  Brief:   Read chip id until TP FW become valid, need call when reset/power
*on/resume...
...
#ifdef CONFIG_TOUCHSCREEN_FTS_GESTURE_EN
	fts_gesture_recovery(client);
#endif
}

/*****************************************************************************
*  Name: fts_hwreset_proc
*  Brief: Execute reset operation
*  Input:
*  Output:
*  Return:
*****************************************************************************/
void fts_hwreset_proc(void)
{
	gpio_direction_output(fts_wq_data->pdata->reset_gpio, 0);
	usleep_range(100, 200);/* Trtp = 100us */
}

/*****************************************************************************
*  Name: fts_hwreset_proc_release
*  Brief: Execute reset release operation
*  Input: hdelayms - delay time unit:ms
*  Output:
*  Return:
*****************************************************************************/
void fts_hwreset_proc_release(int hdelayms)
{
	usleep_range(1000, 2000);/* Tvdr = 1ms */
			      ...
			      struct fts_ts_platform_data *pdata)
{
	...
	/* Enable KEY_POWER event if wakup support is required */
	if (data->pdata->touch_wakeup_on)
		__set_bit(KEY_POWER, input_dev->keybit);

#ifdef CONFIG_TOUCHSCREEN_FTS_MT_PROTOCOL_B_EN
...
#ifdef CONFIG_TOUCHSCREEN_FTS_REPORT_PRESSURE_EN
...
static int fts_power_source_init(struct fts_ts_data *data)
{
	...
	if (regulator_can_change_voltage(data->vcc_i2c) > 0) {
	...
	rc = regulator_set_load(data->vdd, FTS_NORMAL_OPERATION_CURRENT);
	if (rc < 0) {
		FTS_ERROR("Regulator set load failed %d", rc);
		goto reg_vdd_put;
	}

	if (regulator_can_change_voltage(data->vdd) > 0) {
...
static int fts_power_source_ctrl(struct fts_ts_data *data, int enable)
{
	...
	if (enable) {
		...
		usleep_range(200, 300); /* At lease 100us */
...
static void fts_show_touch_buffer(u8 *buf, int point_num)
{
	...
	if (len > (POINT_READ_BUF - 3))
		...
		len += FTS_ONE_TCH_LEN;
	count += snprintf(g_sz_debug, sizeof(g_sz_debug), "%02X,%02X,%02X",
			  buf[0], buf[1], buf[2]);
	for (i = 0; i < len; i++) {
		count += snprintf(g_sz_debug + count, sizeof(g_sz_debug) -
				  count, ",%02X", buf[i + 3]);
	}
...
#ifdef CONFIG_TOUCHSCREEN_FTS_PSENSOR_EN
...
static void fts_report_value(struct fts_ts_data *data)
{
	...
	if (data->gesture_wakeup_mode) {
		FTS_ERROR("wakeup by touch\n");
		data->gesture_wakeup_mode = false;
...
#ifdef CONFIG_TOUCHSCREEN_FTS_ESDCHECK_EN
			...
			const struct i2c_device_id *id)
{
	...
	if (err < 0) {
		FTS_ERROR("[REG]Fail to configure the regulators");
		goto free_gpio;
	}
	...
	/*
	 * After reset, first a few I2C error observed
	 * To avoid error, increase delay to 600ms
	 * TODO: revisit to optimize the time
	 */
	fts_hwreset_proc_release(600);
	err = fts_wait_tp_to_valid(client);
	if (err < 0) {
		FTS_ERROR("I2C read fail, TP not ready");
		goto disable_reg;
	}
...
#ifdef CONFIG_TOUCHSCREEN_FTS_APK_NODE_EN
	fts_create_apk_debug_channel(client);
#endif

#ifdef CONFIG_TOUCHSCREEN_FTS_SYSFS_NODE_EN
...
#ifdef CONFIG_TOUCHSCREEN_FTS_AUTO_UPGRADE_EN
	fts_ctpm_upgrade_init();
#endif

#ifdef CONFIG_TOUCHSCREEN_FTS_TEST_EN
...
#ifdef CONFIG_TOUCHSCREEN_FTS_POWER_SOURCE_CUST_EN
#ifdef CONFIG_TOUCHSCREEN_FTS_POWER_SOURCE_OFF_ON_SUSPEND_EN
	if (gpio_is_valid(data->pdata->reset_gpio)) {
		/* Set reset to 0 first */
		gpio_direction_output(fts_wq_data->pdata->reset_gpio, 0);
...
static int fts_ts_resume(struct device *dev)
{
	...
	if (data->pdata->touch_wakeup_on) {
		...
		fts_i2c_write_reg(data->client, FTS_REG_POWER_MODE,
				FTS_REG_POWER_MODE_NORMAL_VALUE);
...
#ifdef CONFIG_TOUCHSCREEN_FTS_POWER_SOURCE_OFF_ON_SUSPEND_EN
	fts_wait_tp_to_valid(data->client);
#endif
