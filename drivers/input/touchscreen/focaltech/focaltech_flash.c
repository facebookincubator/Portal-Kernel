...
{
#ifdef CONFIG_TOUCHSCREEN_FTS_AUTO_CLB_EN
	u8 uc_temp = 0x00;
	u8 i = 0;
	...
	if ((chip_types.chip_idh == 0x11) || (chip_types.chip_idh == 0x12) ||
	    (chip_types.chip_idh == 0x13) || (chip_types.chip_idh == 0x14)) {
		for (i = 0; i < 100; i++) {
...
int fts_GetFirmwareSize(char *firmware_name)
{
	...
	snprintf(filepath, sizeof(filepath), "%s%s", FTXXXX_INI_FILEPATH_CONFIG,
		 firmware_name);
	if (pfile == NULL)
		pfile = filp_open(filepath, O_RDONLY, 0);
	if (IS_ERR(pfile)) {
		FTS_ERROR("error occurred while opening file %s", filepath);
		return -EIO;
	}
...
int fts_ReadFirmware(char *firmware_name, u8 *firmware_buf)
{
	...
	if (pfile == NULL)
		pfile = filp_open(filepath, O_RDONLY, 0);
	if (IS_ERR(pfile)) {
		FTS_ERROR("[UPGRADE] Error occurred while opening file %s.\n",
			  filepath);
		return -EIO;
	}
	inode = pfile->f_path.dentry->d_inode;
...
* Return: fw version
***********************************************************************/
int fts_ctpm_get_app_ver(struct i2c_client *client)
{
	int i_ret = 0;

	if (fts_updatefun_curr.get_app_i_file_ver)
		i_ret = fts_updatefun_curr.get_app_i_file_ver(client);
...
static int fts_ctpm_check_fw_status(struct i2c_client *client)
{
	...
	FTS_DEBUG
	    ("[UPGRADE]: chip_id = %02x%02x, chip_types.chip_idh = %02x%02x",
...
static int fts_ctpm_check_fw_ver(struct i2c_client *client)
{
	...
	uc_host_fm_ver = fts_ctpm_get_app_ver(client);

	FTS_ERROR("[UPGRADE]: uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x!!",
...
static int fts_ctpm_check_need_upgrade(struct i2c_client *client)
{
	...
	if (fw_status < 0) {
		...
		if (fts_ctpm_get_i_file(client, 1) != 0) {
			FTS_DEBUG
			    ("[UPGRADE]******Get upgrade file(fw) fail******");
			return -EIO;
		}

		if (fts_ctpm_check_fw_ver(client) == 1) {
			FTS_DEBUG
			    ("[UPGRADE]**********need upgrade fw**********");
			bUpgradeFlag = true;
		} else {
			FTS_DEBUG
			    ("[UPGRADE]********Don't need upgrade fw********");
			bUpgradeFlag = false;
		}
	} else {
		...
		if (fts_ctpm_get_i_file(client, 0) != 0) {
			FTS_DEBUG
			    ("[UPGRADE]*****Get upgrade file(flash) fail*****");
			fts_ctpm_rom_or_pram_reset(client);
			return -EIO;
		}
		fts_ctpm_rom_or_pram_reset(client);
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	FTS_ERROR("[UPGRADE]***********check upgrade need or not***********");
	bUpgradeFlag = fts_ctpm_check_need_upgrade(client);
	FTS_ERROR("[UPGRADE]**********bUpgradeFlag = 0x%x**********",
		  bUpgradeFlag);

	if (bUpgradeFlag <= 0) {
		FTS_ERROR("[UPGRADE]**********No Upgrade, exit**********");
		return 0;
	}
	/* FW Upgrade */
	do {
		uc_upgrade_times++;
		FTS_ERROR("[UPGRADE]*************star upgrade(%d)*************",
			  uc_upgrade_times);
		...
		if (i_ret == 0) {
			...
			fts_i2c_read_reg(client, FTS_REG_FW_VER, &uc_tp_fm_ver);
			FTS_ERROR(
			  "[UPGRADE]**Success upgrade to new fw version 0x%x**",
			  uc_tp_fm_ver);
		...
		fts_ctpm_rom_or_pram_reset(client);
	} while (uc_upgrade_times < 2);	/* if upgrade fail,
					 *  upgrade again. then return
					 */
...
#endif /* #ifdef CONFIG_TOUCHSCREEN_FTS_AUTO_UPGRADE_EN */
