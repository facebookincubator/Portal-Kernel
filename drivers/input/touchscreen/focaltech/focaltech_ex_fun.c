			       ...
			       size_t count, loff_t *ppos)
{
	...
	int ret = 0;
	char tmp[25] = {0};
	int buflen = min(count, sizeof(writebuf));
	...
	case PROC_UPGRADE: {
		...
		snprintf(upgrade_file_path, sizeof(upgrade_file_path), "%s",
			 writebuf + 1);
		...
		if (fts_updatefun_curr.upgrade_with_app_bin_file)
			ret = fts_updatefun_curr.upgrade_with_app_bin_file(
			      ...
			      size_t count, loff_t *ppos)
{
	...
	case PROC_UPGRADE:
		...
		if (ret < 0)
			num_read_chars = scnprintf(buf, READ_BUF_SIZE, "%s",
						  "get fw version failed.\n");
		else
			num_read_chars = scnprintf(buf, READ_BUF_SIZE,
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	fts_hwreset_proc();
	fts_hwreset_proc_release(200);


	count = scnprintf(buf, PAGE_SIZE, "hw reset executed\n");
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (fts_i2c_read_reg(fts_i2c_client, FTS_REG_FW_VER, &fwver) < 0) {
		num_read_chars = scnprintf(buf, PAGE_SIZE,
					  "I2c transfer error!\n");
	}
#ifdef CONFIG_TOUCHSCREEN_FTS_ESDCHECK_EN
	fts_esdcheck_proc_busy(0);
	...
	if (fwver == 255)
		...
		num_read_chars = scnprintf(buf, PAGE_SIZE, "%02X\n", fwver);
	}
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (!g_rwreg_result.op) {
		if (g_rwreg_result.result == 0) {
			count = scnprintf(buf, PAGE_SIZE, "Read %02X: %02X\n",
					 g_rwreg_result.reg,
				 ...
				 size_t count)
{
	...
	if (num_read_chars == 2) {
		...
		if (g_rwreg_result.result < 0) {
			...
			FTS_INFO("Write 0x%02x into register(0x%02x) successful"
				 , regvalue, regaddr);
			g_rwreg_result.result = 0;
		}
	}
...
#ifdef CONFIG_TOUCHSCREEN_FTS_AUTO_UPGRADE_FOR_LCD_CFG_EN
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	count = scnprintf(buf, PAGE_SIZE,  "Esd check: %s\n",
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	count += scnprintf(buf, PAGE_SIZE, "FTS_CHIP_TYPE: \t\t\t%04X\n",
		  FTS_CHIP_TYPE);
...
#ifdef CONFIG_TOUCHSCREEN_FTS_GLOVE_EN
	count += scnprintf(buf + count, PAGE_SIZE - count,
		  "FTS_GLOVE_EN: \t\t\t%s\n", "ON");
#else
	count += scnprintf(buf + count, PAGE_SIZE - count,
...
#ifdef CONFIG_TOUCHSCREEN_FTS_COVER_EN
	count += scnprintf(buf + count, PAGE_SIZE - count,
		  "FTS_COVER_EN: \t\t%s\n", "ON");
#else
	count += scnprintf(buf + count, PAGE_SIZE - count,
...
#ifdef CONFIG_TOUCHSCREEN_FTS_CHARGER_EN
	count += scnprintf(buf + count, PAGE_SIZE - count,
		  "FTS_CHARGER_EN: \t\t\t%s\n", "ON");
#else
	count += scnprintf(buf + count, PAGE_SIZE - count,
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	count = scnprintf(buf, PAGE_SIZE, "Log: %s\n",
			 g_show_log ? "On" : "Off");
				...
				struct device_attribute *attr, char *buf)
{
	...
	fts_i2c_read_reg(client, FTS_REG_POWER_MODE,
			 &regvalue); /* power mode 0:active 1:monitor 3:sleep */
	count += scnprintf(tmp + count, buf_size, "Power Mode:0x%02x\n",
			  regvalue);

	fts_i2c_read_reg(client, FTS_REG_FW_VER, &regvalue); /* FWver */
	if (buf_size <= count)
		return -ENOMEM;
	count += scnprintf(tmp + count, buf_size - count, "FW Ver:0x%02x\n",
			  regvalue);

	fts_i2c_read_reg(client, FTS_REG_VENDOR_ID, &regvalue); /* Vendor ID */
	if (buf_size <= count)
		return -ENOMEM;
	count += scnprintf(tmp + count, buf_size - count, "Vendor ID:0x%02x\n",
			  regvalue);
	...
	if (buf_size <= count)
		return -ENOMEM;
	count += scnprintf(tmp + count, buf_size - count,
			  "LCD Busy Number:0x%02x\n", regvalue);

	fts_i2c_read_reg(client, FTS_REG_GESTURE_EN,
			 &regvalue); /* 1 Gesture mode,0 Normal mode */
	if (buf_size <= count)
		return -ENOMEM;
	count += scnprintf(tmp + count, buf_size - count,
			  "Gesture Mode:0x%02x\n", regvalue);
	...
	fts_i2c_read_reg(client, FTS_REG_FLOW_WORK_CNT,
			 &regvalue); /* Flow work counter */
	if (buf_size <= count)
		return -ENOMEM;
	count += scnprintf(tmp + count, buf_size - count,
			  "ESD count:0x%02x\n", regvalue);
