...
#define FTS_SYSFS_ECHO_ON(buf)                                                 \
	((strncasecmp(buf, "1", 1) == 0) || (strncasecmp(buf, "on", 2) == 0))
#define FTS_SYSFS_ECHO_OFF(buf)                                                \
	((strncasecmp(buf, "0", 1) == 0) || (strncasecmp(buf, "off", 3) == 0))
...
struct ts_event {
	...
	u8 au8_touch_event[FTS_MAX_POINTS]; /* touch event: 0 -- down;
					     * 1-- up; 2 -- contact
					     */
