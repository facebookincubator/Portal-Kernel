...
int api_protocol_check_mode(uint8_t *inbuf, uint8_t *outbuf)
{
	...
	tp_log_info("ilitek ic. mode =0x%x , it's %s\n", outbuf[0],
			((outbuf[0] == 0x5A) ? "AP MODE" : "BL MODE"));
...
int api_protocol_get_ptl(uint8_t *inbuf, uint8_t *outbuf)
{
	...
	ilitek_data->ptl.ver = (((int)outbuf[0]) << 16) + (((int)outbuf[1]) << 8) + outbuf[2];
...
int api_protocol_init_func(void){
	...
	if(ilitek_data->ptl.ver_major == 0x3 || ilitek_data->ptl.ver == 0x10600 || ilitek_data->ptl.ver == 0x10700) {
	...
	} else if(ilitek_data->ptl.ver_major == 0x6 || ilitek_data->ptl.ver == 0x10800) {
...
int api_protocol_set_funcmode(int mode)
{
	...
	printk("ilitek_data->ptl.ver:0x%x\n", ilitek_data->ptl.ver);
