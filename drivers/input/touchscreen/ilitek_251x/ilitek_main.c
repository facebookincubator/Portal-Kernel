...
unsigned char fw_name[20];
unsigned char ilitek_hwminor;
static int __init read_hwminor(char *hwminor_str)
{
	int ret;

	snprintf(fw_name, sizeof(fw_name), "FW6%sX.hex", hwminor_str);
	fw_name[sizeof(fw_name) - 1] = '\0'; /* Force NULL termination */

	ret = kstrtou8(hwminor_str, 16, &ilitek_hwminor);
	if (ret < 0)
		return ret;

	return 0;
}
__setup("androidboot.hw_minor=0x", read_hwminor);

unsigned char ilitek_hwmajor;
static int __init read_hwmajor(char *hwmajor_str)
{
	int ret;

	ret = kstrtou8(hwmajor_str, 16, &ilitek_hwmajor);
	if (ret < 0)
		return ret;

	return 0;
}
__setup("androidboot.hw_major=0x", read_hwmajor);
...
static ssize_t ilitek_firmware_version_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	/* Avoid effecting boot FW upgrade flow */
...
int ilitek_read_data_and_report_6XX(void) {
	...
	#ifdef ILITEK_GESTURE
	#if ILITEK_GESTURE == ILITEK_CLICK_WAKEUP
		tp_log_info("driver trigger power key event\n.");
		input_report_key(input, KEY_WAKEUP, 1);
		input_sync(input);
		input_report_key(input, KEY_WAKEUP, 0);
		input_sync(input);
...
static int ilitek_get_gpio_num(void)
{
	...
	ilitek_data->touch_id = of_get_named_gpio(np, "touch_id-gpio", 0);
	if (gpio_is_valid(ilitek_data->touch_id))
		tp_log_err("touch_id = %d\n", ilitek_data->touch_id);
#endif
#endif
#else
...
static int ilitek_request_gpio(void)
{
	...
	if (gpio_is_valid(ilitek_data->touch_id)) {
		ret = gpio_request(ilitek_data->touch_id, "touch_id-gpio");
		if (ret) {
			tp_log_err("Failed to request touch_id\n");
			/* Make sure driver will not use it. */
			ilitek_data->touch_id = -1;
		} else {
			ret = gpio_direction_input(ilitek_data->touch_id);
			if (ret)
				tp_log_err("Failed to direction input touch_id err\n");
		}
	}
#endif
	return ret;
...
static int ilitek_power_on(bool status)
{
	...
	if (status) {
		if (ilitek_data->vdd) {
			ret = regulator_set_load(ilitek_data->vdd, 100000);
			if (ret < 0) {
				tp_log_err("regulator_set_load vdd fail\n");
				return -EINVAL;
			}
...
int ilitek_read_tp_info(void)
{
	...
	if (api_protocol_set_cmd(ILITEK_TP_CMD_GET_TP_RESOLUTION, NULL, outbuf) < ILITEK_SUCCESS)
		goto transfer_err;
	if (api_protocol_set_cmd(ILITEK_TP_CMD_READ_MODE, NULL, outbuf) <
			ILITEK_SUCCESS)
		goto transfer_err;
transfer_err:
	return ret;
