...
static int32_t ilitek_sensortest_proc_show(struct seq_file *m, void *v)
{
	...
	printk("ilitek_allnode_max_threshold = %d\n",ilitek_allnode_max_threshold);
	ilitek_reset(ilitek_data->reset_time);
	ilitek_irq_enable();
	ilitek_data->operation_protection = false;
...
static ssize_t ilitek_sensortest_write(struct file *pFile, const char __user *buf, size_t size, loff_t *pPos)
{
	...
	tp_log_info("\n");
	//check buffer size and trim down to 512 if needed
	if (size > sizeof(buffer)) {
		tp_log_err("userspace buffer is bigger than local buffer\n");
		size = 512;
	}
...
int hex_mapping_convert(uint32_t addr,uint8_t *buffer) {
	...
	for (count = 0; count < HEX_FWVERSION_SIZE - 4; count++)
		pr_cont("0x%x,", ilitek_data->upg.hex_fw_ver[count]);
	//get df start address
	tp_log_info("hex df start address:");
	...
	{
			pr_cont("0x%x,", buffer[count]);
	}
	tp_log_info("hex ic type:");
...
int32_t ilitek_parse_hex_file(uint32_t *df_startaddr, uint32_t *df_endaddr,
			      uint32_t *df_checksum, uint32_t *ap_startaddr,
			      uint32_t *ap_endaddr, uint32_t *ap_checksum,
			      int32_t hex_len, uint8_t *CTPM_FW,
			      uint8_t *save_hex_content)
{
	...
	//save customers version
	tp_log_info("end addr:0x%x\n", addr);
	if (*df_endaddr > *df_startaddr) {
		ilitek_data->has_df = true;
		/* Firmware version stored from df_startaddr +
		 * HEX_FWVERSION_OFFSET.  Contain 4 bytes FW version and
		 * 4 bytes Customers version.
		 */
		memcpy(ilitek_data->upg.hex_fw_ver + 4, CTPM_FW +
				(*df_startaddr + HEX_FWVERSION_OFFSET + 4), 4);
		tp_log_info("Customer Version:%d.%d.%d.%d\n",
				ilitek_data->upg.hex_fw_ver[4],
				ilitek_data->upg.hex_fw_ver[5],
				ilitek_data->upg.hex_fw_ver[6],
				ilitek_data->upg.hex_fw_ver[7]);
	}
	if (hex_end == 0) {
...
static ssize_t ilitek_update_with_hex_read(struct file *pFile, char __user *buf, size_t nCount, loff_t *pPos)
{
	...
	if (!(strstr(ilitek_data->upg.filename, ".hex"))) {
		...
		nCount = sprintf(tmpbuf, "ilitek_data->upg.filename is invalid ilitek_data->upg.filename = %s\n", ilitek_data->upg.filename);
		goto out;
	} else {
		request_firmware(&fw, ilitek_data->upg.filename,
				&ilitek_data->client->dev);
		if (!fw) {
			tp_log_err("hex File Open Error ilitek_data->upg.filename = %s\n", ilitek_data->upg.filename);
			nCount = snprintf(tmpbuf, sizeof(tmpbuf),
					"hex Open Error upg.filename = %s\n",
					ilitek_data->upg.filename);
			goto out;
		} else {
			...
			if (!(save_hex_content)) {
				...
				nCount = snprintf(tmpbuf, sizeof(tmpbuf),
				"alloctation save_hex_content memory failed\n");
				goto out;
			}
	...
	if (api_protocol_set_cmd(ILITEK_TP_CMD_GET_KERNEL_VERSION, NULL, outbuf) < ILITEK_SUCCESS) {
		...
		ret = ilitek_parse_hex_file(&df_startaddr, &df_endaddr, &df_checksum, &ap_startaddr, &ap_endaddr,
			&ap_checksum, fw->size, CTPM_FW, save_hex_content);
		if (ret < 0) {
