...
#define MIP4_EVENT_FORMAT_RETRY_COUNT 10

/* Define event format when I2C read fails */
#define DEFAULT_EVENT_FORMAT 1
#define DEFAULT_EVENT_SIZE 8
...
int mip4_ts_enable(struct mip4_ts_info *info)
{
	...
	if (info->enabled) {
		dev_err(&info->client->dev, "%s already enabled just reset\n",
								__func__);
		/* Do not reset if fw_upgrading in progress */
		if (!info->fw_upgrading)
			mip4_ts_reset(info);

		if (info->touch_wakeup_on)
...
static int mip4_ts_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	struct mip4_ts_info *info =
		container_of(self, struct mip4_ts_info, fb_notifier);
...
int mip4_ts_get_fw_version(struct mip4_ts_info *info, u8 *ver_buf)
{
	...
	if (!info->enabled) {
		memcpy(ver_buf, info->fw_version, FW_MAX_SECT_NUM*sizeof(u16));
		return 0;
	}
...
static int mip4_ts_alert_handler_sram(struct mip4_ts_info *info, u8 *data)
{
	...
	if (info->sram_addr_num > 8) {
		dev_err(&info->client->dev,
			"%s [ERROR] sram_addr_num [%d]\n",
			__func__, info->sram_addr_num);
		goto error;
	}

	for (i = 0; i < info->sram_addr_num; i++) {
		info->sram_addr[i] = data[2 + 4 * i] |
					(data[2 + 4 * i + 1] << 8) |
					(data[2 + 4 * i + 2] << 16) |
					(data[2 + 4 * i + 3] << 24);
		dev_info(&info->client->dev,
			"%s - sram_addr #%d [0x%08X]\n",
...
static irqreturn_t mip4_ts_interrupt(int irq, void *dev_id)
{
	...
	/* Check packet size */
	if ((size <= 0) || (size > 200)) {
		dev_dbg(&info->client->dev, "%s [ERROR] Packet size [%d]\n",
								__func__, size);
		if (!startup) {
			info->i2c_error_cnt++;
			goto error;
...
int mip4_ts_config(struct mip4_ts_info *info)
{
	...
	u8 rbuf[16];
	int ret = 0;
	int retry = MIP4_EVENT_FORMAT_RETRY_COUNT;

	dev_dbg(&info->client->dev, "%s [START]\n", __func__);

	/* Disable IRQ before reading configs */
	disable_irq(info->client->irq);
	...
	if (retry < 0) {
		/*
		 * In case event format read error happens, we use
		 * default values to make interrupt handler working
		 * If event format is not correct, interrupt handler
		 * ignores all touch interrupt
		 */
		info->event_format = DEFAULT_EVENT_FORMAT;
		info->event_size = DEFAULT_EVENT_SIZE;

		dev_err(&info->client->dev,
			"%s [ERROR] event format - use default\n", __func__);
		goto error;
	}
...
int mip4_ts_fw_update_from_kernel(struct mip4_ts_info *info, bool force)
{
	...
	/* Update firmware */
	do {
		ret = mip4_ts_flash_fw(info, fw->data, fw->size,
							force, true, false);
		if (ret >= fw_err_none) {
			break;
		}
					...
					char *path, bool force, bool isc)
{
	...
	dev_notice(&info->client->dev, "Firmware update started\n");
	...
	if (fw_size > 0) {
		...
		if (nread != fw_size) {
			...
			/* Update firmware */
			ret = mip4_ts_flash_fw(info, fw_data, fw_size,
							force, true, isc);
		}
...
error_not_enabled:
	...
	if (ret < fw_err_none) {
		...
		dev_notice(&info->client->dev, "Firmware update done\n");
	}
...
static ssize_t mip4_ts_sys_fw_update(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	case fw_err_file_open:
		scnprintf(data, sizeof(data),
			"F/W update failed : File open error\n");
		break;
	...
	/* Read configuration only when firmware is updated */
	if (ret == fw_err_none) {
		/* Re-config driver */
		mip4_ts_config(info);
...
static DEVICE_ATTR(fw_update, S_IRUSR, mip4_ts_sys_fw_update, NULL);
...
static void mip4_ts_check_fw_work(struct work_struct *work)
{
	struct mip4_ts_info *info =
		container_of(work, struct mip4_ts_info, check_fw_work);
	int ret;
	...
	if (ret < 0) {
		dev_err(&info->client->dev,
			"%s [ERROR] mip4_ts_fw_update_from_kernel\n", __func__);
		return;
	}
...
static int mip4_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	INIT_WORK(&info->check_fw_work, mip4_ts_check_fw_work);
	...
	/* If touch wakeup is needed, enable wakeup feature */
	if (info->touch_wakeup_on)
	...
	schedule_work(&info->check_fw_work);
...
#if (defined(CONFIG_PM) || USE_FB_NOTIFY)
/*
