...
static int mip4_ts_proc_table_data(struct mip4_ts_info *info, u8 data_type_size, u8 data_type_sign, u8 buf_addr_h, u8 buf_addr_l, u8 row_num, u8 col_num, u8 buf_col_num, u8 rotate)
{
	...
	case 1:
		for (i_x = 0; i_x < max_x; i_x++) {
			printk("[%2d]", i_x);
			scnprintf(data, sizeof(data), "[%2d]", i_x);
			strlcat(info->print_buf, data, PAGE_SIZE);
			memset(data, 0, 10);
		}
		break;
	case 2:
		for (i_x = 0; i_x < max_x; i_x++) {
			printk("[%4d]", i_x);
			scnprintf(data, sizeof(data), "[%4d]", i_x);
			strlcat(info->print_buf, data, PAGE_SIZE);
			memset(data, 0, 10);
		}
		break;
	case 4:
		for (i_x = 0; i_x < max_x; i_x++) {
			printk("[%5d]", i_x);
			scnprintf(data, sizeof(data), "[%5d]", i_x);
			strlcat(info->print_buf, data, PAGE_SIZE);
			memset(data, 0, 10);
		}
		break;
	...
	for (i_y = 0; i_y < max_y; i_y++) {
		/* print line header */
		printk("[%2d]", i_y);
		scnprintf(data, sizeof(data), "[%2d]", i_y);
		strlcat(info->print_buf, data, PAGE_SIZE);
		memset(data, 0, 10);
		...
		for (i_x = 0; i_x < max_x; i_x++) {
			...
			case 1:
				...
				scnprintf(data, sizeof(data), " %3d", info->image_buf[i_y * max_x + i_x]);
				break;
			case 2:
				printk(" %5d", info->image_buf[i_y * max_x + i_x]);
				scnprintf(data, sizeof(data), " %5d", info->image_buf[i_y * max_x + i_x]);
				break;
			case 4:
				printk(" %6d", info->image_buf[i_y * max_x + i_x]);
				scnprintf(data, sizeof(data), " %6d", info->image_buf[i_y * max_x + i_x]);
				break;
			default:
				dev_err(&info->client->dev, "%s [ERROR] data_type_size[%d]\n", __func__, data_type_size);
				goto error;
...
static int mip4_ts_proc_vector_data(struct mip4_ts_info *info, u8 data_type_size, u8 data_type_sign, u8 buf_addr_h, u8 buf_addr_l, u8 key_num, u8 vector_num, u16 *vector_id, u16 *vector_elem_num, int table_size)
{
	...
	for (i_line = 0; i_line < (key_exist + vector_num); i_line++) {
		if ((i_line == 0) && (key_exist == 1)) {
			elem_len = key_num;
			printk("[Key]");
			scnprintf(data, sizeof(data), "[Key]");
		} else {
			...
			case 1:
				printk("[Screen Rx]");
				scnprintf(data, sizeof(data), "[Screen Rx]");
				break;
			case 2:
				printk("[Screen Tx]");
				scnprintf(data, sizeof(data), "[Screen Tx]");
				break;
			case 3:
				printk("[Key Rx]");
				scnprintf(data, sizeof(data), "[Key Rx]");
				break;
			case 4:
				printk("[Key Tx]");
				scnprintf(data, sizeof(data), "[Key Tx]");
				break;
			case 5:
				printk("[Pressure]");
				scnprintf(data, sizeof(data), "[Pressure]");
				break;
			default:
				printk("[Vector %d]", i_vector);
				scnprintf(data, sizeof(data), "[Vector %d]", i_vector);
				break;
			}
			i_vector++;
		}
		...
		for (i = i_elem; i < (i_elem + elem_len); i++) {
			...
			case 1:
				...
				scnprintf(data, sizeof(data), " %3d", info->image_buf[table_size + i]);
				break;
			case 2:
				printk(" %5d", info->image_buf[table_size + i]);
				scnprintf(data, sizeof(data), " %5d", info->image_buf[table_size + i]);
				break;
			case 4:
				printk(" %6d", info->image_buf[table_size + i]);
				scnprintf(data, sizeof(data), " %6d", info->image_buf[table_size + i]);
				break;
			default:
				dev_err(&info->client->dev, "%s [ERROR] data_type_size[%d]\n", __func__, data_type_size);
				goto error;
...
int mip4_ts_run_test(struct mip4_ts_info *info, u8 test_type)
{
	...
	case MIP4_TEST_TYPE_CM:
		...
		scnprintf(info->print_buf, PAGE_SIZE, "\n=== Cm Test ===\n\n");
		break;
...
static ssize_t mip4_ts_sys_fw_version(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (mip4_ts_get_fw_version(info, rbuf)) {
		...
		scnprintf(data, sizeof(data), "F/W Version : ERROR\n");
		goto error;
	}
	...
	scnprintf(data, sizeof(data), "F/W Version : %02X.%02X/%02X.%02X/%02X.%02X/%02X.%02X\n", rbuf[0], rbuf[1], rbuf[2], rbuf[3], rbuf[4], rbuf[5], rbuf[6], rbuf[7]);
...
static ssize_t mip4_ts_sys_fw_update_from_storage(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	/* Update firmware */
	ret = mip4_ts_fw_update_from_storage(info,
				info->fw_path_ext, true, false);

	switch (ret) {
	case fw_err_none:
		scnprintf(data, sizeof(data), "F/W update success.\n");
		break;
	case fw_err_uptodate:
		scnprintf(data, sizeof(data), "F/W is already up-to-date.\n");
		break;
	case fw_err_download:
		scnprintf(data, sizeof(data), "F/W update failed : Download error\n");
		break;
	case fw_err_file_type:
		scnprintf(data, sizeof(data), "F/W update failed : File type error\n");
		break;
	case fw_err_file_open:
		scnprintf(data, sizeof(data), "F/W update failed : File open error[%s]\n", info->fw_path_ext);
		break;
	case fw_err_file_read:
		scnprintf(data, sizeof(data), "F/W update failed : File read error\n");
		break;
	default:
		scnprintf(data, sizeof(data), "F/W update failed.\n");
		break;
	}
	...
	result = scnprintf(buf, 255, "%s\n", data);
	return result;
}
/*
* Update bootloader from external storage
*/
static ssize_t mip4_ts_sys_bl_update_from_storage(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	...
	/* Update firmware */
	ret = mip4_ts_fw_update_from_storage(info,
				info->bl_path_ext, true, true);

	switch (ret) {
	case fw_err_none:
		scnprintf(data, sizeof(data), "Bootloader update success.\n");
		break;
	case fw_err_uptodate:
		scnprintf(data, sizeof(data),
	...
	case fw_err_download:
		scnprintf(data, sizeof(data),
				"Bootloader update failed : Download error\n");
		break;
	case fw_err_file_type:
		scnprintf(data, sizeof(data),
				"Bootloader update failed : File type error\n");
		break;
	case fw_err_file_open:
		scnprintf(data, sizeof(data),
			"Bootloader update failed : File open error[%s]\n",
			info->bl_path_ext);
		break;
	case fw_err_file_read:
		scnprintf(data, sizeof(data),
			"Bootloader update failed : File read error\n");
		break;
	default:
		scnprintf(data, sizeof(data), "Bootloader update failed.\n");
		break;
	}
...
static ssize_t mip4_ts_sys_fw_path_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	scnprintf(data, sizeof(data), "Path : %s\n", info->fw_path_ext);
...
static ssize_t mip4_ts_sys_bin_version(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (mip4_ts_get_fw_version_from_bin(info, rbuf)) {
		...
		scnprintf(data, sizeof(data), "BIN Version : ERROR\n");
		goto error;
	}

	dev_info(&info->client->dev, "%s - BIN Version : %02X.%02X/%02X.%02X/%02X.%02X/%02X.%02X\n", __func__, rbuf[0], rbuf[1], rbuf[2], rbuf[3], rbuf[4], rbuf[5], rbuf[6], rbuf[7]);
	scnprintf(data, sizeof(data), "BIN Version : %02X.%02X/%02X.%02X/%02X.%02X/%02X.%02X\n", rbuf[0], rbuf[1], rbuf[2], rbuf[3], rbuf[4], rbuf[5], rbuf[6], rbuf[7]);
...
static ssize_t mip4_ts_sys_flash_cal_table(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (mip4_ts_flash_cal_table(info)) {
		...
		scnprintf(data, sizeof(data), "ERROR");
		goto error;
	}
	scnprintf(data, sizeof(data), "DONE");
...
static ssize_t mip4_ts_sys_info(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	scnprintf(info->print_buf, PAGE_SIZE, "Device Name : %s\n", MIP4_TS_DEVICE_NAME);

	scnprintf(data, sizeof(data), "Driver Chip Name : %s\n", CHIP_NAME);
	strlcat(info->print_buf, data, PAGE_SIZE);

	wbuf[0] = MIP4_R0_INFO;
	wbuf[1] = MIP4_R1_INFO_IC_NAME;
	mip4_ts_i2c_read(info, wbuf, 2, rbuf, 4);
	scnprintf(data, sizeof(data), "Chip Model Code : %.*s\n", 4, rbuf);
	strlcat(info->print_buf, data, PAGE_SIZE);

	wbuf[0] = MIP4_R0_INFO;
	wbuf[1] = MIP4_R1_INFO_PRODUCT_NAME;
	mip4_ts_i2c_read(info, wbuf, 2, rbuf, 16);
	scnprintf(data, sizeof(data), "Product Name : %.*s\n", 16, rbuf);
	strlcat(info->print_buf, data, PAGE_SIZE);
	...
	scnprintf(data, sizeof(data), "Resolution : X[%d] Y[%d]\n", res_x, res_y);
	strlcat(info->print_buf, data, PAGE_SIZE);

	len_x = (rbuf[8]) | (rbuf[9] << 8);
	len_y = (rbuf[10]) | (rbuf[11] << 8);
	scnprintf(data, sizeof(data), "Panel size (dmm) : X[%d] Y[%d]\n", len_x, len_y);
	strlcat(info->print_buf, data, PAGE_SIZE);

	ppm_x = rbuf[12];
	ppm_y = rbuf[13];
	scnprintf(data, sizeof(data), "Pixels per mm : X[%d] Y[%d]\n", ppm_x, ppm_y);
	strlcat(info->print_buf, data, PAGE_SIZE);

	scnprintf(data, sizeof(data), "Node Num : X[%d] Y[%d] Key[%d]\n", rbuf[4], rbuf[5], rbuf[6]);
	strlcat(info->print_buf, data, PAGE_SIZE);
...
static ssize_t mip4_ts_sys_power_on(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	scnprintf(data, sizeof(data), "Power : On\n");

	ret = scnprintf(buf, 225, "%s\n", data);
	return ret;
...
static ssize_t mip4_ts_sys_power_off(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	scnprintf(data, sizeof(data), "Power : Off\n");
...
static ssize_t mip4_ts_sys_restart(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	scnprintf(data, sizeof(data), "Restart\n");
...
static ssize_t mip4_ts_sys_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (!strcmp(attr->attr.name, "mode_glove")) {
		...
		dev_err(&info->client->dev, "%s [ERROR] Unknown mode[%s]\n", __func__, attr->attr.name);
		scnprintf(data, sizeof(data), "%s : Unknown Mode\n", attr->attr.name);
		goto exit;
	}

	if (mip4_ts_i2c_read(info, wbuf, 2, rbuf, 1)) {
		dev_err(&info->client->dev, "%s [ERROR] mip4_ts_i2c_read\n", __func__);
		scnprintf(data, sizeof(data), "%s : ERROR\n", attr->attr.name);
	} else {
		dev_info(&info->client->dev, "%s - addr[0x%02X%02X] value[%d]\n", __func__, wbuf[0], wbuf[1], rbuf[0]);
		scnprintf(data, sizeof(data), "%s : %d\n", attr->attr.name, rbuf[0]);
	}
	...
	for (iy = 0; iy < size; iy++) {
		for (ix = (iy + 1); ix < size; ix++) {
			...
			if (info->image_buf[idx] != 255) {
				...
				scnprintf(data, sizeof(data), "[%d]", ch);
				strlcat(info->print_buf, data, PAGE_SIZE);
...
static ssize_t mip4_ts_sys_wakeup_gesture(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	scnprintf(data, sizeof(data), "gesture:%d\n", info->wakeup_gesture_code);
...
#if (CHIP_MODEL != CHIP_NONE)
static DEVICE_ATTR(fw_update_storage, S_IRUSR, mip4_ts_sys_fw_update_from_storage, NULL);
static DEVICE_ATTR(bl_update_storage, S_IRUSR,
			mip4_ts_sys_bl_update_from_storage, NULL);
static DEVICE_ATTR(fw_path, (S_IRUSR | S_IWUSR ), mip4_ts_sys_fw_path_show, mip4_ts_sys_fw_path_store);
...
#endif /* DEBUG */
static DEVICE_ATTR(test_auto, S_IRUSR, mip4_ts_sys_test_auto, NULL);
#if USE_WAKEUP_GESTURE
