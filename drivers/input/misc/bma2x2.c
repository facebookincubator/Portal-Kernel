		...
		u8 *data, u16 len)
{
	...
	if (retry >= BMA_MAX_RETRY_I2C_XFER) {
		dev_info(&client->dev, "I2C xfer error");
		return -EIO;
	}
					...
					struct bma2x2_data *data)
{
	...
	while (read_count++ < CHECK_CHIP_ID_TIME_MAX) {
		if (bma2x2_smbus_read_byte(client, BMA2X2_CHIP_ID_REG,
							&chip_id) < 0) {
			dev_err(&client->dev, "Bosch Sensortec Device not foundi2c bus read error, read chip_id:%d\n",
			chip_id);
			err = -ENODEV;
			return err;
		}
		...
		const char *buf, size_t count)
{
	...
	rc = sscanf(buf, "%3d %3d", &type, &value);
	if (rc != 2) {
		dev_err(&client->dev, "sscanf failed = %d\n", rc);
		return -EINVAL;
	}
		...
		const char *buf, size_t count)
{
...
#ifdef CONFIG_SENSORS_BMI058
	dev_info(&client->dev, "diff y is %d,value1 is %d, value2 is %d\n",
			diff, value1, value2);
	test_result_branch = 2;
#else
	dev_info(&client->dev, "diff x is %d,value1 is %d, value2 is %d\n",
			diff, value1, value2);
	...
	dev_info(&client->dev, "diff z is %d,value1 is %d, value2 is %d\n",
			diff, value1, value2);
	...
	bma2x2_soft_reset(bma2x2->bma2x2_client);
	msleep(20);
	dev_info(&client->dev, "self test finished\n");
...
static ssize_t bma2x2_register_store(struct device *dev,
		...
		const char *buf, size_t count)
{
	int address, value, rc;
	struct i2c_client *client = to_i2c_client(dev);
	struct bma2x2_data *bma2x2 = i2c_get_clientdata(client);

	rc = sscanf(buf, "%3d %3d", &address, &value);
	if (rc != 2) {
		dev_err(&client->dev, "scanf failed %d\n", rc);
		return -EINVAL;
	}
		...
		const char *buf, size_t count)
{
	...
	struct i2c_client *client = to_i2c_client(dev);
	struct bma2x2_data *bma2x2 = i2c_get_clientdata(client);
	unsigned int curr_delay = atomic_read(&bma2x2->delay);
	...
	if (data > curr_delay) // Faster polling request gets precedence
		data = curr_delay;
...
static void bma2x2_set_enable(struct device *dev, int enable)
{
	...
	en_count = atomic_read(&bma2x2->enable);
	if (enable) {
		if (en_count == 0) {
		...
		if (en_count == 1) {
			...
			dev_warn(&client->dev,
				"bma2x2: Calling accel disable without first enabling it!");
			goto out;
		}
		atomic_dec(&bma2x2->enable);
	}
		...
		const char *buf, size_t count)
{
	...
	mutex_lock(&bma2x2->en_count_mutex);
	if (data == 1) {
		bma2x2->en_count++;
		if (bma2x2->en_count == 1) {
			bma2x2_set_enable(dev, 1);
		}
	} else if (data == 0) {
		if (bma2x2->en_count == 0) {
			dev_warn(dev, "Calling accel disable without first enabling it!");
			mutex_unlock(&bma2x2->en_count_mutex);
			return -EINVAL;
		}
		bma2x2->en_count--;
		if (bma2x2->en_count == 0) {
...
/*dev_info(&client->dev, "wait 2ms cal ready flag is %d\n", tmp);
 */
		timeout++;
		if (timeout == 50) {
			dev_info(&client->dev, "get fast calibration ready error\n");
			return -EINVAL;
		};

	} while (tmp == 0);

	dev_info(&client->dev, "x axis fast calibration finished\n");
	return count;
}
 ...
 */
	...
	dev_info(&client->dev, "y axis fast calibration finished\n");
	return count;
}
 ...
 */
	...
	dev_info(&client->dev, "z axis fast calibration finished\n");
	return count;
}
...
/*Update BMI058 fifo_data_sel to the BMA2x2 common definition*/
	...
	if ((bma2x2->bst_pd != NULL) &&
		(bma2x2->bst_pd->place != BOSCH_SENSOR_PLACE_UNKNOWN)) {
		place = bma2x2->bst_pd->place;
		/* sensor with place 0 needs not to be remapped */
		if ((place > 0) && (place < MAX_AXIS_REMAP_TAB_SZ)) {
...
static void bma2x2_tap_timeout_handle(unsigned long data)
{
	...
	dev_info(&client->dev, "tap interrupt handle, timeout\n");
...
static DEVICE_ATTR(range, 0660,
		bma2x2_range_show, bma2x2_range_store);
static DEVICE_ATTR(bandwidth, 0660,
...
static DEVICE_ATTR(fifo_data_frame, 0440,
...
static DEVICE_ATTR(offset_x, 0660,
		bma2x2_offset_x_show,
		bma2x2_offset_x_store);
static DEVICE_ATTR(offset_y, 0660,
		bma2x2_offset_y_show,
		bma2x2_offset_y_store);
static DEVICE_ATTR(offset_z, 0660,
...
static void bma2x2_high_g_interrupt_handle(struct bma2x2_data *bma2x2)
{
	...
	for (i = 0; i < 3; i++) {
		...
		dev_info(&bma2x2->bma2x2_client->dev,
...
static void bma2x2_irq_work_func(struct work_struct *work)
{
	...
	if ((status&0x80) == 0x80) {
		/* dev_info(&client->dev, "New data interrupt happened\n");*/
	...
	if (status & 0x20) {
		if (atomic_read(&bma2x2->en_double_tap) == 1) {
			dev_info(&client->dev, "single tap interrupt happened\n");
			...
			if (bma2x2->tap_times == 0)	{
				...
				dev_info(&client->dev, "double tap\n");
				mutex_lock(&bma2x2->tap_mutex);
				bma2x2->tap_times = 0;
	...
	case 0x01:
		dev_info(&client->dev, "Low G interrupt happened\n");
	...
	case 0x08:
		dev_info(&client->dev, "slow/ no motion interrupt happened\n");
		input_report_rel(bma2x2->dev_interrupt,
	...
	case 0x10:
		dev_info(&client->dev, "double tap interrupt happened\n");
		input_report_rel(bma2x2->dev_interrupt,
			DOUBLE_TAP_INTERRUPT,
			DOUBLE_TAP_INTERRUPT_HAPPENED);
		break;
	...
	case 0x40:
		bma2x2_get_orient_status(bma2x2->bma2x2_client,
				    &first_value);
		dev_info(&client->dev, "orient interrupt happened,%s\n",
				orient[first_value]);
		if (first_value == 0)
	...
	case 0x80:
		bma2x2_get_orient_flat_status(bma2x2->bma2x2_client,
				    &sign_value);
		dev_info(&client->dev, "flat interrupt happened,flat status is %d\n",
				    sign_value);
		if (sign_value == 1) {
			input_report_abs(bma2x2->dev_interrupt,
				FLAT_INTERRUPT,
		...
		const struct i2c_device_id *id)
{
	...
	input_set_capability(dev_interrupt, EV_REL,
		SINGLE_TAP_INTERRUPT);
#ifdef CONFIG_SENSORS_BMA2X2_ORIENT_INTERRUPT
	...
	if (client->dev.platform_data != NULL) {
		...
		err = of_property_read_u32(np, "bosch,place", &temp_val);
		if (err) {
			dev_err(&client->dev, "Unable to read sensor place parameter. Using defaults.\n");
			temp_val = 0;
		}
		if (temp_val > 7) {
			dev_err(&client->dev, "Invalid place parameter, use default value 0\n");
			data->bst_pd->place = 0;
		} else {
			data->bst_pd->place = temp_val;
		}
	}
