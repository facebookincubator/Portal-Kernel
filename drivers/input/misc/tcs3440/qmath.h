...
static INLINE fixedp _qkdiv(long long x, long long y, int xFb, int yFb, int resFb)
{
	fixedp ret;
	uint64_t tmp = (uint64_t)x << ((xFb) + (yFb) - (resFb));
	do_div(tmp, y);
	ret = tmp;
	return ret;
}

static INLINE fixedp _qk_s64_div(long long x, long y, int xFb, int yFb, int resFb)
{
	s64 result;
	uint64_t tmp = (uint64_t)x << ((xFb) + (yFb) - (resFb));
	result = div_s64(tmp, y);
	return (fixedp)result;
}
