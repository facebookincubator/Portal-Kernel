...
static void
gpio_keys_gpio_report_event(struct gpio_button_data *bdata, bool hw_event)
{
	...
	/*
	 * We come here both during resume from suspend and from the hw event
	 * workqueue (button press). While the latter requires notification
	 * to the privacy driver to handle the event, the former does not.
	 */
	if (hw_event)
		fbp_report_key_event(button->code, state);
