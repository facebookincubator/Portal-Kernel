...
msm_spi_use_dma(struct msm_spi *dd, struct spi_transfer *tr, u8 bpw)
{
	...
	/*
	 * Before enabling DMA, check and return false if the buffer
	 * is allocated by vmalloc.
	 */
	if ((tr->tx_buf && is_vmalloc_addr(tr->tx_buf)) ||
		(tr->rx_buf && is_vmalloc_addr(tr->rx_buf)))
		return false;
