...
static unsigned long apr_adsp_fw_cnt;

static bool is_adsp_fw_state_event_allowed(u32 event_id, u32 adsp_fw_state)
{
	bool ret = true;

	if (event_id >= FB_ADSP_EVENT_MAX ||
		(event_id != FB_ADSP_EVENT_QDSP_DOWN &&
		event_id != FB_ADSP_EVENT_QDSP_UP)) {
		pr_debug("%s: forbidden fb adsp event received\n", __func__);
		ret = false;
		goto exit;
	}

	if (event_id == FB_ADSP_EVENT_QDSP_DOWN) {
		if (adsp_fw_state != FB_ADSP_FW_CRASH) {
			pr_debug("%s: fb adsp fw down event blocked,\
				 state: %u\n", __func__, adsp_fw_state);
			ret = false;
		}
	} else if (event_id == FB_ADSP_EVENT_QDSP_UP) {
		if (adsp_fw_state != FB_ADSP_FW_STARTUP &&
			adsp_fw_state != FB_ADSP_FW_RECOVERY) {
			pr_debug("%s: fb adsp fw up event blocked,\
				 state: %u\n", __func__, adsp_fw_state);
			ret = false;
		}
	}
...
static void adsp_log_fw_state_event(u32 event_id, u32 adsp_fw_state)
{
	union fb_adsp_event fb_event;
	int ret = 0;

	if (false == is_adsp_fw_state_event_allowed(event_id, adsp_fw_state)) {
		pr_debug("%s: fb adsp fw event: %u blocked, state: %u\n",
			 __func__, event_id, adsp_fw_state);
		return;
	}

	fb_event.event_err = event_id;
	fb_event.state_err = adsp_fw_state;

	ret = fb_event_log_int(FBE_ADSP, fb_event.value);
	if (ret) {
		pr_err("%s: fb adsp fw event log failure, ret: %d!\n",
			__func__, ret);
	}
}
...
static int apr_send_uevent(char *env1, char *env2)
{
	...
	return kobject_uevent_env(&apr_dev_ptr->kobj, KOBJ_CHANGE, env);
}
...
static void apr_adsp_notify_down(void)
{
	char cnt[32];

	snprintf(cnt, sizeof(cnt), "ADSP_FW_COUNT=%ld", apr_adsp_fw_cnt);
	adsp_log_fw_state_event(FB_ADSP_EVENT_QDSP_DOWN, FB_ADSP_FW_CRASH);
	apr_send_uevent("ADSP_FB_UEVENT=ADSP_FW_CRASHED", cnt);
}

static void apr_adsp_notify_up(void)
{
	char *event, cnt[32];

	snprintf(cnt, sizeof(cnt), "ADSP_FW_COUNT=%ld", apr_adsp_fw_cnt);
	if (apr_adsp_fw_cnt++) {
		event = "ADSP_FB_UEVENT=ADSP_FW_RECOVERED";
		adsp_log_fw_state_event(FB_ADSP_EVENT_QDSP_UP, FB_ADSP_FW_RECOVERY);
	} else {
		event = "ADSP_FB_UEVENT=ADSP_FW_STARTUP";
		adsp_log_fw_state_event(FB_ADSP_EVENT_QDSP_UP, FB_ADSP_FW_STARTUP);
	}

	apr_send_uevent(event, cnt);
}
