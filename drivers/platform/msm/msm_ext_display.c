...
static int msm_ext_disp_get_vend_prod_edid_blk(struct platform_device *pdev,
	struct msm_ext_disp_vend_prod_edid_blk *blk)
{
	...
	ret = data->codec_ops.get_vend_prod_edid_blk(data->pdev, blk);
		...
		enum msm_ext_disp_cable_state state, u32 flags)
{
	...
	if (state == EXT_DISPLAY_CABLE_CONNECT) {
		...
		ops->get_vend_prod_edid_blk =
					msm_ext_disp_get_vend_prod_edid_blk;
	} else {
		...
		ops->get_vend_prod_edid_blk = NULL;
	}
end:
	return ret;
}
