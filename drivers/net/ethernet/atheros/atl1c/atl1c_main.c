...
static int atl1c_suspend(struct device *dev)
{
	...
	struct atl1c_adapter *adapter = netdev_priv(netdev);
	struct atl1c_hw *hw = &adapter->hw;
	struct ar8151_platform_ops *ops = msm_ar8151_get_ops(pdev);
	...
	if (ops && ops->suspend) {
		err = ops->suspend(adapter->platform_handle);
		if (err)
			dev_err(&pdev->dev, "platform_ops suspend : %d\n", err);
	}
...
static int atl1c_resume(struct device *dev)
{
	...
	if (ops && ops->resume) {
		err = ops->resume(adapter->platform_handle);
		if (err)
			dev_err(&pdev->dev, "platform_ops.resume : %d\n", err);
	}
...
static void atl1c_shutdown(struct pci_dev *pdev)
{
	...
	if (ops) {
		dev_err(&pdev->dev, "Shutdown atl1c: suspend is skipped\n");
		return;
	}
...
static int atl1c_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	int err = 0;
	err = msm_ar8151_modinit();
	if (err) {
		dev_err(&pdev->dev, "cannot enable PCI platform\n");
		return -EPROBE_DEFER;
	}
	...
	adapter->platform_handle = msm_ar8151_dev_init(&pdev->dev);
	if (!adapter->platform_handle) {
		err = -ENODEV;
		dev_err(&pdev->dev, "msm_ar8151_dev_init failed\n");
		goto err_ioremap;
	} else {
		/* disable msi */
		adapter->no_msi = true;
