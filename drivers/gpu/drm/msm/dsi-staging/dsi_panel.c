...
static int dsi_panel_power_on(struct dsi_panel *panel)
{
	...
	/*
	 * Some touch controllers rely on the same regulators powering on
	 * the display to be enabled for touch-to-wakeup to work. In these
	 * cases, delay powering off the sources until display's resuming
	 * from sleep.
	 */
	if (gpio_is_valid(panel->reset_config.reset_gpio) &&
			panel->reset_config.rst_gpio_delay &&
			panel->display_suspend) {
		gpio_set_value(panel->reset_config.reset_gpio, 0);
		// Add delay before proceeding.
		msleep(panel->reset_config.rst_gpio_delay);
	}

	if (panel->reset_config.disable_pwr_delay && panel->display_suspend) {
		rc = dsi_pwr_enable_regulator(&panel->power_info, false);
		if (rc) {
			pr_err("[%s] failed to disable vregs while turning on display!\n",
							panel->name);
			goto exit;
		}
		/* Add a delay to let the regulators go down all the way */
		msleep(panel->reset_config.disable_pwr_delay);
	}
	panel->display_suspend = false;
...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
	...
	if (!panel->reset_config.disable_pwr_delay) {
		rc = dsi_pwr_enable_regulator(&panel->power_info, false);
		if (rc)
			pr_err("[%s] failed to disable vregs, rc=%d\n",
							panel->name, rc);
	}
	panel->display_suspend = true;
	return rc;
}
...
static int dsi_panel_ext_bl_register(struct dsi_panel *panel)
{
	struct device_node *ext_backlight;

	ext_backlight = of_parse_phandle(panel->panel_of_node,
				"qcom,mdss-dsi-bl-ext-backlight", 0);
	if (ext_backlight) {
		panel->bl_config.bd =
			of_find_backlight_by_node(ext_backlight);
		of_node_put(ext_backlight);
...
static int dsi_panel_bklt_pwm(struct dsi_panel *panel, int level)
{
	...
	if (!bl->pwm) {
		dev_err(panel->parent, "no PWM\n");
		return -ENODEV;
	}

	if (level == 0) {
		ret = pwm_config_us(bl->pwm, 0, bl->pwm_period_usecs);
		if (ret)
			dev_err(dev, "pwm_config() failed err=%d.\n", ret);
		pwm_disable(bl->pwm);
		return 0;
	}

	duty = (level * bl->pwm_period_usecs) / bl->bl_max_level;

	dev_dbg(dev, "level=%d ; pwm_period=%d us; duty=%d us\n",
		level, bl->pwm_period_usecs, duty);

	if (panel->bl_config.pwm_period_usecs >= USEC_PER_SEC) {
		ret = pwm_config_us(bl->pwm, duty, bl->pwm_period_usecs);
		if (ret) {
			dev_err(dev, "pwm_config_us() failed err=%d.\n", ret);
			return ret;
		}
	} else {
		period = bl->pwm_period_usecs * NSEC_PER_USEC;
		ret = pwm_config(bl->pwm, duty * NSEC_PER_USEC, period);
		if (ret) {
			dev_err(dev, "pwm_config() failed err=%d.\n", ret);
			return ret;
		}
	...
	if (pwm_is_enabled(bl->pwm))
		return 0;

	return pwm_enable(bl->pwm);
}
...
int dsi_panel_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	case DSI_BACKLIGHT_PWM:
		dsi_panel_bklt_pwm(panel, bl_lvl);
		break;
	...
	case DSI_BACKLIGHT_DCS:
		...
		break;
	case DSI_BACKLIGHT_EXT:
		if (bl->bd) {
			bl->bd->props.brightness = bl_lvl;
			rc = backlight_update_status(bl->bd);
		}
		break;
	default:
...
static int dsi_panel_bl_register(struct dsi_panel *panel)
{
	...
	case DSI_BACKLIGHT_DCS:
		break;
	case DSI_BACKLIGHT_EXT:
		dsi_panel_ext_bl_register(panel);
		break;
	default:
		pr_err("Backlight type(%d) not supported\n", bl->type);
		rc = -ENOTSUPP;
				 ...
				 struct device_node *of_node)
{
	...
	val = 0; // set to default value
	of_property_read_u32(of_node, "qcom,platform-reset-gpio-delay", &val);
	panel->reset_config.rst_gpio_delay = val;

	val = 0; // set to default value
	of_property_read_u32(of_node, "qcom,dsi-pwr-supply-toggle-delay", &val);
	panel->reset_config.disable_pwr_delay = val;

	val = 0; // set to default value
	of_property_read_u32(of_node,
			"qcom,mdss-dsi-pre-panel-off-delay", &val);
	panel->reset_config.pre_panel_off_delay_ms = val;
				     ...
				     struct device_node *of_node)
{
	...
	} else if (!strcmp(bl_type, "bl_ctrl_ext")) {
		panel->bl_config.type = DSI_BACKLIGHT_EXT;
	} else {
	...
	rc = of_property_read_u32(of_node, "qcom,mdss-brightness-default-level",
		&val);
	if (rc) {
		pr_debug("[%s] brightness-default-level unspecified, defaulting to max level\n",
			 panel->name);
		panel->bl_config.brightness = panel->bl_config.brightness_max_level;
	} else {
		panel->bl_config.brightness = val;
	}
	...
	if (panel->bl_config.type == DSI_BACKLIGHT_PWM) {
		...
		if (rc) {
			...
			panel->bl_config.pwm = of_pwm_get(of_node, "backlight");
			if (IS_ERR_OR_NULL(panel->bl_config.pwm)) {
				pr_err("[%s] Cannot get PWM device\n",
					panel->name);
				panel->bl_config.pwm = NULL;
				goto error;
			}
...
int dsi_panel_disable(struct dsi_panel *panel)
{
	...
	if (panel->reset_config.pre_panel_off_delay_ms)
		usleep_range(panel->reset_config.pre_panel_off_delay_ms * 1000,
			(panel->reset_config.pre_panel_off_delay_ms * 1000)
				+ 100);
