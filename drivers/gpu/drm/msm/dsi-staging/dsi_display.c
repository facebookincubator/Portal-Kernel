	...
	struct mipi_dsi_device *dsi)
{
	...
	if (display->t_clk_post && display->t_clk_pre) {
		panel->host_config.t_clk_post = display->t_clk_post;
		panel->host_config.t_clk_pre = display->t_clk_pre;
	} else {
...
static int dsi_display_parse_dt(struct dsi_display *display)
{
	...
	if (!of_node) {
		...
		display->bridge_force_hs_clk_lane = of_property_read_bool(
			display->pdev->dev.of_node,
			"qcom,bridge-force-hs-clk-lane");
		display->bridge_continuous_splash = of_property_read_bool(
			display->pdev->dev.of_node,
			"qcom,bridge-continuous-splash");
		display->bridge_lp11_init = of_property_read_bool(
			display->pdev->dev.of_node,
			"qcom,bridge-lp11-init");

		rc2 = of_property_read_u32(display->pdev->dev.of_node, "qcom,mdss-dsi-t-clk-post", &val);
		if (!rc2) {
			display->t_clk_post = val;
			pr_debug("t_clk_post = %02x\n", display->t_clk_post);
		}
		val = 0;
		rc2 = of_property_read_u32(display->pdev->dev.of_node, "qcom,mdss-dsi-t-clk-pre", &val);
		if (!rc2) {
			display->t_clk_pre = val;
			pr_debug("t_clk_pre = %02x\n", display->t_clk_pre);
		}
	} else {
		display->panel_of = of_node;
...
static int dsi_display_res_init(struct dsi_display *display)
{
	...
	if (panel_type == EXT_BRIDGE)
		display->panel->host_config.force_hs_clk_lane =
				display->bridge_force_hs_clk_lane;
...
int dsi_display_cont_splash_config(void *dsi_display)
{
	...
	if (dsi_display_has_ext_bridge(display)
		&& !display->bridge_continuous_splash) {
...
int dsi_display_prepare(struct dsi_display *display)
{
	...
	if (!display->is_cont_splash_enabled) {
		...
		if (display->panel->type == EXT_BRIDGE
				&& display->bridge_lp11_init) {
			bridge = of_drm_find_bridge(display->panel_of);
			if (bridge && bridge->funcs->power_on)
				bridge->funcs->power_on(bridge);
		}
