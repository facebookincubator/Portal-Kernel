...
struct usbpd {
	...
	bool			force_usb2_device_when_dp_connected;
};
...
/* Flag showing if DP is connected. true: connected, false not connected */
static bool is_dp;
...
static inline void start_usb_peripheral(struct usbpd *pd)
{
	...
	extcon_set_cable_state_(pd->extcon, EXTCON_USB_CC,
			cc == ORIENTATION_CC2);
	extcon_set_cable_state_(pd->extcon, EXTCON_USB_SPEED, !is_dp);
		...
		size_t num_data, enum pd_sop_type sop)
{
	...
	/* Set device mode if DP is connected */
	hdr = PD_MSG_HDR(msg_type, is_dp ? DR_DFP : pd->current_dr,
		pd->current_pr, pd->tx_msgid, num_data, pd->spec_rev);
...
static void handle_vdm_rx(struct usbpd *pd, struct rx_msg *rx_msg)
{
	...
	case SVDM_CMD_TYPE_RESP_ACK:
		...
		case USBPD_SVDM_DISCOVER_SVIDS:
			 ...
			 */
#ifdef CONFIG_USB_USE_USB2_DEVICE_WHEN_DP_CONNECTED
			if (!pd->force_usb2_device_when_dp_connected
				|| !has_dp) {
				start_usb_host(pd, !has_dp);
			} else {
				/*
				 * If DP is connected then
				 * configure as device
				 */
				stop_usb_host(pd);
				pd->current_dr = DR_UFP;
...
static void usbpd_sm(struct work_struct *w)
{
	...
	case PE_UNKNOWN:
		ptn36502_setmode(USB3_ONLY, usbpd_get_plug_orientation(pd));
		if (pd->current_pr == PR_SINK) {
...
struct usbpd *usbpd_create(struct device *parent)
{
	...
	pd->force_usb2_device_when_dp_connected =
			device_property_present(parent,
