				...
				uint32_t param_val, uint32_t block_id)
{
	...
	inst_id = (param_reg - IAXXX_PLUGIN_INS_GRP_PARAM_ID_ADDR) /
					(4 * IAXXX_PLUGIN_INS_GRP_REG_NUM);

	if (!priv->iaxxx_state->plgin[inst_id].plugin_inst_state) {
		dev_err(dev, "Plugin instance 0x%x is not created %s()\n",
				inst_id, __func__);
		goto plg_set_param_err;
	}

	ret = regmap_write(priv->regmap, param_reg, param_id);
	if (ret) {
		dev_err(dev, "write failed %s()\n", __func__);
		goto plg_set_param_err;
	}

	ret = regmap_write(priv->regmap, param_val_reg, param_val);
	if (ret) {
		dev_err(dev, "write failed %s()\n", __func__);
		goto plg_set_param_err;
	}
...
EXPORT_SYMBOL(iaxxx_core_plg_set_param);


/*****************************************************************************
 * iaxxx_core_plg_get_param()
 * @brief get a param in a plugin instance
 *
 * @id		Plugin Instance Id
 * @param_id	Param Id
				...
				uint32_t *param_val, uint32_t block_id)
{
	...
	ret = regmap_read(priv->regmap, param_val_reg,
				param_val);
	if (ret) {
		dev_err(dev, "read failed %s()\n", __func__);
		goto plg_get_param_err;
...
EXPORT_SYMBOL(iaxxx_core_plg_get_param);
		...
		const struct firmware *fw, uint32_t pkg_id, uint32_t *proc_id)
{
	...
	for (i = 0; i < header.number_of_sections; i++) {
		...
		if (file_section.start_address == IAXXX_BIN_INFO_SEC_ADDR)
			...
			buf_data = vzalloc(file_section.length
						* sizeof(uint32_t));
			if (!buf_data)
				return -ENOMEM;
			...
			if (rc) {
				dev_err(dev,
					"Unable to clear CHECKSUM_VALUE1, rc = %d\n",
					rc);
				return rc;
			}
			...
			if (rc) {
				dev_err(dev,
					"Unable to clear CHECKSUM_VALUE2, rc = %d\n",
					rc);
				return rc;
			}

			/*
			* Write sectAddr and sectLen to CHECKSUM_ADDR
			* and CHECKSUM_LEN register respectively.
			*/
			rc = regmap_write(priv->regmap,
					IAXXX_SRB_CHKSUM_BUFFER_ADDR_ADDR,
					file_section.start_address);
			if (rc) {
				dev_err(dev,
					"Failed to set CHECKSUM_ADDRESS, rc = %d\n",
					rc);
				return rc;
			}

			/*
			* Program the CHECKSUM_LENGTH with
			* the buffer length (in bytes)
			*/
			rc = regmap_write(priv->regmap,
					IAXXX_SRB_CHKSUM_LENGTH_ADDR,
					file_section.length);
			if (rc) {
				dev_err(dev,
					"Failed to set CHECKSUM_LENGTH, rc = %d\n",
					rc);
				return rc;
			}
			...
			rc = regmap_update_bits(priv->regmap,
					IAXXX_SRB_CHKSUM_ADDR,
					IAXXX_SRB_CHKSUM_REQ_MASK,
					0x1 << IAXXX_SRB_CHKSUM_REQ_POS);
			if (rc) {
				dev_err(dev,
					"Failed to set CHECKSUM_REQUEST, rc = %d\n",
					rc);
				return rc;
			}
			...
			if (rc) {
				dev_err(dev,
					"Failed to read CHECKSUM_VALUE1, rc = %d\n",
					rc);
				return rc;
			}
			...
			if (rc) {
				dev_err(dev,
					"Failed to read CHECKSUM_VALUE2, rc = %d\n",
					rc);
				return rc;
			}
		}
					...
					uint32_t pkg_id, uint32_t *proc_id)
{
	...
	iaxxx_fw_notifier_call(priv->dev, IAXXX_EV_PACKAGE, (char *)pkg_name);
out:
	release_firmware(fw);
	mutex_unlock(&priv->plugin_lock);
	return rc;
