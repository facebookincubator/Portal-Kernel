...
#define IAXXX_SYSID_INVALID 0xFFFF
			...
			uint16_t event_id, uint16_t dst_id, uint32_t dst_opaque)
{
	 ...
	 */
	ret = regmap_write(priv->regmap, IAXXX_SRB_EVT_ID_ADDR, event_id);
	if (ret) {
		dev_err(dev, "write failed %s()\n", __func__);
		return ret;
	}
	sys_id = ((dst_id << 16) | src_id);
	ret = regmap_write(priv->regmap, IAXXX_SRB_EVT_SUB_ADDR, sys_id);
	if (ret) {
		dev_err(dev, "write failed %s()\n", __func__);
		return ret;
	}
	ret = regmap_write(priv->regmap, IAXXX_SRB_EVT_SUB_DST_OPAQUE_ADDR,
			dst_opaque);
	if (ret) {
		dev_err(dev, "write failed %s()\n", __func__);
		return ret;
	}
	ret = regmap_update_bits(priv->regmap, IAXXX_SRB_EVT_ADDR,
		(1 << IAXXX_SRB_EVT_SUB_REQ_POS), IAXXX_SRB_EVT_SUB_REQ_MASK);
	if (ret) {
			...
			uint16_t event_id, uint16_t dst_id, uint32_t dst_opaque)
{
	...
	/*
	 * Update all event subscription registers
	 * Event ID, Subsystem IDS of source and destination, destination opaque
	 */
	ret = regmap_write(priv->regmap, IAXXX_SRB_EVT_ID_ADDR, event_id);
	if (ret) {
		dev_err(dev, "write failed %s()\n", __func__);
...
/**
 * iaxxx_get_event_flush - flush the event queue
 *
 * This function is used to flush the event Q if any pending events
 * are left before device enter into suspend mode.
 *
 */
int iaxxx_get_event_flush(struct iaxxx_priv *priv)
{
	...
	do {
		...
		if (event.event_src == IAXXX_CM4_CTRL_MGR_SRC_ID
			&& event.event_id == IAXXX_CRASH_EVENT_ID) {
			dev_err(dev, "FW crash %s: src:0x%.04x, proc id:0x%.04X\n",
			__func__, event.event_src, event.src_opaque);
			iaxxx_fw_crash(dev, IAXXX_FW_CRASH_ON_FLUSH_EVENTS);
			goto out;
		}

		/* Read the count of available events */
		rc = regmap_read(priv->regmap,
				IAXXX_SRB_EVT_COUNT_ADDR, &count);
		if (rc) {
