...
#define IAXXX_REGMAP_WAIT_CLEAR_TIMEOUT 0xFF
 ...
 */
#define UPDATE_BLOCK_FAIL_RETRIES 10
...
iaxxx_regmap_wait_match(struct iaxxx_priv *priv, uint32_t reg, uint32_t match)
{
	...
	if (retries == max_retries) {
		rc = -ETIMEDOUT;
		dev_err(dev,
			"No update block response from FW. Host timedout:%d\n",
			rc);
	}
...
static int iaxxx_read_error_register(struct iaxxx_priv *priv)
{
	...
	for (i = 0 ; i < IAXXX_MAX_CORES; i++)
		dev_info(priv->dev, "Block id %d : Err 0x%x Inst id 0x%x\n", i,
			...
			priv->iaxxx_state->err.plg_inst_id[i] == 0xFF ? 0
...
static int iaxxx_update_block_request(struct iaxxx_priv *priv, uint32_t *status)
{
	...
	/* Need the lock since callable from any cell drivers */
	mutex_lock(&priv->srb_lock);

	/*
	 * Set the UPDATE_BLOCK_REQUEST bit in and some reserved bits
	...
	if (rc) {
		...
		if (rc == -ETIMEDOUT)
			*status = IAXXX_REGMAP_WAIT_CLEAR_TIMEOUT;
		mutex_unlock(&priv->srb_lock);
		return rc;
	}
...
static int iaxxx_update_block1_request(struct iaxxx_priv *priv, uint32_t *status)
{
	int rc;
	int ret;
	struct device *dev = priv->dev;
	...
	/*
	 * Poll waiting for the UPDATE_BLOCK_REQUEST bit to clear
	 * amd reserved bits to match the value written
	 */
...
static int iaxxx_update_block2_request(struct iaxxx_priv *priv, uint32_t *status)
{
	int rc;
	int ret;
	struct device *dev = priv->dev;
				...
				u16 event_id, u16 event_src, u16 event_dst)
{
	...
	rc = regmap_update_bits(priv->regmap, IAXXX_SRB_EVT_SUB_ADDR,
			       IAXXX_SRB_EVT_SUB_SRC_ID_MASK,
			       event_src << IAXXX_SRB_EVT_SUB_SRC_ID_POS);
	if (rc) {
	...
	rc = regmap_update_bits(priv->regmap, IAXXX_SRB_EVT_SUB_ADDR,
			       IAXXX_SRB_EVT_SUB_DST_ID_MASK,
			       event_dst << IAXXX_SRB_EVT_SUB_DST_ID_POS);
	if (rc) {
	...
	rc = regmap_update_bits(priv->regmap, IAXXX_SRB_EVT_ID_ADDR,
			       IAXXX_SRB_EVT_ID_REG_MASK,
			       event_id << IAXXX_SRB_EVT_ID_REG_POS);
	if (rc) {
				...
				u16 event_src, u16 event_dst, u32 opaque_data)
{
	...
	rc = regmap_write(priv->regmap,
			  IAXXX_SRB_EVT_SUB_DST_OPAQUE_ADDR, opaque_data);
	if (rc) {
	...
	rc = regmap_update_bits(priv->regmap, IAXXX_SRB_EVT_ADDR,
			       IAXXX_SRB_EVT_SUB_REQ_MASK,
			       0x1 << IAXXX_SRB_EVT_SUB_REQ_POS);
	if (rc) {
	...
	if (rc) {
		...
		/* Clear the request bit */
		iaxxx_regmap_clear_bits(priv, IAXXX_SRB_EVT_ADDR,
					IAXXX_SRB_EVT_SUB_REQ_MASK);
	...
	rc = regmap_read(priv->regmap, IAXXX_SRB_EVT_ADDR, &status);
	if (rc) {
		dev_err(dev, "Failed to read EVENT_SUB_REQUEST, rc = %d\n", rc);
		goto out;
	}

	WARN_ON(status & IAXXX_SRB_EVT_SUB_REQ_MASK);
				...
				u16 event_id, u16 event_src, u16 event_dst)
{
	...
	rc = regmap_update_bits(priv->regmap, IAXXX_SRB_EVT_ADDR,
			       IAXXX_SRB_EVT_UNSUB_REQ_MASK,
			       0x1 << IAXXX_SRB_EVT_UNSUB_REQ_POS);
	if (rc) {
		dev_err(dev, "Failed to EVENT_SUB_REQUEST, rc = %d\n", rc);
		goto out;
	}
	...
	WARN_ON(status & IAXXX_SRB_EVT_UNSUB_REQ_MASK);
...
int iaxxx_next_event_request(struct iaxxx_priv *priv, struct iaxxx_event *ev)
{
	...
	rc = regmap_update_bits(priv->regmap, IAXXX_SRB_EVT_NEXT_REQ_ADDR,
			       IAXXX_SRB_EVT_NEXT_REQ_NOT_MASK,
			       0x1 << IAXXX_SRB_EVT_NEXT_REQ_NOT_POS);
	if (rc) {
	...
	if (rc) {
		...
		/* Clear the request bit */
		iaxxx_regmap_clear_bits(priv, IAXXX_SRB_EVT_NEXT_REQ_ADDR,
					IAXXX_SRB_EVT_NEXT_REQ_NOT_MASK);
	...
	rc = regmap_read(priv->regmap, IAXXX_SRB_EVT_NEXT_REQ_ADDR, &status);
	if (rc) {
		dev_err(dev, "Failed to read EVT_NEXT_REQ, rc = %d\n", rc);
		goto out;
	}

	WARN_ON(status & IAXXX_SRB_EVT_NEXT_REQ_NOT_MASK);
	rc = regmap_bulk_read(priv->regmap,
			IAXXX_SRB_EVT_SRC_INFO_ADDR, data, ARRAY_SIZE(data));
	if (rc) {
		dev_err(dev, "Failed to read EVENT_INFO, rc = %d\n", rc);
		goto out;
	}

	ev->event_src = GET_BITS(data[0], IAXXX_SRB_EVT_SRC_INFO_SYS_ID);
	ev->event_id  = GET_BITS(data[0], IAXXX_SRB_EVT_SRC_INFO_EVT_ID);
						...
						int id)
{
	...
	if (rc) {
		...
		case SYSRC_ERR_MEM:
			...
			break;
		case SYSRC_ERR_BUSY:
			dev_err(dev, "FW BUSY err:0x%x\n", *status);
			break;
		case SYSRC_ERR_PEND:
			dev_err(dev, "FW PEND err:0x%x\n", *status);
			break;
		case SYSRC_ERR_STATE:
			dev_err(dev, "FW STATE ERROR err:0x%x\n", *status);
			break;
		case SYSRC_ERR_LIMIT:
			dev_err(dev, "FW LIMIT ERROR err:0x%x\n", *status);
			break;
		case SYSRC_ERR_IO:
			dev_err(dev, "FW IO ERROR err:0x%x\n", *status);
			break;
		case SYSRC_ERR_PLUGIN:
			/* No recovery */
			dev_err(dev, "FW PLUGIN ERROR err:0x%x\n", *status);
			return -EINVAL;
		case SYSRC_ERR_PARM:
			dev_err(dev, "FW PARAM ERROR err:0x%x\n", *status);
			break;
		case SYSRC_ERR_BOUNDARY:
			dev_err(dev, "FW BOUNDARY ERROR err:0x%x\n", *status);
			break;
		case IAXXX_REGMAP_WAIT_CLEAR_TIMEOUT:
			/* No recovery here */
			dev_err(dev,
				"FW REGMAP_WAIT_CLEAR_TIMEOUT err:0x%x\n",
				*status);
			return -EINVAL;
		default:
			/* Unnkonw error. Do recovery */
			dev_err(dev, "FW Unknown error:0x%x\n", *status);
		}
	} else
		return rc;
