...
static int madera_ext_clk_control(struct madera *madera, int enable)
{
	int ret = 0;

	if (enable) {
		ret = clk_prepare_enable(madera->ext_clk);
		if (ret)
			dev_err(madera->dev,
					"%s: clk_prepare_enable failed: %d\n",
					__func__, ret);
	} else
		clk_disable_unprepare(madera->ext_clk);
...
static int madera_runtime_resume(struct device *dev)
{
	...
	ret = madera_ext_clk_control(madera, 1);
	if (ret)
		return ret;
...
err:
	...
	madera_ext_clk_control(madera, 0);
	return ret;
}
...
int madera_dev_init(struct madera *madera)
{
	...
	unsigned int board_rev_gpio1;
	unsigned int board_rev_gpio2;
	unsigned int board_rev_gpio3;
	unsigned int board_rev_gpio4;
	...
	ret = regmap_read(madera->regmap, MADERA_GPIO1_CTRL_1, &board_rev_gpio1);
	if (ret) {
		dev_err(dev, "Failed to read Board Revision: %d\n", ret);
		goto err_reset;
	}
	ret = regmap_read(madera->regmap, MADERA_GPIO2_CTRL_1, &board_rev_gpio2);
	if (ret) {
		dev_err(dev, "Failed to read Board Revision: %d\n", ret);
		goto err_reset;
	}
	ret = regmap_read(madera->regmap, MADERA_GPIO3_CTRL_1, &board_rev_gpio3);
	if (ret) {
		dev_err(dev, "Failed to read Board Revision: %d\n", ret);
		goto err_reset;
	}
	ret = regmap_read(madera->regmap, MADERA_GPIO4_CTRL_1, &board_rev_gpio4);
	if (ret) {
		dev_err(dev, "Failed to read Board Revision: %d\n", ret);
		goto err_reset;
	}

	dev_info(dev, "%s Board ID %x\n", name,
				(board_rev_gpio4>>0xF)<<3 |
				(board_rev_gpio3>>0xF)<<2 |
				(board_rev_gpio2>>0xF)<<1 |
				(board_rev_gpio1>>0xF));
	...
	/* Register for Ext Clock */
	madera->ext_clk = devm_clk_get(madera->dev, "cs47l35_ext_clk");
	if (IS_ERR(madera->ext_clk)) {
		ret = PTR_ERR(madera->ext_clk);
		dev_err(madera->dev, "%s: clk_get failed, ret = %d\n",
				__func__, ret);
		goto err_reset;
	}
