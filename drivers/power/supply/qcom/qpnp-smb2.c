...
static int smb2_parse_dt(struct smb2 *chip)
{
	...
	rc = of_property_read_u32(node, "otg-voltage", &chg->otg_voltage);
	if (rc < 0)
		chg->otg_voltage = -EINVAL;
		...
		union power_supply_propval *val)
{
	...
	struct smb2 *chip = container_of(chg, struct smb2, chg);
...
static int smb2_init_hw(struct smb2 *chip)
{
	...
	if (chg->otg_voltage > 0) {
		rc = smblib_masked_write(chg, SSUPPLY_VBOOST_REG,
					 OTG_VOLTAGE_MASK, chg->otg_voltage);
		if (rc < 0) {
			pr_err("Couldn't set otg voltage rc=%d\n", rc);
			return rc;
		}
	}
