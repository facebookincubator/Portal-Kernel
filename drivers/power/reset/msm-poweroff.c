...
struct gpio_desc *silego_shipmode_ctrl_gpio = NULL;
...
void msm_set_charger_state(bool connected)
{
	charger_connected = connected;
}
EXPORT_SYMBOL(msm_set_charger_state);
...
static void msm_restart_prepare(const char *cmd)
{
	...
	/*
	 * On crash and if download mode is disabled, use warm reset and
	 * disable sdi to reboot the device and to preserve DRAM for pstore ramoops
	 */
...
static void do_msm_poweroff(void)
{
	...
	if (silego_shipmode_ctrl_gpio && !charger_connected) {
		pr_notice("Power off by silego\n");
		gpiod_set_value(silego_shipmode_ctrl_gpio, 1);
		msleep(110);
	}
...
skip_sysfs_create:
	...
	silego_shipmode_ctrl_gpio = devm_gpiod_get(dev,
					"silego-shipmode-ctrl",
					GPIOD_ASIS);
	if (IS_ERR(silego_shipmode_ctrl_gpio))
		silego_shipmode_ctrl_gpio = NULL;
	else
		gpiod_direction_output(silego_shipmode_ctrl_gpio, 0);
