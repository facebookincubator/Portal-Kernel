...
$(INSTALLED_DTBIMAGE_TARGET): $(INSTALLED_KERNEL_TARGET)
	cat $(KERNEL_OUT)/arch/$(KERNEL_ARCH)/boot/dts/facebook/$(TARGET_DEVICE)/*.dtb > $@
...
ext_mod_src2out = $(patsubst %,$(TARGET_OUT_INTERMEDIATES)/%$(2),$(1))
ext_mod_out2src = $(patsubst $(TARGET_OUT_INTERMEDIATES)/%,%,$(1))

KERNEL_CONF := $(abspath $(KERNEL_OUT)/scripts/kconfig/conf)
KERNEL_MCONF := $(abspath $(KERNEL_OUT)/scripts/kconfig/mconf)

KERNEL_MODULES_PATH = $(sort $(abspath $(KERNEL_MODULES_BUILD) $(KERNEL_MODULES_INCS)))
KERNEL_MODULES_UAPI = $(addprefix $(BUILD_ROOT_LOC),$(KERNEL_MODULES_BUILD))
...
$(TARGET_PREBUILT_INT_KERNEL): $(KERNEL_OUT) $(KERNEL_HEADERS_INSTALL)
	$(hide) echo "Building kernel..."
	$(MAKE) -C $(TARGET_KERNEL_SOURCE) O=$(BUILD_ROOT_LOC)$(KERNEL_OUT) $(KERNEL_MAKE_ENV) ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(KERNEL_CROSS_COMPILE) $(real_cc) $(KERNEL_CFLAGS) KBUILD_EXTMOD_INCS="$(KERNEL_MODULES_PATH)"
	$(MAKE) -C $(TARGET_KERNEL_SOURCE) O=$(BUILD_ROOT_LOC)$(KERNEL_OUT) $(KERNEL_MAKE_ENV) ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(KERNEL_CROSS_COMPILE) $(real_cc) $(KERNEL_CFLAGS) KBUILD_EXTMOD_INCS="$(KERNEL_MODULES_PATH)" modules
	$(MAKE) -C $(TARGET_KERNEL_SOURCE) O=$(BUILD_ROOT_LOC)$(KERNEL_OUT) INSTALL_MOD_PATH=$(BUILD_ROOT_LOC)../$(KERNEL_MODULES_INSTALL) INSTALL_MOD_STRIP=1 $(KERNEL_MAKE_ENV) ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(KERNEL_CROSS_COMPILE) $(real_cc) modules_install
	$(mv-modules)
	$(clean-module-folder)
endif
...
$(KERNEL_HEADERS_INSTALL): $(KERNEL_OUT)
	$(hide) if [ ! -z "$(KERNEL_HEADER_DEFCONFIG)" ]; then \
			...
			$(MAKE) -C $(TARGET_KERNEL_SOURCE) O=$(BUILD_ROOT_LOC)$(KERNEL_OUT) $(KERNEL_MAKE_ENV) ARCH=$(KERNEL_HEADER_ARCH) CROSS_COMPILE=$(KERNEL_CROSS_COMPILE) $(real_cc) \
				KBUILD_EXTMOD_UAPI="$(KERNEL_MODULES_UAPI)" headers_install;\
...
.PHONY: kernelconfig
kernelconfig: .KATI_NINJA_POOL := console
...
kernel-ext-modules: $(KERNEL_MODULES_BUILD)

# Rule for making independent external module
$(KERNEL_MODULES_BUILD): %: $(call ext_mod_src2out,%,/include/generated/autoconf.h) $(call ext_mod_src2out,%,/Kbuild)
	$(hide) $(MAKE) -C $(TARGET_KERNEL_SOURCE) O=$(BUILD_ROOT_LOC)$(KERNEL_OUT) $(KERNEL_MAKE_ENV) \
		ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(KERNEL_CROSS_COMPILE) $(real_cc) \
		EXTRA_CFLAGS="-include $(BUILD_ROOT_LOC)$@/include/generated/autoconf.h" \
		M=$(BUILD_ROOT_LOC)$@ KBUILD_EXTMOD_INCS="$(KERNEL_MODULES_PATH)" modules

# Dependency include/generated/autoconf.h -> include/config/auto.conf
$(call ext_mod_src2out,$(KERNEL_MODULES_BUILD),/include/generated/autoconf.h): %/include/generated/autoconf.h: %/include/config/auto.conf

# Create required auto-generated files from properly .config
#	$(ANDROID_OUT)/obj/$(module_path)/include/config/auto.conf
#	$(ANDROID_OUT)/obj/$(module_path)/include/generated/autoconf.h
# Note: auto-generated files will be update only if .config is newer or missing
$(call ext_mod_src2out,$(KERNEL_MODULES_BUILD),/include/config/auto.conf): %/include/config/auto.conf: %/.config $(KERNEL_CONF)
	$(eval MODOUT=$(patsubst %/include/config/auto.conf,%,$@))
	$(eval MODSRC=$(call ext_mod_out2src,$(MODOUT)))
	$(hide) mkdir -p $(@D) $(@D)/../generated
	$(hide) cd $(MODOUT) && env srctree=$(abspath $(MODSRC)) $(KERNEL_CONF) --silentoldconfig Kconfig

# Prepare .config file based of properly {project_name}_defconfig
#	source: ${module_path)/configs/$(TARGET_PRODUCT)_defconfig
#	target: $(ANDROID_OUT)/obj/$(module_path)/.config
# Note: .config file will be update only if _defconfig is newer or missing
$(call ext_mod_src2out,$(KERNEL_MODULES_BUILD),/.config): $(TARGET_OUT_INTERMEDIATES)/%/.config: %/configs/$(KERNEL_DEFCONFIG) $(KERNEL_CONF)
	$(eval MODOUT=$(patsubst %/.config,%,$@))
	$(eval MODSRC=$(call ext_mod_out2src,$(MODOUT)))
	$(hide) mkdir -p $(@D)
	$(hide) cd $(MODOUT) && env srctree=$(abspath $(MODSRC)) $(KERNEL_CONF) --defconfig=$(abspath $<) Kconfig

# Prepare Kbuild file in output folder
$(call ext_mod_src2out,$(KERNEL_MODULES_BUILD),/Kbuild): $(TARGET_OUT_INTERMEDIATES)/%/Kbuild: %/Kbuild.out
	$(hide) mkdir -p $(@D)
	$(hide) cp $^ $@

# Kernel menuconfig for specrific module
$(addsuffix -menuconfig,$(KERNEL_MODULES_BUILD)): %-menuconfig: %/configs/$(TARGET_PRODUCT)_defconfig $(KERNEL_CONF) $(KERNEL_MCONF)
	$(eval MODSRC=$(patsubst %-menuconfig,%,$@))
	$(eval MODOUT=$(call ext_mod_src2out,$(MODSRC)))
	@echo
	@echo "=============================================================="
	@echo "WARNING: Ninja build environment does not show ncurses output."
	@echo "         Please use following commands."
	@echo ""
	@echo "  # Make helpful tool mconf"
	@echo "  /usr/bin/make -C $(TARGET_KERNEL_SOURCE) O=$(BUILD_ROOT_LOC)$(KERNEL_OUT) ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(KERNEL_CROSS_COMPILE) menuconfig"
	@echo ""
	@echo "  # Show menuconfig of the module"
	@echo "  bash -c \"cd $(MODOUT) && env srctree=$(abspath $(MODSRC)) $(KERNEL_MCONF) Kconfig\""
	@echo ""
	@echo "  # Generate/Update current defconfig"
	@echo "  bash -c \"cd $(MODOUT) && env srctree=$(abspath $(MODSRC)) $(KERNEL_CONF) --savedefconfig=$(abspath $<) Kconfig\""

# Kernel menuconfig for specrific module
$(addsuffix -clean,$(KERNEL_MODULES_BUILD)):
	$(eval MODSRC=$(patsubst %-clean,%,$@))
	$(eval MODOUT=$(call ext_mod_src2out,$(MODSRC)))
	$(hide) echo "Cleanup $(MODOUT)"
	$(hide) rm -fr $(MODOUT)

$(KERNEL_CONF) $(KERNEL_MCONF): $(TARGET_PREBUILT_INT_KERNEL)
