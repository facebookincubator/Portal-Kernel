...
#define TRACE_IPADDR_LEN 128

TRACE_EVENT(tcp_state,

	TP_PROTO(char *dest_address,
		 int is_reset,
		 int state),

	TP_ARGS(dest_address, is_reset, state),

	TP_STRUCT__entry(
		__array(	char,		dest_address,	TRACE_IPADDR_LEN)
		__field(	int,		is_reset			)
		__field(	int,		state				)
	),

	TP_fast_assign(
		memcpy(__entry->dest_address, dest_address, TRACE_IPADDR_LEN);
		__entry->is_reset = is_reset;
		__entry->state = state;
	),

	TP_printk("dest_address=%s is_reset=%d state=%d",
		__entry->dest_address, __entry->is_reset, __entry->state)
);
