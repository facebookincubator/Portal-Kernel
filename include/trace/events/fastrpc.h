...
TRACE_EVENT(fastrpc_rpmsg_send,
	...
	TP_PROTO(int cid, uint64_t smq_ctx,
		uint64_t ctx, uint32_t sc,
	       	uint64_t addr, uint64_t size),

	TP_ARGS(cid, smq_ctx, ctx, sc, addr, size),
	...
	TP_printk("to cid %d: smq_ctx 0x%llx, ctx 0x%llx, sc 0x%x, addr 0x%llx, size %llu",
		__entry->cid, __entry->smq_ctx, __entry->ctx,
...
TRACE_EVENT(fastrpc_context_interrupt,
	...
	TP_printk("to cid %d: smq_ctx 0x%llx, ctx 0x%llx, sc 0x%x",
		__entry->cid, __entry->smq_ctx,
		__entry->ctx, __entry->sc)
);
...
TRACE_EVENT(fastrpc_context_restore,
	...
	TP_printk("for cid %d: smq_ctx 0x%llx, ctx 0x%llx, sc 0x%x",
		__entry->cid, __entry->smq_ctx,
		__entry->ctx, __entry->sc)
);
...
TRACE_EVENT(fastrpc_context_complete,
	...
	TP_ARGS(cid, smq_ctx, retval, ctx, sc),
	...
	TP_printk("from cid %d: smq_ctx 0x%llx, retval 0x%x, ctx 0x%llx, sc 0x%x",
...
TRACE_EVENT(fastrpc_context_alloc,
	...
	TP_printk("for: smq_ctx 0x%llx, ctx 0x%llx, sc 0x%x",
		__entry->smq_ctx, __entry->ctx, __entry->sc)
);
