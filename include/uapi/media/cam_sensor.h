 ...
 *                      needs to be probed
 * @eeprom_slave_addr  :   EEPROM slave address
 * @eeprom_id_reg_addr :   Register address for EEPROM Id
 * @eeprom_id          :   EEPROM Id
 * @eeprom_id_mask     :   Mask for EEPROM Id
 * @camera_project_id_reg_addr  :   Register address for camera porject Id
 * @camera_project_id           :   Camera project Id
 * @eeprom_name                 :   EEPROM name
 * @privacy_probe               :   Indicates if camera probe to be skipped
 *                                  when booting in privacy.
 * @reserved
 */
struct cam_cmd_probe {
	...
	uint32_t    eeprom_id_reg_addr;
	uint32_t    eeprom_id;
	uint32_t    eeprom_id_mask;
	uint32_t    camera_project_id_reg_addr;
	uint32_t    camera_project_id;
	char        eeprom_name[32];
	uint16_t    privacy_probe;
	uint16_t    reserved;
} __attribute__((packed));
