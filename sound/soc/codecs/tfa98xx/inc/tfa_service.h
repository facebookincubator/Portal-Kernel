...
enum Tfa98xx_Error {
	Tfa98xx_Error_Ok = 0,
	/* 1. Currently only used to keep in sync with tfa_error */
	Tfa98xx_Error_Device,
	/* 2. */
	Tfa98xx_Error_Bad_Parameter,
	/* 3. generic failure, avoid mislead message */
	Tfa98xx_Error_Fail,
	/* 4. no clock detected */
	Tfa98xx_Error_NoClock,
	/* 5. */
	Tfa98xx_Error_StateTimedOut,
	/* 6. communication with the DSP failed */
	Tfa98xx_Error_DSP_not_running,
	/* 7. amp is still running */
	Tfa98xx_Error_AmpOn,
	/* 8. the given handle is not open */
	Tfa98xx_Error_NotOpen,
	/* 9. too many handles */
	Tfa98xx_Error_InUse,
	/* 10. if a buffer is too small */
...
enum Tfa98xx_Status_ID {
	...
	/* Invalid sequence of commands, in case the DSP expects some commands
	 * in a specific order
	 */
	Tfa98xx_I2C_Req_Invalid_Seq = 5,
	/* Generic error */
	Tfa98xx_I2C_Req_Invalid_Param = 6,
	/* I2C buffer has overflowed: host has sent too many parameters,
...
/**
 * If needed, this function can be used to get
 * a text version of the status ID code
 ...
 */
enum Tfa98xx_Error
tfa98xx_compare_features(struct tfa_device *tfa, int features_from_MTP[3],
...
/**
 * The wrapper functions to call the dsp msg,
 * register and memory function for tfa or probus
 */
...
int tfa_set_bf(struct tfa_device *tfa, const uint16_t bf,
 ...
 * @param p_reg_value a pointer to the register
 *        where to write the bitfield value
 */
...
#define TFA_GET_BF(tfa, fieldname) \
	tfa_get_bf(tfa, TFA_FAM(tfa, fieldname))
