...
struct tfa_device {
	...
	int default_profile;	/* default profile */
	int default_rate;	/* default rate */
 ...
 *       when the device responds but with an unknown id
 ...
 * Whenever a base state is requested an attempt is made to
 * actively bring the device into this state. However this may depend on
 * external conditions beyond control of this software layer.
 * Therefore in case the state cannot be set an erro will
 * be returned and the current state remains unchanged.
 * The base states, lower values below 0x10, are all mutually exclusive,
 * they higher ones can also function as a sticky modifier which means
 * for example that operating state could be in either muted or unmuted state.
 * Or in case of init_cf it can be internal clock (always)
 * or external audio clock.
 * This function is intended to be used for device mute/unmute synchronization
 * when called from higher layers.
 * Mostly internal calls will use this to control
