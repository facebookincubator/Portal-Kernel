...
typedef enum nxpTfa9872BfEnumList {
	...
	/* IC VDDP voltage ( 1023*VDDP/9.5 V) */
	...
	/* enable idle feature for channel 1 */
	...
	/* ram output on mode sam and audio */
	TFA9872_BF_SAMOSEL = 0x6970,
	/* low power mode 0 detection */
...
#define TFA9872_BITNAMETABLE static tfaBfName_t Tfa9872BitNames[] = {\
	...
	/* Power enable, directcontrol mode only             , */\
	{ 0x60d0, "pga_switch_enable"}, \
	/* Switch enable, directcontrol mode only            , */\
	{ 0x60e0, "pga_switch_aux_enable"}, \
	/* Switch enable aux, directcontrol mode only        , */\
	{ 0x6100, "force_idle"}, \
	/* force low power in idle mode                      , */\
	{ 0x6110, "bypass_idle"}, \
	/* bypass low power idle mode                        , */\
	{ 0x6123, "ctrl_attl"}, \
	/* Second channel gain in case of stereo using a single coil. (Total gain depending on INPLEV). (In case of mono OR stereo using 2 separate DCDC channel 1 should be disabled using TDMDCE), */\
	...
	/* class D gain ctrl_fb_50k ctrl_fb_100k             , */\
	...
	/* select tdm valid for speaker subsystem            , */\
	...
	{ 0x82a0, "cs_sam_set"}, \
	/* Enable SAM input for current sense                , */\
	{ 0x8305, "cs_ktemp"}, \
