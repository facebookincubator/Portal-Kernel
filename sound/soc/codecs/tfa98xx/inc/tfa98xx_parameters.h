...
/* These warnings are disabled because
 * it is only given by Windows and there is no easy fix
 */
...
/* This marker  is used to indicate if
 * all msgs need to be written to the device
 */
...
static const int nxpTfaSamplerateHz[fs_count] = {
...
typedef struct nxpTfaLsCompensationFilter {
	...
	/* Loudspeaker compensation on/off; when 'off',
	 * the DSP code doesn't apply the bwExt => bwExtOn GUI flag
	 * should be gray to avoid confusion
	 */
	uint8_t lsCompOn;
...
typedef struct nxpTfaIntegratorFilter {
	...
	float cutOffFreq; /* cut off freq in Hertz; range:[100.0 4000.0] */
...
typedef struct nxpTfaContAntiAlias {
	/* index determines destination type; anti-alias, integrator,eq */
	int8_t index;
	uint8_t type;
	float cutOffFreq;   /* cut off frequency */
	float samplingFreq;
