...
static int pcm_sample_format = 2;
					...
					int next_profile, int vstep)
{
	...
	if ((err == tfa_error_ok) && (tfa98xx->set_mtp_cal)) {
		...
		if (err_cal != tfa_error_ok) {
			pr_err("Error, set calibration value in mtp, err=%d\n",
				err_cal);
		} else {
					      ...
					      bool unregister)
{
	...
	for (i = 0; i < tfa_cnt_get_dev_nprof(tfa98xx->tfa); i++) {
		if (strnstr(tfa_cont_profile_name(tfa98xx, i), ".tap",
			strlen(tfa_cont_profile_name(tfa98xx, i)))) {
			tap_profile = true;
...
static ssize_t tfa98xx_dbgfs_spk_detect_read(struct file *file,
				     char __user *user_buf, size_t count,
				     loff_t *ppos)
{
	struct i2c_client *i2c = file->private_data;
...
static const struct file_operations tfa98xx_dbgfs_spk_detect_fops = {
	.open = simple_open,
	.read = tfa98xx_dbgfs_spk_detect_read,
	.llseek = default_llseek,
};
...
static void tfa98xx_debug_init(struct tfa98xx *tfa98xx, struct i2c_client *i2c)
{
	...
	debugfs_create_file("spk-detect", S_IRUGO, tfa98xx->dbg_dir,
			    i2c, &tfa98xx_dbgfs_spk_detect_fops);
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	if (profile < 0) {
		pr_err("tfa98xx: %s: invalid profile %d (mixer_profile=%d, rate=%d)\n",
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		...
		/* Flag DSP as invalidated as the profile change may
		 * invalidate the current DSP configuration.
		 * That way, further stream start can trigger a tfa_dev_start.
		 */
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	if (!name)
		return -ENOMEM;
	scnprintf(name, MAX_CONTROL_NAME, "TFA%s Profile", tfa98xx->fw.name);
	...
	for (prof = 0; prof < nprof; prof++) {
		 ...
		 * and add the container profile index to the
		 * supported samplerates of this mixer profile
		 */
	...
	scnprintf(name, MAX_CONTROL_NAME, "TFA%s Stop", tfa98xx->fw.name);
	tfa98xx_controls[mix_index].name = name;
	tfa98xx_controls[mix_index].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
...
static int tfa98xx_load_container(struct tfa98xx *tfa98xx)
{
	...
	return request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				tfa98xx->fw_name, tfa98xx->dev, GFP_KERNEL,
...
static int tfa98xx_probe(struct snd_soc_codec *codec)
{
	...
	/* TODO: dynamic control of mclk if necessary */
	clk_prepare_enable(tfa98xx->mclk);
...
static int tfa98xx_get_spk_detect_value(struct tfa98xx *tfa98xx)
{

	int ret;

	if (gpio_is_valid(tfa98xx->spk_gpio)) {
		ret = devm_gpio_request_one(tfa98xx->dev, tfa98xx->spk_gpio,
			GPIOF_DIR_IN, "TFA98XX_SPK_DETECT");
		if (ret)
			return 0;

		return gpio_get_value_cansleep(tfa98xx->spk_gpio);
	}
	return 0;
}
...
static int tfa98xx_parse_dt(struct device *dev, struct tfa98xx *tfa98xx,
	...
	tfa98xx->mclk = devm_clk_get(dev, "mclk");
	if (IS_ERR(tfa98xx->mclk)) {
		if (PTR_ERR(tfa98xx->mclk) == -EPROBE_DEFER)
			return -EPROBE_DEFER;
		tfa98xx->mclk = NULL;
	}
	...
	tfa98xx->spk_gpio =  of_get_named_gpio(np, "spk-gpio", 0);
	if (tfa98xx->spk_gpio < 0)
		dev_dbg(dev, "No spk GPIO provided.\n");

	if (!of_property_read_u32(np, "sample-rate", &val)) {
		if (val == 32000) {
			tfa98xx->rate = 32000;
			tfa98xx->default_profile = 6;
		} else if (val == 48000) {
			tfa98xx->rate = 48000;
			tfa98xx->default_profile = 8;
		}
	}

	if (!of_property_read_u32(np, "bit-width", &val)) {
		if (val == 32)
			pcm_sample_format = 2;
		else if (val == 24)
			pcm_sample_format = 1;
	...
	tfa98xx->spk_detect = tfa98xx_get_spk_detect_value(tfa98xx);

	if(tfa98xx->spk_detect == 1) {
		/* Check fw-name and use default name if not defined */
		if (of_property_read_string(np, "fw-name-2", &tfa98xx->fw_name))
			tfa98xx->fw_name = fw_name;
	} else {
		if (of_property_read_string(np, "fw-name-1", &tfa98xx->fw_name))
			tfa98xx->fw_name = fw_name;
	}
			     ...
			     const struct i2c_device_id *id)
{
	...
	tfa98xx->i2c = i2c;
	tfa98xx->dsp_init = TFA98XX_DSP_INIT_STOPPED;
	tfa98xx->rate = DEFAULT_SAMPLE_RATE; /* init to the default sample rate (48kHz) */
	tfa98xx->tfa = NULL;
	...
	tfa98xx->tfa->default_rate = tfa98xx->rate;
	tfa98xx->tfa->default_profile = tfa98xx->default_profile;
...
static int tfa98xx_i2c_remove(struct i2c_client *i2c)
{
	...
	if (gpio_is_valid(tfa98xx->spk_gpio))
		devm_gpio_free(&i2c->dev, tfa98xx->spk_gpio);
...
static int __init tfa98xx_i2c_init(void)
{
	...
	tfa98xx_cache = kmem_cache_create(
			...
			PAGE_SIZE, /* Structure size, fit in single page */
