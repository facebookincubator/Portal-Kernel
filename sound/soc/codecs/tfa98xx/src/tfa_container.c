					...
					nxpTfaVolumeStep2File_t *vp, int vstep)
{
	...
	if (vstep < vp->vsteps) {
		...
		vol = (unsigned short)
		      (-2 * float_to_int(*((uint32_t *)&vp->vstep[vstep].attenuation)));
				...
				int enable_partial_update)
{
	...
	/* The algoparams and mbdrc msg id will be changed to the
	 * reset type when SBSL=0
	 * if SBSL=1 the msg will remain unchanged.
	 * It's up to the tuning engineer to choose the 'without_reset'
	 * types inside the vstep. In other words:
	 * the reset msg is applied during SBSL==0 else it remains unchanged.
	 */
				...
				int vstep_idx, int vstep_msg_idx)
{
	...
	for (i = 0; i < nrMessages; i++) {
		...
		if ((vstep_msg_idx >= TFA_MAX_VSTEP_MSG_MARKER) ||
		    (vstep_msg_idx == i)) {
			err = tfaContWriteVstepMax2_One(tfa, msgInfo,
							p_msgInfo, enp);
			if (err != Tfa98xx_Error_Ok) {
				/*
...
/*
 * write a parameter file to the device
 * The VstepIndex and VstepMsgIndex are only used to write a specific msg
 * from the vstep file.
 */
				...
				int vstep_idx, int vstep_msg_idx)
{
	...
	case equalizerHdr:
		err = tfa_cont_write_filterbank(tfa,
				((nxpTfaEqualizerFile_t *)hdr)->filter);
		break;
	case patchHdr:
	...
	case drcHdr:
		if (hdr->version[0] == NXPTFA_DR3_VERSION) {
			 ...
			 * 127x3 (381) bytes first block contains the device
			 * and sample rate independent settings
			 * 127x3 (381) bytes block the device and
			 * sample rate specific values.
			...
			nxpTfaContBiquad_t *bq)
{
	...
	/* This is used to set back the index */
	...
	for (i = 0; i < runs; i++) {
		...
		if (dmem == Tfa98xx_DMEM_ERR) {
			...
			/* Dont exit with an error here,
			 * We could continue without problems
			 */
			return Tfa98xx_Error_Ok;
		}

		/* send a DSP memory message that targets
		 * the devices specific memory for the filter
...
enum Tfa98xx_Error tfaContWriteRegsProf(struct tfa_device *tfa, int prof_idx)
{
	...
	/* process the list until the end of
	 * the profile or the default section
	 */
	for (i = 0; i < prof->length; i++) {
		/* We only want to write the values before
		 * the default section when we switch profile
		 */
		...
		if (prof->list[i].type == dscRegister)
			err = tfaRunWriteRegister(tfa,
				(nxpTfaRegpatch_t *)
				(prof->list[i].offset+(char *)tfa->cnt));
		if (err)
			break;
	}
	return err;
...
static void create_dsp_buffer_msg(struct tfa_device *tfa,
		nxpTfaMsg_t *msg, char *buffer, int *size)
{
	int i, nr = 0;
					...
					nxpTfaDescPtr_t *dsc)
{
	...
	if (tfa->ext_dsp == 0 &&
	    !(dsc->type == dscBitfield ||
	    dsc->type == dscRegister))
		return Tfa98xx_Error_Ok;
			...
			unsigned int prof_idx)
{
	...
	return tfa->default_rate; /* default of HW */
}

static enum Tfa98xx_Error get_sample_rate_info(struct tfa_device *tfa,
		nxpTfaProfileList_t *prof, nxpTfaProfileList_t *previous_prof,
		int fs_previous_profile)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	nxpTfaBitfield_t *bitf;
	unsigned int i;
	int fs_default_profile = tfa->default_profile;	/* default */
	int fs_next_profile = tfa->default_profile;		/* default */
	...
	for (i = 0; i < previous_prof->length; i++) {
		...
		if (i < previous_prof->length) {
			if (previous_prof->list[i].type == dscBitfield) {
				bitf = (nxpTfaBitfield_t *)
					(previous_prof->list[i].offset+(uint8_t *)tfa->cnt);
				if (bitf->field == TFA_FAM(tfa, AUDFS)) {
	...
	if (fs_next_profile != fs_default_profile) {
		 ...
		 * the AUDFS from the default profile is not
		 * the same as the AUDFS from the previous profile
					...
					int vstep_idx)
{
	...
	nxpTfaProfileList_t *prof = tfaContGetDevProfList(tfa->cnt,
						tfa->dev_idx, prof_idx);
	nxpTfaProfileList_t *previous_prof = tfaContGetDevProfList(tfa->cnt,
						tfa->dev_idx,
	...
	nxpTfaFileDsc_t *file;
	int size = 0, ready, fs_previous_profile = tfa->default_profile;
	...
	/* We only make a power cycle when
	 * the profiles are not in the same group
	 */
	...
	if (prof->group != previous_prof->group || prof->group == 0) {
		...
		if (tfa->tfa_family == 2) {
			...
			TFA_SET_BF_VOLATILE(tfa, SBSL, 0);
			/* Sending commands to DSP we need to
			 * make sure RST is 0 (otherwise we get no response)
			 */
	...
	if (tfa->ext_dsp != 0) {
		...
		for (i = j; i < prof->length; i++) {
			/* We only want to write the values before the default
			 * section when we switch profile
			 */
		...
		case dscCmd:
			...
			if (tfa->ext_dsp != 0) {
				size = *(uint16_t *)
					(prof->list[i].offset+(char *)tfa->cnt);
				err = dsp_msg(tfa, size,
						prof->list[i].offset+2+(char *)tfa->cnt);
				if (tfa->verbose) {
					const char *cmd_id =
						prof->list[i].offset+2+(char *)tfa->cnt;
...
int tfaContGetCalProfile(struct tfa_device *tfa)
{
	...
	for (prof = 0; prof < tfa->cnt->nprof; prof++) {
		if (strnstr(tfaContProfileName(tfa->cnt, tfa->dev_idx, prof),
		    ".cal", strlen(tfaContProfileName(tfa->cnt, tfa->dev_idx, prof))) != NULL) {
			cal_idx = prof;
...
int tfaContIsTapProfile(struct tfa_device *tfa, int prof_idx)
{
	...
	if (strnstr(tfaContProfileName(tfa->cnt, tfa->dev_idx, prof_idx),
	    ".tap", strlen(tfaContProfileName(tfa->cnt, tfa->dev_idx, prof_idx))) != NULL) {
...
int tfa_tib_dsp_msgmulti(struct tfa_device *tfa, int length, const char *buffer)
{
	...
	/* TODO: get rid of these magic constants max size should
	 * depend on the tfa device type
	 */
