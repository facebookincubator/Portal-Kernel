...
no_overload_function_available(struct tfa_device *tfa,
					int not_used)
{
	(void)tfa;
	(void)not_used;
...
*/
static enum Tfa98xx_Error
tfa_dsp_system_stable(struct tfa_device *tfa, int *ready)
{
...
static enum Tfa98xx_Error
tfanone_dsp_system_stable(struct tfa_device *tfa, int *ready)
{
...
static int
tfanone_set_swprofile(struct tfa_device *tfa, unsigned short new_value)
{
...
/*****************************************************************************/
static enum Tfa98xx_Error
tfa9912_faim_protect(struct tfa_device *tfa, int status)
{
...
static enum Tfa98xx_Error tfa9912_specific(struct tfa_device *tfa)
{
	...
	if (tfa->rev == 0x1a13) {
		...
		reg_write(tfa, 0xc6, 0x004e); /* POR = 0x000e */
...
static enum Tfa98xx_Error tfa9912_auto_copy_mtp_to_iic(struct tfa_device *tfa)
{
	 ...
	 * Workaround for 72, 88 and 9912/9892(see PLMA5290)
	 */
	return reg_write(tfa, 0xA3, 0x20);
}

static int
tfa9912_set_swprofile(struct tfa_device *tfa, unsigned short new_value)
{
	int active_value = tfa_dev_get_swprof(tfa);
...
/* PLMA5322, PLMA5528 - Limit values of DCVOS and DCVOF to
 * range specified in datasheet.
 */
...
static enum Tfa98xx_Error tfa9872_specific(struct tfa_device *tfa)
{
	...
	/* Bypass OVP by setting bit 3 from register
	 * 0xB0 (bypass_ovp = 1): PLMA5258
	 */
...
static enum Tfa98xx_Error
tfa9888_tfa_dsp_write_tables(struct tfa_device *tfa, int sample_rate)
{
...
/*****************************************************************************/
static enum Tfa98xx_Error
tfa9896_faim_protect(struct tfa_device *tfa, int status)
{
	enum Tfa98xx_Error ret = Tfa98xx_Error_Ok;
...
*/
static enum
Tfa98xx_Error tfa9896_dsp_write_vsfwdelay_table(struct tfa_device *tfa)
{
...
static enum
Tfa98xx_Error tfa9896_dsp_write_cvfracdelay_table(struct tfa_device *tfa)
{
	return tfa_dsp_cmd_id_write(tfa, MODULE_FRAMEWORK,
...
static enum Tfa98xx_Error tfa9897_specific(struct tfa_device *tfa)
{
	...
	/* $48:[3] - 1 == > 0; iddqtestbst - default value changed.
	* When Iddqtestbst is set to "0", the slewrate is reduced.
	* This will lower the overshoot on IN-B
	* to avoid NMOS damage of booster
	*/
...
*/
static enum
Tfa98xx_Error tfa9897_dsp_write_vsfwdelay_table(struct tfa_device *tfa)
{
	return tfa_dsp_cmd_id_write(tfa, MODULE_FRAMEWORK,
				TFA1_FW_PAR_ID_SET_CURRENT_DELAY,
