			...
			int filter_index, unsigned short *address, int channel)
{
	...
	unsigned short bq_table[7][4] = {
		 ...
		 * Every index has 5 values except index 13 has 6 values
		 */
	};
				...
				enum Tfa98xx_saam *saam)
{
	...
	*saam = tfa->support_saam == supportYes ?
...
void tfa2_manual_mtp_cpy(struct tfa_device *tfa, uint16_t reg_row_to_keep, uint16_t reg_row_to_set, uint8_t row)
{
	uint16_t value;
	int loop = 0;
	enum Tfa98xx_Error error;
...
tfa98xx_set_mute_tfa2(struct tfa_device *tfa, enum Tfa98xx_Mute mute)
{
	...
	/* This thread performs this function several times in a row within a few
	 * milliseconds. Therefore, a small sleep is needed between each change
	 * in the mute state of the AMP to achive its normal operation.
	 */
	msleep(10);
			...
			const char *buf24)
{
	...
	if (tfa->ext_dsp == 1) {
		...
		if (error == Tfa98xx_Error_Buffer_too_small) {
			...
			/* (b) add the current DSP message
			 * to a new multi-message
			 */
			error = tfa_tib_dsp_msgmulti(tfa, length, buf);
			if (error == Tfa98xx_Error_Fail)
				return Tfa98xx_Error_Fail;
		...
		/* When the lastmessage is done we can
		 * send the multi-msg to the target
		 */
		if (lastmessage == 1) {
			...
			if (tfa->has_msg == 0) /* via i2c */ {
				/* Send tot the target selected */
				error = (tfa->dev_ops.dsp_msg)(tfa, length,
							(const char *)blob);
			} else { /* via msg hal */
...
tfa98xx_check_rpc_status(struct tfa_device *tfa, int *pRpcStatus)
{
	...
	/* 1) write DMEM = XMEM to the DSP XMEM */
	{
		...
		/* write the contents of CF_MAD which is the
		 * subaddress following CF_CONTROLS
		 */
...
enum Tfa98xx_Error
tfa98xx_dsp_write_mem_word(struct tfa_device *tfa, unsigned short address,
			int value, int memtype)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
	...
	*pbSupportDrc = 0;
	...
	if (tfa->supportDrc != supportNotSet) {
		*pbSupportDrc = (tfa->supportDrc == supportYes);
		...
		if (error == Tfa98xx_Error_Ok)
			tfa->supportDrc = *pbSupportDrc ?
					  supportYes : supportNo;
	}
	return error;
}
...
enum Tfa98xx_Error tfa_cf_powerup(struct tfa_device *tfa)
{
	...
	for (tries = CFSTABLE_TRIES; tries > 0; tries--) {
		...
		if (status)
			break;
		msleep_interruptible(10); /* to avoid busload */
	}
	if (tries == 0) { /* timedout */
...
enum Tfa98xx_Error tfaRunStartDSP(struct tfa_device *tfa)
{
	...
	/* Clear count_boot, should be reset to 0 before
	 * the DSP reset is released
	 */
...
enum Tfa98xx_Error tfaRunStartup(struct tfa_device *tfa, int profile)
{
	...
	tfa_dev_set_state(tfa, TFA_STATE_INIT_CF, strnstr(tfaContProfileName(tfa->cnt, tfa->dev_idx, profile),
		".cal", strlen(tfaContProfileName(tfa->cnt, tfa->dev_idx, profile))) != NULL);
	...
	*calibrateDone = 0;
	...
	if (TFA_GET_BF(tfa, MTPOTC)) {
		...
		if (tries_mtp_busy < MTPBWAIT_TRIES) {
			 ...
			 * is way to long!
			 * Setting this to 25 will take it
			...
			while ((*calibrateDone == 0) &&
				...
				*calibrateDone = TFA_GET_BF(tfa, MTPEX);
				if (*calibrateDone == 1)
					break;
				msleep_interruptible(50); /* to avoid busload */
				tries++;
			}
...
/*
 * tfa_dev_start will only do the basics: Going from powerdown to operating or
 * a profile switch.
 * for calibrating or akoustic shock handling use
 * the tfa98xxCalibration function.
 */
			...
			int vstep)
{
	 ...
	 */
	if (strnstr(tfaContProfileName(tfa->cnt, tfa->dev_idx, next_profile),
	    ".standby", strlen(tfaContProfileName(tfa->cnt, tfa->dev_idx, next_profile))) != NULL) {
	...
	/* PLMA5539: Gives information about
	 * current setting of powerswitch
	 */
	if (tfa->verbose) {
...
enum Tfa98xx_Error tfa_dsp_get_calibration_impedance(struct tfa_device *tfa)
{
	...
	if (tfa_dev_mtp_get(tfa, TFA_MTP_OTC)) {
		...
		/* Make sure the calibrateDone bit is set before
		 * getting the values from speakerboost!
		...
		error = tfa_dsp_cmd_id_write_read(tfa, MODULE_SPEAKERBOOST,
					SB_PARAM_GET_RE25C, nr_bytes, bytes);
		if (error == Tfa98xx_Error_Ok) {
...
int tfa_plop_noise_interrupt(struct tfa_device *tfa, int profile, int vstep)
{
	...
	if (tfa_irq_get(tfa, tfa9912_irq_stnoclk)) {
		...
		if (no_clk == 1) {
			 ...
			 *  Since the NOCLK interrupt is only enabled for 72
			 *  this is the place
			 *  However: Not tested yet! But also does not
			 *  harm normal flow!
			 */
			if (strnstr(tfaContProfileName(tfa->cnt, tfa->dev_idx, profile),
			    ".saam", strlen(tfaContProfileName(tfa->cnt, tfa->dev_idx, profile)))) {
