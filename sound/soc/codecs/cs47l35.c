...
static ssize_t fw_version_dsp2_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cs47l35 *cs = dev_get_drvdata(dev);

	return snprintf(buf, PAGE_SIZE, "0x%x\n",
			cs->core.adsp[1].fw_version_dsp2);
}
static const DEVICE_ATTR_RO(fw_version_dsp2);

static ssize_t fw_version_dsp3_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cs47l35 *cs = dev_get_drvdata(dev);

	return snprintf(buf, PAGE_SIZE, "0x%x\n",
			cs->core.adsp[2].fw_version_dsp3);
}
static const DEVICE_ATTR_RO(fw_version_dsp3);
...
static int cs47l35_probe(struct platform_device *pdev)
{
	...
	*/
	...
	for (i = 0; i < CS47L35_NUM_ADSP; i++) {
		...
		if (cs47l35->core.adsp[i].num == 2) {
			ret = sysfs_create_file(&pdev->dev.kobj,
					&dev_attr_fw_version_dsp2.attr);
		}

		if (cs47l35->core.adsp[i].num == 3) {
			ret = sysfs_create_file(&pdev->dev.kobj,
					&dev_attr_fw_version_dsp3.attr);
		}

		if (ret)
			dev_err(&pdev->dev, "Failed to create fw_version_dsp file\n");
	}
