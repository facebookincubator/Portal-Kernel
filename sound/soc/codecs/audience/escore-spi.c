				...
				void *buf, int len)
{
	...
	/*
	 * While streaming 1st, 2nd and 3rd, 4th bytes
	 * swapped, during read need to swap the bytes
	 */
#ifndef CONFIG_SND_SOC_ES817_CICERO
	for (rdcnt = 0; rdcnt < len; rdcnt += 4) {
		data = *((u32 *)buf);
			    ...
			    const void *buf, int len)
{
	...
	int offset = 0;
#endif
	/* Store the previous spi bits per word, to restore it back */
	if (escore_spi->bits_per_word != 8) {
		spi_bits_per_word = escore_spi->bits_per_word;
		rc = ESCORE_SPI_SET_BITS_PER_WORD(8);
	}
...
spi_write_err:
	...
	if (spi_bits_per_word != 0)
		rc = ESCORE_SPI_SET_BITS_PER_WORD(spi_bits_per_word);
