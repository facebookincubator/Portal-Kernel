...
/* This function must be called with access_lock acquired */
static int _es_stop_route(struct escore_priv *escore)
{
...
exit:
	pr_debug("put input control %s val %s\n", kcontrol->id.name,
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.enumerated.item[0] = value;
	pr_debug("get input control %s val %s\n", kcontrol->id.name,
...
exit:
	pr_debug("put output control %s val %s\n", kcontrol->id.name,
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	/* value = escore_read(NULL, reg); */
	ucontrol->value.enumerated.item[0] = value;
	pr_debug("get output control %s val %s\n", kcontrol->id.name,
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	if (atomic_read(&escore->active_streams)) {
		...
		pr_debug("%s:add algo preset: %d", __func__,
				escore->algo_preset);
		rc = escore_write_locked(NULL, ES_PRESET, escore->algo_preset);
		if (rc)
			dev_err(escore_priv.dev, "%s(): Set Preset failed:%d\n",
				__func__, rc);
	}
