...
#ifdef CONFIG_SND_SOC_ES817_SINGLE_WDB_TRANSFER
static int escore_response_check(struct escore_priv *escore)
{
	int rc;
	...
	defined(CONFIG_SND_SOC_ES_HIGH_BW_BUS_SPI)
		...
		rc = escore->bus.ops.high_bw_read(escore, &resp,
				sizeof(resp));
		if (rc < 0) {
			pr_err("%s(): WDB read error:%d\n",
					__func__, rc);
			return rc;
		}

		resp = escore->bus.ops.high_bw_bus_to_cpu(escore, resp);
		pr_debug("WDB Response 0x%0x\n", resp);
		if (resp != (ES_WRITE_DATA_BLOCK << 16)) {
			pr_debug("%s(): response not ready 0x%0x\n",
					__func__, resp);
			rc = -EIO;
		...
		size_t len)
{
	...
	size_t size = 0;
	size_t chunk_hdr_sz = 8;
	size_t remaining = len;
	u8 *dptr = (u8 *) buf;
	int num_of_file_size_bytes = 2;
	u32 resp;
	u32 resp32;
	...
	while (remaining) {
		...
		if (!loop_cnt) {
			...
			cmd = cmd | (dptr[0] << 8 | dptr[1]);
			dptr += 2;
			remaining -= num_of_file_size_bytes;
	...
	defined(CONFIG_SND_SOC_ES_HIGH_BW_BUS_SPI)
			...
			/* Write 8 byte header */
			rc = escore->bus.ops.high_bw_write(escore, dptr, chunk_hdr_sz);
			if (rc < 0) {
				pr_err("%s(): WDB error:%d\n", __func__, rc);
				goto out;
			}
			dptr += chunk_hdr_sz;
			remaining -= chunk_hdr_sz;
			/* After completing wdb response should be 0x802f0000,
			 * retry until we receive response*/

			/* Check for 8byte header response */
			rc = escore_response_check(escore);
			if (rc) {
				pr_err("%s(): 8byte header write error 0x%0x\n",
						__func__, rc);
				goto out;
			}
			loop_cnt = true;
		} /* End of if loop_count */

		/* Send the actual file, at one time write 512 bytes */
		pr_debug("Header 0x%x0x%x0x%x0x%x\n", dptr[0], dptr[1], dptr[2], dptr[3]);
		...
		int context)
{
	...
	escore->chip_mode = BARGEIN;
#else
	rc = es705_switch_route(ROUTE_VQ);
#endif
out:
	return rc;
}
