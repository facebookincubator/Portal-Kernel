...
#define MADERA_FW_VERSION_DSP2               0x120006
#define MADERA_FW_VERSION_DSP3               0x1a0006
/*
...
static int wm_adsp_load(struct wm_adsp *dsp)
{
	...
	if (dsp->num == 2) {
		ret = regmap_read(regmap, MADERA_FW_VERSION_DSP2,
				&dsp->fw_version_dsp2);
		adsp_info(dsp, "Madera firmware version dsp2 0x%x\n",
				dsp->fw_version_dsp2);
	} else if (dsp->num == 3) {
		ret = regmap_read(regmap, MADERA_FW_VERSION_DSP3,
				&dsp->fw_version_dsp3);
		adsp_info(dsp, "Madera firmware version dsp3 0x%x\n",
				dsp->fw_version_dsp3);

	} else {
		adsp_err(dsp, "Failed to read madera firmware version\n");
		if (ret != 0)
			goto out_fw;
	}
