...
struct audio_afe_clk_v2 {
	struct pinctrl_info pnctrl_info;
	struct clk c;
	struct afe_clk_set afe_clk;
	int port;
};
...
static inline struct audio_afe_clk_v2 *to_audio_ap_clk_v2(struct clk *clk)
{
	return container_of(clk, struct audio_afe_clk_v2, c);
}

static int audio_afe_clk_v2_prepare(struct clk *clk)
{
	struct audio_afe_clk_v2 *aclk = to_audio_ap_clk_v2(clk);
	struct pinctrl_info *pinfo = &aclk->pnctrl_info;
	int ret;

	if (pinfo->pinctrl && pinfo->active) {
		ret = pinctrl_select_state(pinfo->pinctrl, pinfo->active);
		if (ret)
			return ret;
	}

	aclk->afe_clk.enable = 1;
	return afe_set_lpass_clock_v2(aclk->port, &aclk->afe_clk);
}

static void audio_afe_clk_v2_unprepare(struct clk *clk)
{
	struct audio_afe_clk_v2 *aclk = to_audio_ap_clk_v2(clk);
	struct pinctrl_info *pinfo = &aclk->pnctrl_info;

	if (pinfo->pinctrl && pinfo->sleep)
		pinctrl_select_state(pinfo->pinctrl, pinfo->sleep);

	aclk->afe_clk.enable = 0;
	afe_set_lpass_clock_v2(aclk->port, &aclk->afe_clk);
}

static int audio_afe_clk_v2_set_rate(struct clk *clk, unsigned long rate)
{
	struct audio_afe_clk_v2 *aclk = to_audio_ap_clk_v2(clk);

	aclk->afe_clk.clk_freq_in_hz = rate;

	return 0;
}

static int audio_afe_clk_v2_is_enabled(struct clk *clk)
{
	struct audio_afe_clk_v2 *aclk = to_audio_ap_clk_v2(clk);

	return aclk->afe_clk.enable;
}
...
static struct clk_ops audio_afe_clk_v2_ops = {
	.prepare = audio_afe_clk_v2_prepare,
	.unprepare = audio_afe_clk_v2_unprepare,
	.set_rate  = audio_afe_clk_v2_set_rate,
	.is_enabled = audio_afe_clk_v2_is_enabled,
};
...
static struct audio_afe_clk_v2 audio_spk_i2s_clk = {
	.c = {
		.dbg_name = "audio_spk_i2s_clk",
		.ops = &audio_afe_clk_v2_ops,
		CLK_INIT(audio_spk_i2s_clk.c),
	},
	.afe_clk = {
		.clk_set_minor_version = AFE_API_VERSION_I2S_CONFIG,
		.clk_id = Q6AFE_LPASS_CLK_ID_SPEAKER_I2S_OSR,
		/* LRCLK=48kHz
		 * MCLK=256*LRCLK for S32LE
		 * MCLK=512*LRCLK for S16LE
		 */
