...
static int msm_ext_disp_edid_ctl_vendor_info(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_info *uinfo)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct msm_ext_disp_audio_codec_rx_data *codec_data;
	struct msm_ext_disp_vend_prod_edid_blk edid_blk;
	int rc;
	...
	if (!codec_data->ext_disp_ops.get_vend_prod_edid_blk) {
		dev_dbg(codec->dev, "%s: get_vend_prod_edid_blk() is NULL\n",
			__func__);
		uinfo->type = SNDRV_CTL_ELEM_TYPE_BYTES;
		uinfo->count = 0;
		return 0;
	...
	rc = codec_data->ext_disp_ops.get_vend_prod_edid_blk(
				codec_data->ext_disp_core_pdev, &edid_blk);

	if (!IS_ERR_VALUE(rc)) {
		uinfo->type = SNDRV_CTL_ELEM_TYPE_BYTES;
		uinfo->count = edid_blk.blk_size;
	}
...
static int msm_ext_disp_edid_vendor_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol) {
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct msm_ext_disp_audio_codec_rx_data *codec_data;
	struct msm_ext_disp_vend_prod_edid_blk edid_blk;
	...
	codec_data = snd_soc_codec_get_drvdata(codec);
	if (!codec_data || !codec_data->ext_disp_ops.get_vend_prod_edid_blk) {
		dev_err(codec->dev, "%s: codec_data or get_vend_prod_edid_blk() is NULL\n",
			__func__);
		return -EINVAL;
	}
	...
	if (!IS_ERR_VALUE(rc)) {
		...
		memcpy(ucontrol->value.bytes.data,
		       edid_blk.vend_prod_blk,
		       edid_blk.blk_size);

		dev_dbg(codec->dev, "%s: vend_prod_blk_size:%d,\n",
			__func__, edid_blk.blk_size);
	}
...
static const struct snd_kcontrol_new msm_ext_disp_codec_rx_controls[] = {
	...
	},
	{
		.access = SNDRV_CTL_ELEM_ACCESS_READ |
			  SNDRV_CTL_ELEM_ACCESS_VOLATILE,
		.iface  = SNDRV_CTL_ELEM_IFACE_PCM,
		.name   = "Vendor/Product Info EDID",
		.info   = msm_ext_disp_edid_ctl_vendor_info,
		.get    = msm_ext_disp_edid_vendor_get,
	},
		...
		struct platform_device *pdev)
{
	...
	if (!of_property_read_string(pdev->dev.of_node, "codec_name",
		&codec_name)) {
		dev_set_name(&pdev->dev, "%s", codec_name);
	}
