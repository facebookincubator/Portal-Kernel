...
#define DRV_NAME "ripley-asoc-snd"
...
#define TDM_SLOT_OFFSET_MAX 16

#define MSM_HIFI_ON 1

#define MSM_BACKEND_DBMDX_AMP_RX	(MSM_BACKEND_DAI_MAX + 1)
#define MSM_BACKEND_DBMDX_AMP_TX	(MSM_BACKEND_DAI_MAX + 2)
...
struct msm_asoc_mach_data {
	...
	const char *be_dl_name; /* BE dai-link name */
};
...
static unsigned int tdm_slot_offset[TDM_PORT_MAX][TDM_SLOT_OFFSET_MAX] = {
	{0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48, 52, 56, 60},/* TX_0 | RX_0 */
...
static struct dev_config mi2s_dbmdx_amp_rx = {
	SAMPLING_RATE_48KHZ, SNDRV_PCM_FORMAT_S32_LE, 2};

static struct dev_config mi2s_dbmdx_amp_tx = {
	SAMPLING_RATE_48KHZ, SNDRV_PCM_FORMAT_S32_LE, 2};
...
static SOC_ENUM_SINGLE_EXT_DECL(dbmdx_amp_rx_sample_rate, mi2s_rate_text);
...
static SOC_ENUM_SINGLE_EXT_DECL(dbmdx_amp_tx_sample_rate, mi2s_rate_text);
...
static SOC_ENUM_SINGLE_EXT_DECL(dbmdx_amp_rx_chs, mi2s_ch_text);
static SOC_ENUM_SINGLE_EXT_DECL(dbmdx_amp_tx_chs, mi2s_ch_text);
static SOC_ENUM_SINGLE_EXT_DECL(mi2s_rx_format, bit_format_text);
static SOC_ENUM_SINGLE_EXT_DECL(mi2s_tx_format, bit_format_text);
static SOC_ENUM_SINGLE_EXT_DECL(dbmdx_amp_rx_format, bit_format_text);
static SOC_ENUM_SINGLE_EXT_DECL(dbmdx_amp_tx_format, bit_format_text);
...
static int dbmdx_amp_rx_sample_rate_put(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct dev_config *dcfg = &mi2s_dbmdx_amp_rx;

	dcfg->sample_rate =
		mi2s_get_sample_rate(ucontrol->value.enumerated.item[0]);

	pr_debug("%s: bmdx_amp_rx_sample_rate = %d, item = %d\n", __func__,
		dcfg->sample_rate, ucontrol->value.enumerated.item[0]);

	return 0;
}

static int dbmdx_amp_rx_sample_rate_get(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct dev_config *dcfg = &mi2s_dbmdx_amp_rx;

	ucontrol->value.enumerated.item[0] =
		mi2s_get_sample_rate_val(dcfg->sample_rate);
...
static int dbmdx_amp_tx_sample_rate_put(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct dev_config *dcfg = &mi2s_dbmdx_amp_tx;

	mi2s_dbmdx_amp_tx.sample_rate =
		mi2s_get_sample_rate(ucontrol->value.enumerated.item[0]);

	pr_debug("%s: bmdx_amp_tx_sample_rate = %d, item = %d\n", __func__,
		dcfg->sample_rate, ucontrol->value.enumerated.item[0]);

	return 0;
}

static int dbmdx_amp_tx_sample_rate_get(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct dev_config *dcfg = &mi2s_dbmdx_amp_tx;
...
static int dbmdx_amp_rx_ch_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct dev_config *dcfg = &mi2s_dbmdx_amp_rx;

	pr_debug("%s: dbmdx_amp_rx_ch  = %d\n", __func__, dcfg->channels);
	ucontrol->value.enumerated.item[0] = dcfg->channels - 1;

	return 0;
}

static int dbmdx_amp_rx_ch_put(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct dev_config *dcfg = &mi2s_dbmdx_amp_rx;

	dcfg->channels = ucontrol->value.enumerated.item[0] + 1;
	pr_debug("%s: dbmdx_amp_rx_ch  = %d\n", __func__, dcfg->channels);

	return 1;
}

static int dbmdx_amp_tx_ch_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct dev_config *dcfg = &mi2s_dbmdx_amp_tx;

	pr_debug("%s: dbmdx_amp_tx_ch  = %d\n", __func__, dcfg->channels);
	ucontrol->value.enumerated.item[0] = dcfg->channels - 1;

	return 0;
}

static int dbmdx_amp_tx_ch_put(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct dev_config *dcfg = &mi2s_dbmdx_amp_tx;
...
static int dbmdx_amp_rx_format_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct dev_config *dcfg = &mi2s_dbmdx_amp_rx;

	ucontrol->value.enumerated.item[0] =
		mi2s_get_format_value(dcfg->bit_format);

	pr_debug("%s: dbmdx_amp_rx_format = %d, item = %d\n", __func__,
		dcfg->bit_format, ucontrol->value.enumerated.item[0]);

	return 0;
}

static int dbmdx_amp_rx_format_put(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct dev_config *dcfg = &mi2s_dbmdx_amp_rx;
...
static int dbmdx_amp_tx_format_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct dev_config *dcfg = &mi2s_dbmdx_amp_tx;
	...
	pr_debug("%s: dbmdx_amp_tx_format = %d, item = %d\n", __func__,
		dcfg->bit_format, ucontrol->value.enumerated.item[0]);

	return 0;
}

static int dbmdx_amp_tx_format_put(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct dev_config *dcfg = &mi2s_dbmdx_amp_tx;
...
static const struct snd_kcontrol_new msm_snd_controls[] = {
	...
	SOC_ENUM_EXT("QUAT_MI2S_RX SampleRate", quat_mi2s_rx_sample_rate,
			mi2s_rx_sample_rate_get,
			mi2s_rx_sample_rate_put),
	SOC_ENUM_EXT("DBMDX_AMP_RX SampleRate", dbmdx_amp_rx_sample_rate,
			dbmdx_amp_rx_sample_rate_get,
			dbmdx_amp_rx_sample_rate_put),
	...
	SOC_ENUM_EXT("QUAT_MI2S_TX SampleRate", quat_mi2s_tx_sample_rate,
			mi2s_tx_sample_rate_get,
			mi2s_tx_sample_rate_put),
	SOC_ENUM_EXT("DBMDX_AMP_TX SampleRate", dbmdx_amp_tx_sample_rate,
			dbmdx_amp_tx_sample_rate_get,
			dbmdx_amp_tx_sample_rate_put),
	...
	SOC_ENUM_EXT("QUAT_MI2S_TX Channels", quat_mi2s_tx_chs,
			msm_mi2s_tx_ch_get, msm_mi2s_tx_ch_put),
	SOC_ENUM_EXT("DBMDX_AMP_RX Channels", quat_mi2s_rx_chs,
			dbmdx_amp_rx_ch_get, dbmdx_amp_rx_ch_put),
	SOC_ENUM_EXT("DBMDX_AMP_TX Channels", quat_mi2s_tx_chs,
			dbmdx_amp_tx_ch_get, dbmdx_amp_tx_ch_put),
	...
	SOC_ENUM_EXT("QUAT_MI2S_TX Format", mi2s_tx_format,
			msm_mi2s_tx_format_get, msm_mi2s_tx_format_put),
	SOC_ENUM_EXT("DBMDX_AMP_RX Format", mi2s_rx_format,
			dbmdx_amp_rx_format_get, dbmdx_amp_rx_format_put),
	SOC_ENUM_EXT("DBMDX_AMP_TX Format", mi2s_tx_format,
			dbmdx_amp_tx_format_get, dbmdx_amp_tx_format_put),
				  ...
				  struct snd_pcm_hw_params *params)
{
	...
	case MSM_BACKEND_DBMDX_AMP_RX:
		param_set_mask(params, SNDRV_PCM_HW_PARAM_FORMAT,
			mi2s_dbmdx_amp_rx.bit_format);
		rate->min = rate->max = mi2s_dbmdx_amp_rx.sample_rate;
		channels->min = channels->max =
			mi2s_dbmdx_amp_rx.channels;
		break;

	case MSM_BACKEND_DBMDX_AMP_TX:
		param_set_mask(params, SNDRV_PCM_HW_PARAM_FORMAT,
			mi2s_dbmdx_amp_tx.bit_format);
		rate->min = rate->max = mi2s_dbmdx_amp_tx.sample_rate;
		channels->min = channels->max =
			mi2s_dbmdx_amp_tx.channels;
		break;
					 ...
					 unsigned long opcode, void *ptr)
{
	...
	case AUDIO_NOTIFIER_SERVICE_UP:
		...
		rtd = snd_soc_get_pcm_runtime(card, pdata->be_dl_name);
		if (!rtd) {
			dev_err(card->dev,
				"%s: snd_soc_get_pcm_runtime for %s failed!\n",
				__func__, pdata->be_dl_name);
			ret = -EINVAL;
			goto done;
		}
		codec = rtd->codec;
...
static int audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	int ret = 0;
...
static irqreturn_t amp_faultz_irq(int irq, void *priv)
{
	struct snd_soc_card *card = priv;
	struct msm_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);

	dev_err(card->dev, "AMP faultz event\n");
	if (!IS_ERR_OR_NULL(pdata->amp_mute_gpio)) {
		gpiod_direction_output(pdata->amp_mute_gpio, 0);
		dev_dbg(card->dev, "AMP was muted!\n");
	}
...
static int msm_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	dev_dbg(rtd->card->dev,
		"%s: pcm id = %s  stream = %d, dai name %s, dai ID %d\n",
		__func__, substream->pcm->id, substream->stream,
		cpu_dai->name, cpu_dai->id);
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK
		...
		if (!IS_ERR_OR_NULL(pdata->amp_mute_gpio)) {
			dev_dbg(rtd->card->dev,
					"%s: Enable Amp\n", __func__);
			/* Put render AMP in enable mode */
			ret = regulator_enable(pdata->pwr);
			if (!IS_ERR_OR_NULL(pdata->amp_sdz_gpio))
				gpiod_direction_output(
					pdata->amp_sdz_gpio, 1);
			gpiod_direction_output(pdata->amp_mute_gpio, 1);
		}
	}
clk_off:
	if (IS_ERR_VALUE(ret))
...
static void msm_mi2s_snd_shutdown(struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK
		...
		if (!IS_ERR_OR_NULL(pdata->amp_mute_gpio)) {
			dev_dbg(rtd->card->dev,
					"%s: Disable Amp\n", __func__);
			/* Put render AMP in disable mode */
			gpiod_direction_output(pdata->amp_mute_gpio, 0);
			if (!IS_ERR_OR_NULL(pdata->amp_sdz_gpio))
				gpiod_direction_output(
					pdata->amp_sdz_gpio, 0);
			regulator_disable(pdata->pwr);
		}
	}
}

static int msm_mi2s_snd_trigger(struct snd_pcm_substream *substream, int cmd)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
...
static struct snd_soc_dai_link ripley_msm_mi2s_be_dai_links[] = {
	{
		.name = LPASS_BE_PRI_MI2S_RX,
		.stream_name = "Primary MI2S Playback",
		.cpu_dai_name = "msm-dai-q6-mi2s.0",
...
static struct snd_soc_dai_link ripley_tfa98xx_dai_links[] = {
	{
		.name = LPASS_BE_QUAT_MI2S_RX,
		.stream_name = "Quaternary MI2S Playback",
		.cpu_dai_name = "msm-dai-q6-mi2s.3",
...
static struct snd_soc_dai_link ripley_dbmdx_tfa98xx_dai_links[] = {
	{
		.name = "DBMDX TDM1 Playback",
		.stream_name = "DBMDX TDM1 Playback",
		.platform_name = "snd-soc-dummy",
		.cpu_name = "dbmdx-codec",
		.cpu_dai_name = "DBMDX TDM1",
		.codec_name = "tfa98xx.6-0034",
		.codec_dai_name = "tfa98xx-aif-6-34",
		.no_pcm = 1,
		.dai_fmt = SND_SOC_DAIFMT_CBS_CFS | SND_SOC_DAIFMT_I2S,
		.dpcm_playback = 1,
		.be_id = MSM_BACKEND_DBMDX_AMP_RX,
		.be_hw_params_fixup = msm_be_hw_params_fixup,
		.init = &audrx_init,
		.ignore_suspend = 1,
		.ignore_pmdown_time = 1,
	},
	{
		...
		.dai_fmt = SND_SOC_DAIFMT_CBS_CFS | SND_SOC_DAIFMT_I2S,
		.dpcm_capture = 1,
		.be_id = MSM_BACKEND_DBMDX_AMP_TX,
		.be_hw_params_fixup = msm_be_hw_params_fixup,
		.ignore_suspend = 1,
	},
};

static struct snd_soc_dai_link_component ripley_primary_codecs[] = {
	{
		.name = "dbmdx-codec",
		.dai_name = "DBMDX TDM0",
	},
...
static struct snd_soc_dai_link ripley_dbmdx_tdm_pri_amp_dai_links[] = {
	{
		.name = LPASS_BE_PRI_TDM_RX_0,
		.stream_name = "Primary TDM0 Playback",
		.cpu_dai_name = "msm-dai-q6-tdm.36864",
		.platform_name = "msm-pcm-routing",
		.codecs = ripley_primary_codecs,
		.num_codecs = ARRAY_SIZE(ripley_primary_codecs),
		.no_pcm = 1,
		.init = &audrx_init,
	...
	},
	{
		...
		.cpu_dai_name = "msm-dai-q6-tdm.36865",
		.platform_name = "msm-pcm-routing",
		.codec_name = "dbmdx-codec",
		.codec_dai_name = "DBMDX TDM0",
		.no_pcm = 1,
		.dai_fmt = SND_SOC_DAIFMT_CBS_CFS,
		.dpcm_capture = 1,
		.be_id = MSM_BACKEND_DAI_PRI_TDM_TX_0,
...
static struct snd_soc_dai_link ripley_dbmdx_tdm_pri_dai_links[] = {
	{
		.name = LPASS_BE_PRI_TDM_RX_0,
		.stream_name = "Primary TDM0 Playback",
		.cpu_dai_name = "msm-dai-q6-tdm.36864",
...
static struct snd_soc_dai_link ripley_dbmdx_tdm_sec_dai_links[] = {
	{
		.name = LPASS_BE_SEC_TDM_RX_0,
		.stream_name = "Secondary TDM0 Playback",
		.cpu_dai_name = "msm-dai-q6-tdm.36880",
...
static struct snd_soc_dai_link ripley_dbmdx_tdm_quat_dai_links[] = {
	{
		.name = LPASS_BE_QUAT_TDM_RX_0,
		.stream_name = "Quaternary TDM0 Playback",
		.cpu_dai_name = "msm-dai-q6-tdm.36912",
...
static struct snd_soc_dai_link ripley_common_be_dai_links[] = {
	/* Hostless PCM purpose */
	{
...
static struct snd_soc_dai_link ripley_common_misc_fe_dai_links[] = {
	{
		.name = "MultiMedia6",
		.stream_name = "MultiMedia6",
		.cpu_dai_name = "MultiMedia6",
...
static struct snd_soc_dai_link ripley_dai_links[
			 ARRAY_SIZE(msm_common_dai_links) +
			 ARRAY_SIZE(ripley_common_be_dai_links) +
			 ARRAY_SIZE(ripley_common_misc_fe_dai_links) +
			 ARRAY_SIZE(ripley_tfa98xx_dai_links) +
			 ARRAY_SIZE(ripley_dbmdx_tfa98xx_dai_links) +
			 ARRAY_SIZE(ripley_dbmdx_tdm_pri_dai_links) +
			 ARRAY_SIZE(ripley_dbmdx_tdm_sec_dai_links) +
			 ARRAY_SIZE(ripley_dbmdx_tdm_quat_dai_links) +
			 ARRAY_SIZE(ripley_msm_mi2s_be_dai_links) +
			 ARRAY_SIZE(ext_disp_be_dai_link) +
			 ARRAY_SIZE(msm_wcn_be_dai_links)];
...
static struct snd_soc_card snd_soc_card_tasha_msm = {
	.name		= "msm8998-tasha-snd-card",
	.late_probe	= msm_snd_card_late_probe,
};

static struct snd_soc_card snd_soc_card_fb_ripley = {
	.name		= "fb-ripley-snd-card",
};
					...
					struct snd_soc_card *card)
{
	...
	for (i = 0; i < card->num_links; i++) {
		...
		if (dai_link[i].platform_name &&
		    strcmp(dai_link[i].platform_name, "snd-soc-dummy") &&
...
static const struct of_device_id ripley_asoc_machine_of_match[]  = {
	/* Mostly compatible with msm8998-tasha-snd-card. */
	{ .compatible = "facebook,ripley-snd",
	  .data = "ripley_codec"},
	{},
};
...
static struct snd_soc_card *populate_snd_card_dailinks(struct device *dev)
{
	...
	u32 extdisp_found = 0, tfa98xx_found = 0, dbmdx_found = 0;
	u32 dbmdx_tfa98xx_found = 0;

	match = of_match_node(ripley_asoc_machine_of_match, dev->of_node);
	if (!match) {
	...
	if (!strcmp(match->data, "tasha_codec")) {
	...
	} else if (!strcmp(match->data, "ripley_codec")) {
		card = &snd_soc_card_fb_ripley;
		memcpy(ripley_dai_links,
		       msm_common_dai_links,
		       sizeof(msm_common_dai_links));
		total_links = ARRAY_SIZE(msm_common_dai_links);

		memcpy(ripley_dai_links + total_links,
				ripley_common_be_dai_links,
				sizeof(ripley_common_be_dai_links));
		total_links += ARRAY_SIZE(ripley_common_be_dai_links);

		memcpy(ripley_dai_links + total_links,
				ripley_common_misc_fe_dai_links,
				sizeof(ripley_common_misc_fe_dai_links));
		total_links += ARRAY_SIZE(ripley_common_misc_fe_dai_links);

		of_property_for_each_string(dev->of_node, "asoc-codec-names",
				prop, codec_name) {
			dev_dbg(dev, "%s: codec_name=%s\n", __func__,
					codec_name);
			if (!strcmp(codec_name, "tfa98xx"))
				tfa98xx_found = 1;
			if (!strcmp(codec_name, "msm-ext-disp-audio-codec-rx"))
				extdisp_found = 1;
			if (!strcmp(codec_name, "dbmdx"))
				dbmdx_found = 1;
		}

		if (of_property_read_bool(dev->of_node, "tfa98xx,dbmdx-i2s"))
			dbmdx_tfa98xx_found |= true;

		if (tfa98xx_found) {
			memcpy(ripley_dai_links + total_links,
					ripley_tfa98xx_dai_links,
					sizeof(ripley_tfa98xx_dai_links));
			total_links += ARRAY_SIZE(ripley_tfa98xx_dai_links);
		}
		...
		if (dbmdx_found) {
			if (of_property_read_bool(dev->of_node,
					  "dbmdx,tdm-pri")) {
				memcpy(ripley_dai_links + total_links,
					ripley_dbmdx_tdm_pri_dai_links,
					sizeof(ripley_dbmdx_tdm_pri_dai_links));
				total_links += ARRAY_SIZE(ripley_dbmdx_tdm_pri_dai_links);
			} else if (of_property_read_bool(dev->of_node,
					  "dbmdx,tdm-pri-amp")) {
				memcpy(ripley_dai_links + total_links,
					ripley_dbmdx_tdm_pri_amp_dai_links,
					sizeof(ripley_dbmdx_tdm_pri_amp_dai_links));
				total_links += ARRAY_SIZE(ripley_dbmdx_tdm_pri_amp_dai_links);
			}

			if (of_property_read_bool(dev->of_node,
					  "dbmdx,tdm-sec")) {
				memcpy(ripley_dai_links + total_links,
					ripley_dbmdx_tdm_sec_dai_links,
					sizeof(ripley_dbmdx_tdm_sec_dai_links));
				total_links += ARRAY_SIZE(ripley_dbmdx_tdm_sec_dai_links);
			}

			if (of_property_read_bool(dev->of_node,
					  "dbmdx,tdm-quat")) {
				memcpy(ripley_dai_links + total_links,
					ripley_dbmdx_tdm_quat_dai_links,
					sizeof(ripley_dbmdx_tdm_quat_dai_links));
				total_links += ARRAY_SIZE(ripley_dbmdx_tdm_quat_dai_links);
			}

			if (of_property_read_bool(dev->of_node,
					  "fb,mi2s-audio-intf")) {
				memcpy(ripley_dai_links + total_links,
					ripley_msm_mi2s_be_dai_links,
					sizeof(ripley_msm_mi2s_be_dai_links));
				total_links += ARRAY_SIZE(ripley_msm_mi2s_be_dai_links);
			}
		}

		if (dbmdx_tfa98xx_found) {
			memcpy(ripley_dai_links + total_links,
					ripley_dbmdx_tfa98xx_dai_links,
					sizeof(ripley_dbmdx_tfa98xx_dai_links));
			total_links += ARRAY_SIZE(ripley_dbmdx_tfa98xx_dai_links);
		}
...
/*
 * Call back function for adsp loading
 */
				   ...
				   unsigned long opcode, void *data)
{
	...
	if (opcode == AUDIO_NOTIFIER_SERVICE_UP &&
		(cb_data->domain == AUDIO_NOTIFIER_ADSP_DOMAIN))
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	match = of_match_node(ripley_asoc_machine_of_match,
			pdev->dev.of_node);
	if (!match) {
	...
	pdata->faultz_irq = platform_get_irq_byname(pdev, "faultz");
	if (pdata->faultz_irq == -EPROBE_DEFER) {
		ret = pdata->faultz_irq;
		goto err;
	}

	pdata->pwr = devm_regulator_get(&pdev->dev, "pwr");
	if (IS_ERR(pdata->pwr)) {
		ret = PTR_ERR(pdata->pwr);
		goto err;
	}

	if (pdata->faultz_irq > 0) {
		/* GPIO amp_mute is mandatory if faultz irq is defined */
		pdata->amp_mute_gpio = devm_gpiod_get(&pdev->dev, "amp_mute",
				GPIOD_OUT_LOW);
		if (!IS_ERR_OR_NULL(pdata->amp_mute_gpio)) {
			dev_info(&pdev->dev, "Amp mute gpio registered\n");
			pdata->amp_sdz_gpio =
					devm_gpiod_get(&pdev->dev, "amp_sdz",
					GPIOD_OUT_HIGH);
		} else {
			ret = PTR_ERR(pdata->amp_mute_gpio);
			dev_err(&pdev->dev, "Can't request amp_mute, %d\n",
					ret);
			goto err;
		}

		ret = devm_request_irq(&pdev->dev, pdata->faultz_irq,
				amp_faultz_irq, IRQF_TRIGGER_FALLING,
				"amp_faultz", card);
		if (ret < 0) {
			dev_err(&pdev->dev, "Can't request amp_faultz IRQ, %d\n",
					pdata->faultz_irq);
			goto err;
		}
		dev_info(&pdev->dev, "Amp faultz_irq registered\n");
	}

	ret = of_property_read_string(pdev->dev.of_node, "be_dl_name",
					&pdata->be_dl_name);
	if (ret)
		pdata->be_dl_name = LPASS_BE_SLIMBUS_0_RX;
	...
	/* Setup a global ASOC device wakelock */
	ret = device_init_wakeup(&pdev->dev, true);
	if (ret) {
		pr_err("%s: soundcard PM wakelock init failed!\n", __func__);
	} else {
...
static struct platform_driver msm8998_asoc_machine_driver = {
	.driver = {
		...
		.owner = THIS_MODULE,
		.pm = &snd_soc_pm_ops,
		.of_match_table = ripley_asoc_machine_of_match,
	},
...
MODULE_DESCRIPTION("ALSA SoC Aloha");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:" DRV_NAME);
MODULE_DEVICE_TABLE(of, ripley_asoc_machine_of_match);
