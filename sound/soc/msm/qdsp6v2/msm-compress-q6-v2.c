...
static void msm_compr_fb_event(struct snd_compr_stream *stream, uint32_t ev_id)
{
	struct snd_soc_pcm_runtime *rtd;
		...
		uint32_t token, uint32_t *payload, void *priv)
{
	...
	case ASM_SESSION_EVENTX_OVERFLOW:
		msm_compr_fb_event(cstream, FB_ADSP_EVENT_TX_OVERFLOW);
		break;
	case ASM_SESSION_EVENT_RX_UNDERFLOW:
		msm_compr_fb_event(cstream, FB_ADSP_EVENT_RX_UNDERFLOW);
		break;
	default:
