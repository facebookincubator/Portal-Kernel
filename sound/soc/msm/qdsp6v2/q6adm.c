...
int adm_arrange_linear_map(u8 *map, int channels)
{
	u8 idx = 1;
		...
		int perf_mode)
{
	...
	if (tmp_port >= AFE_PORT_ID_TDM_PORT_RANGE_START &&
	    tmp_port <= AFE_PORT_ID_TDM_PORT_RANGE_END)
		ret = adm_arrange_linear_map(ep1_payload.dev_channel_mapping,
				ARRAY_SIZE(ep1_payload.dev_channel_mapping));
	else
		...
		int perf_mode)
{
	...
	if (tmp_port >= AFE_PORT_ID_TDM_PORT_RANGE_START &&
	    tmp_port <= AFE_PORT_ID_TDM_PORT_RANGE_END)
		ret = adm_arrange_linear_map(open.dev_channel_mapping,
				ARRAY_SIZE(open.dev_channel_mapping));
	else
...
int adm_send_device_latency_enable(int port_id, int copp_idx, int enable_flag)
{
	u32 latency_param;
	struct param_hdr_v3 param_hdr = {0};
	int ret = 0;

	pr_info("%s port_id: 0x%x, copp_idx %d latency: %d\n", __func__,
		 port_id, copp_idx, enable_flag);

	if (enable_flag < 0) {
		pr_err("%s: Invalid value for latency %d", __func__, enable_flag);
		return -EINVAL;
	}

	param_hdr.module_id = AUDPROC_MODULE_ID_LATENCY;
	param_hdr.instance_id = INSTANCE_ID_0;
	param_hdr.param_id = AUDPROC_PARAM_ID_ENABLE;
	param_hdr.param_size = sizeof(latency_param);

	latency_param = enable_flag;
...
int adm_send_device_latency(int port_id, int copp_idx, int latency)
{
	u32 latency_param;
	struct param_hdr_v3 param_hdr = {0};
	int ret = 0;
	...
	param_hdr.module_id = AUDPROC_MODULE_ID_LATENCY;
	param_hdr.instance_id = INSTANCE_ID_0;
	param_hdr.param_id = AUDPROC_PARAM_ID_LATENCY;
	param_hdr.param_size = sizeof(latency_param);
