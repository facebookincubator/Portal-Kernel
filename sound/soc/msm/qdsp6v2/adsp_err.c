...
static bool is_adsp_err_event_allowed(u32 adsp_err)
{
	bool ret = true;
	...
	case ADSP_EOK:
	case ADSP_EVERSION:
	case ADSP_ENORESOURCE:
	case ADSP_EALREADY:
	case ADSP_ENOTREADY:
	case ADSP_EPENDING:
	case ADSP_EBUSY:
	case ADSP_EPREEMPTED:
	case ADSP_ECONTINUE:
	case ADSP_EIMMEDIATE:
	case ADSP_ENOTIMPL:
	case ADSP_ERR_MAX:
		ret = false;
		break;
...
static void adsp_error_fb_event(u32 event_id, u32 adsp_err)
{
	union fb_adsp_event fb_event;
	int res = 0;

	if (event_id >= FB_ADSP_EVENT_MAX ||
		event_id != FB_ADSP_EVENT_QDSP_ERROR) {
		return;
	}
	if (adsp_err > ADSP_ERR_MAX) {
		adsp_err = ADSP_ERR_MAX;
	}

	fb_event.event_err = event_id;
	fb_event.state_err = adsp_err;

	if (false == is_adsp_err_event_allowed(adsp_err)) {
		pr_debug("%s: adsp error fb event blocked!\n",
			__func__);
		return;
	}
	res = fb_event_log_int(FBE_ADSP, fb_event.value);
	if (res) {
		pr_err("%s: adsp error fb event failure, res: %d!\n",
			__func__, res);
	}
}
...
{
	adsp_error_fb_event(FB_ADSP_EVENT_QDSP_ERROR, adsp_error);
