...
/* delay module topology */
#define DELAY_MODULE_TOPOLOGY	0x10001112
...
/* delay configured for delay module in micro seconds */
static int delay_value_us = 100000;
...
static int msm_routing_send_device_pp_params(int port_id,  int copp_idx,
					     int fe_id);
static void msm_enable_module_and_set_delay(int port_id, int copp_idx, int delay_value);
					  ...
					  uint32_t passthr_mode)
{
	...
	for (i = 0; i < MSM_BACKEND_DAI_MAX; i++) {
		...
		if (!is_be_dai_extproc(i) &&
			...
			if (topology == DELAY_MODULE_TOPOLOGY) {
				msm_enable_module_and_set_delay(msm_bedais[i].port_id,
					copp_idx,
					delay_value_us);
			}
			if (passthr_mode != COMPRESSED_PASSTHROUGH_DSD
		...
		if (msm_bedais[reg].active && fdai->strm_id !=
			...
			if ((fdai->perf_mode == LEGACY_PCM_MODE) &&
				...
				msm_pcm_routing_cfg_pp(msm_bedais[reg].port_id,
						       ...
						       channels);
			if (topology == DELAY_MODULE_TOPOLOGY) {
				msm_enable_module_and_set_delay(msm_bedais[reg].port_id,
				copp_idx,
				delay_value_us);
			}
		}
...
static int msm_delay_mod_value_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = delay_value_us;
	pr_debug("%s: delay_value = %ld\n", __func__,
			ucontrol->value.integer.value[0]);
	return 0;
}

static int msm_delay_mod_value_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	delay_value_us = ucontrol->value.integer.value[0];
	pr_debug("%s: delay_value = %d\n", __func__, delay_value_us);
	return 0;
}
...
static const struct snd_kcontrol_new delay_module_delay[] = {
	SOC_SINGLE_EXT("Delay Module delayUS", 0, 0, 0xffffffff,
		0, msm_delay_mod_value_get, msm_delay_mod_value_put),
};
		...
		if (fdai->strm_id != INVALID_SESSION) {
			...
			if ((fdai->perf_mode == LEGACY_PCM_MODE) &&
				...
				msm_pcm_routing_cfg_pp(bedai->port_id, copp_idx,
						       topology, channels);
			if (topology == DELAY_MODULE_TOPOLOGY) {
				msm_enable_module_and_set_delay(bedai->port_id,
				copp_idx,
				delay_value_us);
			}
		}
...
static int msm_routing_probe(struct snd_soc_platform *platform)
{
	...
	snd_soc_add_platform_controls(platform, delay_module_delay,
				ARRAY_SIZE(delay_module_delay));
...
int msm_routing_find_backend(int fe_id)
{
	int i;

	if (fe_id < 0 || fe_id > MSM_FRONTEND_DAI_MM_MAX_ID)
		return -EINVAL;

	for (i = 0; i < ARRAY_SIZE(msm_bedais); i++) {
		struct msm_pcm_routing_bdai_data *rd = &msm_bedais[i];

		if (test_bit(fe_id, &rd->fe_sessions[0]))
			return i;
	}
...
static void msm_enable_module_and_set_delay(int port_id, int copp_idx, int delay_value)
{
	adm_send_device_latency_enable(port_id,	copp_idx, 1);
	adm_send_device_latency(port_id, copp_idx, delay_value);
}
