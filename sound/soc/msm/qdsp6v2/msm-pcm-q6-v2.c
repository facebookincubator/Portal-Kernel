...
static bool msm_pcm_default_channel_map(struct msm_audio *portd)
{
	struct snd_pcm_substream *substream = portd->substream;
	struct snd_soc_pcm_runtime *prtd = substream->private_data;
	struct snd_soc_dai_link *dailink = prtd->dai_link;
	struct msm_pcm_routing_bdai_data be_dai;
	bool res = false;
	int be_id, i;

	/* Search for connected BE */
	be_id = msm_routing_find_backend(dailink->be_id);
	if (be_id < 0)
		return res;

	/* Get more info for the connected BE */
	msm_pcm_routing_get_bedai_info(be_id, &be_dai);

	/* Check if BE is TDM interface */
	if (be_dai.port_id >= AFE_PORT_ID_TDM_PORT_RANGE_START &&
	    be_dai.port_id <= AFE_PORT_ID_TDM_PORT_RANGE_END) {
		for (i = 0; i < ARRAY_SIZE(portd->channel_map); i++)
			portd->channel_map[i] = i + 1;
		res = true;
	}
...
static void msm_pcm_fb_event(struct snd_pcm_substream *stream, uint32_t ev_id)
{
	struct snd_soc_pcm_runtime *rtd;
	union fb_adsp_event event;

	if (!stream || !stream->private_data)
		return;

	rtd = stream->private_data;

	event.event = ev_id;
	event.dir = stream->stream;
	event.pcm = rtd->pcm->device;
	event.be_id = rtd->dai_link->be_id;

	fb_event_log_int(FBE_ADSP, event.value);
}
		...
		uint32_t token, uint32_t *payload, void *priv)
{
	...
	case APR_BASIC_RSP_RESULT: {
		...
		case ASM_SESSION_CMD_RUN_V2:
			...
			/* The appearance of new data after an EOS signal
			 * is a symptom for TX UNDERFLOW */
			if (prtd->rendered_eos) {
				prtd->rendered_eos = false;
				msm_pcm_fb_event(substream,
						FB_ADSP_EVENT_TX_UNDERFLOW);
			}
			break;
	...
	case ASM_SESSION_EVENTX_OVERFLOW:
		msm_pcm_fb_event(substream, FB_ADSP_EVENT_TX_OVERFLOW);
		break;
	case ASM_SESSION_EVENT_RX_UNDERFLOW:
		msm_pcm_fb_event(substream, FB_ADSP_EVENT_RX_UNDERFLOW);
		break;
	default:
...
static int msm_pcm_capture_prepare(struct snd_pcm_substream *substream)
{
	...
	if (q6asm_get_svc_version(APR_SVC_ASM) >=
		...
		ret = q6asm_enc_cfg_blk_pcm_format_support_v5(
					...
					prtd->samp_rate,
					prtd->channel_mode,
#ifdef CONFIG_SND_SOC_QDSP6V2_CAPTURE_CHANNEL_MAP
...
static int msm_pcm_open(struct snd_pcm_substream *substream)
{
	...
	prtd->enabled = IDLE;
	prtd->dsp_cnt = 0;
	prtd->set_channel_map = msm_pcm_default_channel_map(prtd);
...
static int msm_pcm_add_chmap_controls_sub(struct snd_soc_pcm_runtime *rtd,
						int stream)
{
	struct snd_pcm *pcm = rtd->pcm;
	...
	ret = snd_pcm_add_chmap_ctls(pcm, stream,
				     snd_pcm_std_chmaps,
	...
	for (i = 0; i < kctl->count; i++) {
		...
		kctl->vd[i].access &= ~(SNDRV_CTL_ELEM_ACCESS_TLV_READWRITE |
...
static int msm_pcm_add_chmap_controls(struct snd_soc_pcm_runtime *rtd)
{
	...
	pr_debug("%s, Playback Channel map cntrl add\n", __func__);
	ret = msm_pcm_add_chmap_controls_sub(rtd, SNDRV_PCM_STREAM_PLAYBACK);
	if (ret < 0) {
		pr_err("%s, playback channel map cntrl add failed\n", __func__);
		return ret;
	}

#ifdef CONFIG_SND_SOC_QDSP6V2_CAPTURE_CHANNEL_MAP
	pr_debug("%s, Capture Channel map cntrl add\n", __func__);
	ret = msm_pcm_add_chmap_controls_sub(rtd, SNDRV_PCM_STREAM_CAPTURE);
	if (ret < 0) {
		pr_err("%s, capture channel map cntrl add failed\n", __func__);
		return ret;
	}
#endif
