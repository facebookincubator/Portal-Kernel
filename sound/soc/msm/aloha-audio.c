...
#define DRV_NAME "aloha-asoc-snd"
...
static struct dev_config tdm_tx_cfg[TDM_INTERFACE_MAX][TDM_PORT_MAX] = {
	{ /* PRI TDM */
		{SAMPLING_RATE_16KHZ, SNDRV_PCM_FORMAT_S16_LE, 4}, /* TX_0 */
...
static int madera_dai_init(struct snd_soc_pcm_runtime *rtd)
{
	int ret;
	struct snd_soc_codec *codec = rtd->codec;

	/* Start the FLL as OFF */
	ret = snd_soc_codec_set_pll(codec, MADERA_FLL1_REFCLK,
			MADERA_FLL_SRC_NONE,
			0, 0);
	if (ret)
		goto madera_init_err;

	/* Set the input of the FLL as MCLK2 and the input of the fll
	 * as 19.2 MHz and the output as MARLEY_SYSCLK_RATE
	 */
	ret = snd_soc_codec_set_pll(codec, MADERA_FLL1_REFCLK,
			MADERA_FLL_SRC_MCLK1, /*MADERA_FLL_SRC_MCLK2,*/
			19200000, MADERA_SYSCLK_RATE);
	if (ret)
		goto madera_init_err;

	/* Use the FLL as the SYSCLK  at a rate of MARLEY_SYSCLK_RATE */
	...
	/* Use the FLL as the DSPCLK at the rate of MARLEY_DSPCLK_RATE */
	ret = snd_soc_codec_set_sysclk(codec, MADERA_CLK_DSPCLK,
			MADERA_CLK_SRC_FLL1, MADERA_DSPCLK_RATE,
			SND_SOC_CLOCK_IN);

	/* Configure AEC */
	snd_soc_write(codec, MADERA_DAC_AEC_CONTROL_1, 0x1);
	snd_soc_write(codec, MADERA_DAC_AEC_CONTROL_2, 0x5);
	snd_soc_write(codec, MADERA_AIF1_TX_ENABLES, 0x3);
	snd_soc_write(codec, MADERA_GPIO8_CTRL_1, 0x2000);
	snd_soc_write(codec, MADERA_GPIO9_CTRL_1, 0x2000);
	snd_soc_write(codec, MADERA_GPIO10_CTRL_1, 0x2000);
	snd_soc_write(codec, MADERA_GPIO10_CTRL_2, 0x7000);
	snd_soc_write(codec, MADERA_GPIO11_CTRL_1, 0x2000);
	snd_soc_write(codec, MADERA_AIF1TX1MIX_INPUT_1_SOURCE, 0x8);
	snd_soc_write(codec, MADERA_AIF1TX2MIX_INPUT_1_SOURCE, 0x9);
...
static int audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	if (!strcmp(dev_name(codec_dai->dev), "cs47l35-codec")) {
		ret = madera_dai_init(rtd);
		if (ret)
			return ret;
	}
	...
	for (i = TDM_0; i < TDM_PORT_MAX; i++) {
		...
		for (j = 0; j < TDM_SLOT_OFFSET_MAX; j++) {
			if (slot_offset[j] != AFE_SLOT_MAPPING_OFFSET_INVALID)
				slot_mask |=
				(1 << ((slot_offset[j] * slots) / slot_width));
			else
				break;
		}
	}
...
static struct snd_soc_dai_link aloha_dspg_dai_links[] = {
	{
		.name = LPASS_BE_PRI_MI2S_RX,
		.stream_name = "Primary MI2S Playback",
		.cpu_dai_name = "msm-dai-q6-mi2s.0",
...
static struct snd_soc_dai_link d4100_dai_links[] = {
	{
		.name = MSM_DAILINK_NAME(Media1),
		.stream_name = "MultiMedia1",
		.cpu_dai_name = "MultiMedia1",
...
static struct snd_soc_dai_link aloha_tas5760_dai_links[] = {
	{
		.name = LPASS_BE_QUAT_MI2S_RX,
		.stream_name = "Quaternary MI2S Playback",
		.cpu_dai_name = "msm-dai-q6-mi2s.3",
...
static struct snd_soc_dai_link aloha_tfa98xx_dai_links[] = {
	{
		.name = LPASS_BE_QUAT_MI2S_RX,
		.stream_name = "Quaternary MI2S Playback",
		.cpu_dai_name = "msm-dai-q6-mi2s.3",
...
static struct snd_soc_dai_link aloha_cs47l35_dai_links[] = {
	{
		.name = LPASS_BE_QUAT_MI2S_RX,
		.stream_name = "Quaternary MI2S Playback",
		.cpu_dai_name = "msm-dai-q6-mi2s.3",
...
static struct snd_soc_dai_link aloha_iaxxx_be_dai_links[] = {
#ifdef CONFIG_IAXXX_PCM_MODE
	{
		.name = LPASS_BE_PRI_MI2S_RX,
		.stream_name = "Primary MI2S Playback",
...
static struct snd_soc_dai_link aloha_common_be_dai_links[] = {
	/* Hostless PCM purpose */
	{
...
static struct snd_soc_dai_link aloha_dai_links[
			 ARRAY_SIZE(msm_common_dai_links) +
			 ARRAY_SIZE(aloha_common_be_dai_links) +
			 ARRAY_SIZE(aloha_iaxxx_be_dai_links) +
			 ARRAY_SIZE(aloha_cs47l35_dai_links) +
			 ARRAY_SIZE(aloha_tas5760_dai_links) +
			 ARRAY_SIZE(aloha_tfa98xx_dai_links) +
			 ARRAY_SIZE(aloha_dspg_dai_links) +
			 ARRAY_SIZE(ext_disp_be_dai_link) +
			 ARRAY_SIZE(msm_wcn_be_dai_links)];
...
static struct snd_soc_card snd_soc_card_aloha_evt0 = {
	.name		= "aloha-evt0-snd-card",
	.late_probe	= msm_snd_card_late_probe,
};

static struct snd_soc_card snd_soc_card_aloha_evt0_d4100 = {
	.name		= "aloha-evt0-d4100-snd-card",
};

static struct snd_soc_card snd_soc_card_aloha_evt1 = {
	.name		= "aloha-evt1-snd-card",
};

static struct snd_soc_card snd_soc_card_fb_aloha = {
	.name		= "fb-aloha-snd-card",
};
...
static const struct of_device_id aloha_asoc_machine_of_match[]  = {
	/* Mostly compatible with msm8998-tasha-snd-card. */
	{ .compatible = "qcom,msm8998-asoc-snd-aloha-evt0",
	  .data = "tasha_codec"},
	{ .compatible = "qcom,msm8998-asoc-snd-aloha-evt0-d4100",
	  .data = "iaxxx_codec"},
	{ .compatible = "facebook,msm8998-asoc-snd-aloha-evt1",
	  .data = "aloha_evt1_codec"},
	{ .compatible = "facebook,aloha-snd",
	  .data = "aloha_codec"},
	{},
};
...
static struct snd_soc_card *populate_snd_card_dailinks(struct device *dev)
{
	...
	u32 cs47l35_found = 0, tas5760_found = 0, iaxxx_found = 0;
	u32 extdisp_found = 0, tfa98xx_found = 0, dspg_found = 0;

	match = of_match_node(aloha_asoc_machine_of_match, dev->of_node);
	if (!match) {
	...
	if (!strcmp(match->data, "tasha_codec")) {
	...
	} else if (!strcmp(match->data, "iaxxx_codec")) {
		card = &snd_soc_card_aloha_evt0_d4100;
		dailink = d4100_dai_links;
		total_links = ARRAY_SIZE(d4100_dai_links);
	} else if (!strcmp(match->data, "aloha_evt1_codec")) {
		card = &snd_soc_card_aloha_evt1;
		memcpy(aloha_dai_links,
		       msm_common_dai_links,
		       sizeof(msm_common_dai_links));
		total_links = ARRAY_SIZE(msm_common_dai_links);

		memcpy(aloha_dai_links + total_links,
				aloha_common_be_dai_links,
				sizeof(aloha_common_be_dai_links));
		total_links += ARRAY_SIZE(aloha_common_be_dai_links);

		of_property_for_each_string(dev->of_node, "asoc-codec-names",
				prop, codec_name) {
			dev_dbg(dev, "%s: codec_name=%s\n", __func__,
					codec_name);
			if (!strcmp(codec_name, "iaxxx-codec"))
				iaxxx_found = 1;
			if (!strcmp(codec_name, "cs47l35"))
				cs47l35_found = 1;
			if (!strcmp(codec_name, "tas5760"))
				tas5760_found = 1;
		...
		if (iaxxx_found) {
			memcpy(aloha_dai_links + total_links,
					aloha_iaxxx_be_dai_links,
					sizeof(aloha_iaxxx_be_dai_links));
			total_links += ARRAY_SIZE(aloha_iaxxx_be_dai_links);
		}
		if (cs47l35_found && tas5760_found) {
			dev_err(dev, "%s: More than one playback codec in DT\n",
					__func__);
			return NULL;
		}
		if (cs47l35_found) {
			memcpy(aloha_dai_links + total_links,
					aloha_cs47l35_dai_links,
					sizeof(aloha_cs47l35_dai_links));
			total_links += ARRAY_SIZE(aloha_cs47l35_dai_links);
		}
		if (tas5760_found) {
			memcpy(aloha_dai_links + total_links,
					aloha_tas5760_dai_links,
					sizeof(aloha_tas5760_dai_links));
			total_links += ARRAY_SIZE(aloha_tas5760_dai_links);
		}
		if (extdisp_found) {
			dev_dbg(dev, "%s(): External display audio support present\n",
				__func__);
		...
		dailink = aloha_dai_links;
	} else if (!strcmp(match->data, "aloha_codec")) {
		card = &snd_soc_card_fb_aloha;
		memcpy(aloha_dai_links,
		       msm_common_dai_links,
		       sizeof(msm_common_dai_links));
		total_links = ARRAY_SIZE(msm_common_dai_links);
		...
		if (tfa98xx_found) {
			memcpy(aloha_dai_links + total_links,
					aloha_tfa98xx_dai_links,
					sizeof(aloha_tfa98xx_dai_links));
			total_links += ARRAY_SIZE(aloha_tfa98xx_dai_links);
		}
		if (dspg_found) {
			memcpy(aloha_dai_links + total_links,
			       aloha_dspg_dai_links,
			       sizeof(aloha_dspg_dai_links));
			total_links += ARRAY_SIZE(aloha_dspg_dai_links);
		}
		if (extdisp_found) {
			dev_dbg(dev, "%s(): External display audio support present\n",
				__func__);
...
/*
 * Dummy platform driver to kick deferred driver
 */
static const struct of_device_id aloha_dummy_match[] = {
...
static struct platform_driver fb_dummy_driver = {
	...
	.driver = {
		.name = "dprobe_trigger",
		.owner = THIS_MODULE,
		.of_match_table = aloha_dummy_match,
...
void fb_aloha_trigger_deferred_drivers(void)
{
	/*
	 * Due to race condition, audio sound card was not created
	 * if ADSP or Knowles codec is up later then the last
	 * successful driver probe.
	 * To fix this, probe a dummy driver to trigger
	 * all pending deferred driver one more time when
	 * AUDIO is up
	 */
...
EXPORT_SYMBOL(fb_aloha_trigger_deferred_drivers);
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	match = of_match_node(aloha_asoc_machine_of_match,
			pdev->dev.of_node);
	if (!match) {
...
static struct platform_driver msm8998_asoc_machine_driver = {
	.driver = {
		...
		.owner = THIS_MODULE,
		.pm = &snd_soc_pm_ops,
		.of_match_table = aloha_asoc_machine_of_match,
	},
...
MODULE_DEVICE_TABLE(of, aloha_asoc_machine_of_match);
